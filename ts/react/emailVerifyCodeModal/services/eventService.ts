...
const getSchematizedContext = (ctx: string): string => {
	...
	const contextMap: { [key: string]: string } = {
		[EVENT_CONSTANTS.context.validateOTP]:
			EVENT_CONSTANTS.context.schematizedEnterOTP,
		[EVENT_CONSTANTS.context.enterOTP]:
			EVENT_CONSTANTS.context.schematizedEnterOTP,
		[EVENT_CONSTANTS.context.sendOTP]:
			EVENT_CONSTANTS.context.schematizedSendOTP,
	};
...
export const sendOtpButtonClickEvent = (
	ctx: string,
	btn: string,
	origin: string,
): void => {
...
export const sendOtpCheckboxEvent = (
	...
	eventStreamService.sendEventWithTarget(eventTypes.buttonClick, ctx, {
		btn,
		field: isChecked
			? EVENT_CONSTANTS.field.checked
			: EVENT_CONSTANTS.field.unchecked,
		origin,
	});
};

export const sendOtpModalActionEvent = (
	ctx: string,
	aType: string,
	origin: string,
): void => {
