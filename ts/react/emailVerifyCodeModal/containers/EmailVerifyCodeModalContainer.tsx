...
export const EmailVerifyCodeModalContainer = ({
	...
	useEffect(() => {
		if (
			onEmailCodeEntered &&
			!legalCheckboxLabel &&
			debouncedCode.length === codeLength
		) {
			...
			dispatch({
				...
				isContinueButtonEnabled:
					debouncedCode.length === codeLength && isChecked,
	...
	useEffect(() => {
		if (legalCheckboxLabel && onComplete && isCodeValid) {
			...
			sendOtpButtonClickEvent(
				EVENT_CONSTANTS.context.enterOTP,
				EVENT_CONSTANTS.btn.signup,
				origin,
			);
			onComplete({
				otpSessionToken: sessionToken,
				otpContactType: contactType,
			});
	...
	return (
		<Modal
			className={
				renderInWebview
					? "email-verify-code-webview"
					: "email-verify-code-modal"
			}
			show={isModalOpen}
			size="lg"
			backdrop="static"
