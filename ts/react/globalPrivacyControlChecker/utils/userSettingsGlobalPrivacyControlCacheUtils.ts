...
const handleGlobalPrivacyControl = async (nowDate: Date) => {
	...
	try {
		const userSettingsGlobalPrivacyControlConfig =
			await universalAppConfigurationService.getUserSettingsGlobalPrivacyControlGuacPolicy();
		serverGpcVersion = userSettingsGlobalPrivacyControlConfig.gpcVersion;
		serverCooldownInMs =
			userSettingsGlobalPrivacyControlConfig.cooldownPeriodInMs;
		serverCooldownPeriodFuzzyInMs =
			userSettingsGlobalPrivacyControlConfig.cooldownPeriodFuzzyInMs;
	} catch (e) {
		...
		serverCooldownPeriodFuzzyInMs =
			globalPrivacyControlConstants.defaultCooldownPeriodFuzzyInMs;
	}
	...
	if (parsedCachedGlobalPrivacyControlStatus) {
		...
		const userGlobalPrivacyControlStatus =
			parsedCachedGlobalPrivacyControlStatus[id];
		...
		if (userGlobalPrivacyControlStatus) {
			const nextTimeToCheckAsISOString =
				userGlobalPrivacyControlStatus.nextTimeToCheck;
			try {
				...
				console.error(
					`Error with local storage, proceeding with globalPrivacyControl`,
				);
			}
		}
	}
	...
	try {
		...
		console.error(
			`Error updating local storage for handleGlobalPrivacyControl`,
		);
