...
const useGameImpressionsIntersectionTracker = (
	...
	const [reportedImpressions, setReportedImpressions] = useState<Set<number>>(
		new Set(),
	);
	...
	const sendImpressions = useCallback(() => {
		const subArrays = parsingUtils
			.splitArray(
				Array.from(impressedRange).filter(
					(index) => !reportedImpressions.has(index),
				),
	...
	useEffect(() => {
		...
		const getSortedImpressedIndexes = (
			...
			entries.forEach((entry) => {
				if (entry?.isIntersecting) {
					const tileIndex = tileElements.findIndex(
						(tileElement) => tileElement === entry.target,
					);
					if (tileIndex >= 0) {
		...
		const onObserve = (
			...
			const sortedImpressedIndexes = getSortedImpressedIndexes(
				entries,
				observer,
			);
