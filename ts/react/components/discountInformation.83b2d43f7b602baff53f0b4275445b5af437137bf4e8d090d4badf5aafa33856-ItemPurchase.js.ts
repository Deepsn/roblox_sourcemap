	...
	const discountLines: NormalizedDiscountLine[] =
		discountInformation.discounts?.map(
			(d: DiscountLineInformation, index: number) => {
				...
				const localizedAttribution =
					d.localizedDiscountAttribution?.trim() || undefined;
				const label =
					localizedAttribution || d.discountCampaign || `Discount ${index + 1}`;
				return {
					discountAmount: amount,
					discountCampaign:
						d.discountCampaign || d.discountId || `line-${index}`,
