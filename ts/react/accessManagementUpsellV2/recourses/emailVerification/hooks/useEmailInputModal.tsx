		...
		<React.Fragment>
			...
			<input
				...
				placeholder={translate(
					addEmailToAccountKeys.LabelEmailInputPlaceholderOver13,
				)}
				value={userEmailInput}
				onFocus={() =>
					emailVerificationEventService.useAddEmailToAccountEvent("")
				}
				onChange={(e) => updateUserEmailInput(e.target.value)}
			/>
			{errorMsg && (
				<p className="text-error modal-error-message">{translate(errorMsg)}</p>
			)}
		</React.Fragment>
	);
