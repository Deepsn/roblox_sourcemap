...
export const hasAccountSwitcherInvalidSessionError = (
	switchResponse: TSwitchResponse,
): boolean => {
	...
	if (!(authenticatedUser.isAuthenticated && authenticatedUser.isUnder13)) {
		try {
			...
			if (userMetaData) {
				...
				let numOfLoggedInAccounts =
					userMetaData.logged_in_users_metadata.length;
				if (
				...
				isAccountLimitReached =
					numOfLoggedInAccounts >= accountSwitcherMaxAccounts;
			}
		} catch (e) {
			// eslint-disable-next-line no-console
...
export const parseLoggedInUsers = async (
	...
	const [userMetaData, isAccountLimitReached] = await parseUserMetaData(
	...
	if (userMetaData) {
		try {
			...
			if (
				userMetaData.removed_user_metadata &&
				userMetaData.removed_user_metadata.user_id !== ""
			) {
				...
				if (users) {
					if (userMetaData.active_user_id !== "") {
						activeUser = users.find(
							(user) => user.id.toString() === userMetaData.active_user_id,
						);
					}
					...
					usersAvailableForSwitching = users.filter(
						(user) =>
							user.id !== loggedOutUserId &&
							user.id.toString() !== userMetaData.active_user_id,
					);
				}
			}
		} catch (e) {
