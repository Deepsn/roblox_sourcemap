...
export class RestRecipeProvider implements RecipeProvider {
	...
	public getRecipe(
		iteration: number,
		cb: (params: ProbeRecipe | null) => void,
	): void {
		let url = this.url;

		const delimiter = url.indexOf("?") === -1 ? "?" : "&";
		url = `${url}${delimiter}iteration=${iteration}`;
		RestRecipeProvider.requester.getData(
			url,
			0,
				...
				try {
					...
					for (let i = 0, l = json.targets.length; i < l; i++) {
						...
						if (!isString(targetInfo.target)) {
							throw new Error(
								`${errPrefix}.targets[${i}].target: not a string`,
							);
						}
					}
