...
export default function openLeaveRobloxWarningModal(
	...
	Dialog.open({
		titleText:
			featurePremiumTranslationResource.get(
				LANG_KEYS.insufficientRobuxLeaveRobloxHeading,
				{},
			) || "Leaving Roblox",
		...
		acceptText:
			commonUiControlsTranslationResource.get(
				LANG_KEYS.commonUiContinueToPaymentAction,
				{},
			) || "Continue to Payment",
		onAccept: () => {
			...
			reportCounter(
				UPSELL_COUNTER_NAMES.ConfirmLeaveRobloxModal,
		...
		onDecline: () => {
			...
			reportCounter(
				UPSELL_COUNTER_NAMES.CancelledFromLeaveRobloxModal,
				itemPurchaseObj?.assetType,
			);
		},
		onCancel: () => {
