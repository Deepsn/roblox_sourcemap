...
export const postItemDetailsWithRetries = async (
	...
	try {
		...
		const itemDetailsHydrationCallResult = await Promise.resolve(
			...
			resultArray.push({
				...
				purchaseInfo:
					item.collectibleItemId === undefined
						? getPurchaseInfo(item, undefined)
						: undefined,
			});
		});
...
export const awaitHydrationForItems = async (
	...
	if (newItemsAwaitingHydration.length > 0 && retriesRemaining > 0) {
		await sleep(
			itemDetailsHydrationServiceTimings.ITEM_DETAIL_LOAD_SLEEP_INTERVAL,
		);
...
export const awaitHydrationForCollectibleDetails = async (
	...
	for (let i = 0; i < collectibleItemIds.length; i++) {
		...
		if (itemDetailEntry) {
			const hydratedItemDetail = getItemDetail(
				itemDetailEntry.id,
				itemDetailEntry.itemType,
			);
			if (
				hydratedItemDetail &&
				hydratedItemDetail.details.collectibleItemDetails === undefined
			) {
...
export const getItemDetails = async (
	...
	items.forEach((item) => {
		...
		if (hydratedItemDetail === null || hydratedItemDetail === undefined) {
			...
			hydratedItemDetail.status ===
				ItemDetailsHydrationStatus.DETAILS_LOADING ||
			hydratedItemDetail.status ===
				ItemDetailsHydrationStatus.DETAILS_NOT_HYDRATED
		) {
			if (
	...
	if (loadingCollectibleItemIds.length > 0 && hydrateCollectibleDetails) {
		...
		for (let i = 0; i < collectibleItemDetailsList.data.length; i++) {
			...
			const itemDetailEntry = hydratedItemDetails.find(
				(item) =>
					item.collectibleItemId === collectibleItemDetails.collectibleItemId,
			);
