...
export function handleResultFromPurchases(
	...
	if (result.status === 200) {
		...
		if (successCount > 0) {
			switch (predominantError.error) {
				...
				case BatchBuyPurchaseResults.PremiumNeeded:
					return {
						success: false,
						message:
							resources.batchBuyPartialSuccessPremiumNeededFailureMessage,
						params: {
							itemCountSuccess: successCount,
							itemCountFailure: predominantError.count,
						},
				...
				case BatchBuyPurchaseResults.InExperienceOnly:
					return {
						success: false,
						message:
							resources.batchBuyPartialSuccessInExperienceOnlyFailureMessage,
						params: {
							itemCountSuccess: successCount,
							itemCountFailure: predominantError.count,
						},
			...
			switch (predominantError.error) {
				...
				case BatchBuyPurchaseResults.InExperienceOnly:
					return {
						success: false,
						message: resources.inExperienceOnlyFailureMessage,
					};
				default:
					return {
						success: false,
		...
		result.status === 400 &&
		result.data.message.includes("InsufficientTotalBalance")
	) {
		return {
			success: false,
			message: resources.insufficientFundsFailureMessage,
...
export default function createMultiItemPurchaseModal() {
	...
	function MultiItemPurchaseModal({
		...
		const startTwoStepVerification = () => {
			systemFeedbackService.loading(
				translate("Message.TwoStepVerificationBatchPurchase"),
			);
			setIsTwoStepVerificationActive(true);
		};
		const stopTwoStepVerification = () => setIsTwoStepVerificationActive(false);
		const [
			hasTwoStepVerificationBeenCompleted,
			setHasTwoStepVerificationBeenCompleted,
		] = useState(false);
		...
		async function purchaseItem(item) {
			...
			try {
				...
				result.data.itemData = {
					assetId:
						item.itemType.toLowerCase() === ItemType.Asset
							? item.id
							: undefined,
					bundleId:
						item.itemType.toLowerCase() === ItemType.Bundle
		...
		const purchaseCollectibleItem = async (item) => {
			...
			if (
				...
				params.collectibleProductId =
					collectibleItemDetails.collectibleProductId;
			}
			...
			try {
				...
				if (
					...
					if (
						...
						result.data.reason =
							BatchBuyPurchaseResults.TwoStepVerificationRequired;
					} else if (
						data?.purchased === false &&
		...
		function handleResult(result) {
			const resultFeedback = handleResultFromPurchases(
				translate,
				result,
				startTwoStepVerification,
			);
		...
		async function purchaseItems() {
			...
			try {
				...
				if (
					data.message &&
					data.message.startsWith("Error/EconomicRestrictions")
				) {
					handleResult(result);
