...
const [LeaveRobloxWarningModal, LeaveRobloxWarningModalService] =
	createLeaveRobloxWarningModal();
...
export function BatchBuyItems({
	...
	const isItemPurchasable = (item) => {
		if (item.collectibleItemId !== undefined) {
			return (
				(item.isMarketPlaceEnabled && item.isPurchasable) ||
				item.resellerAvailable
			);
		}
		return (
	...
	return (
		<React.Fragment>
			...
			{robuxNeeded > 0 && (
				<div id="insufficient-funds-modal">{insufficientFundsModal}</div>
			)}
			{shouldRedirectToVng && (
