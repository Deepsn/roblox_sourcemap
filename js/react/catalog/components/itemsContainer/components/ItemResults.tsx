...
function ItemResults(
	props: ItemResultsProps & WithTranslationsProps,
): JSX.Element {
	const {
		layout,
		isPaginationEnabled,
		...
		(item: ItemWithDetails) => {
			...
			const shoppingCartProps: ShoppingCartProps = {
				...
				removeItemFromCart: (itemId, itemType, displaySystemFeedback) => {
					trackShoppingCartRemoveClick(TCartActionSource.CatalogItemCard, {
						itemId,
						itemType,
					});
			...
			if (processedTimedOptions && processedTimedOptions.length > 0) {
				const selectedTimedOption = processedTimedOptions.find(
			...
			<React.Fragment>
				...
				>
					{orderedItems?.map((item) => {
						if (item.debugInfo) {
							const { dataSource, engagementScore, relevanceScore } =
								item.debugInfo;
	...
	return (
		<React.Fragment>
			...
			{layout.initialized && layout.loading && (
