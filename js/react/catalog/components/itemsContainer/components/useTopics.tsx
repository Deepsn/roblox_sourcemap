...
const useTopics = (
	...
	const updateWidths = useCallback(() => {
		if (topicCarouselComponent) {
			setContainerOverflow(
				topicCarouselComponent.scrollWidth - topicCarouselComponent.offsetWidth,
			);
		}
	...
	const onTopicNavigationButtonClicked = (left: boolean) => {
		...
		if (left) {
			topicCarousel.scrollLeft -=
				catalogConstants.topics.topicCarouselScrollValue;
	...
	const [showLeftTopicNavigationButton, setShowLeftTopicNavigationButton] =
		useState<boolean>();
	const [showRightTopicNavigationButton, setShowRightTopicNavigationButton] =
		useState<boolean>();
	const [showClearTopicNavigationButton, setShowClearTopicNavigationButton] =
		useState<boolean>();

	useEffect(() => {
		const carouselPositionRelative = isRTL
			? carouselPosition * -1
			: carouselPosition;
		const hasOverflow = containerOverflow > 0;
		const showLeftButton = hasOverflow && carouselPositionRelative > 0;
		const showRightButton =
			hasOverflow && carouselPositionRelative + 1 < containerOverflow;
