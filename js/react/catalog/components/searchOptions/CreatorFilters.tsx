...
function CreatorFilter({
	...
	const renderCreatorFilter = (creator: Creator) => {
		const showInputField =
			creator.userId === customText ||
			(typeof creator.userId === "number" && creator.userId >= 2);
		...
		return (
			<li
				...
				style={{
					display:
						creator.userId === robloxUserId && isUGCOnly ? "none" : "block",
				}}
			>
				<input
					type="radio"
				...
				/>
				{!showInputField && (
					<label htmlFor={`radio-creator-${creator.userId}`}>
						{creator.name}
					</label>
				)}
				{showInputField && (
					<label
						htmlFor="radio-creator-custom"
						className="has-input creator-name-input"
					>
						<input
