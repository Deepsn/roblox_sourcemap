...
export type SortsFiltersProps = {
	...
	onSortAggregationChanged: (
		sortAggregation: SortAggregation["sortAggregation"],
	) => void;
};
		...
		<ul>
			{sortMenus.sortOptions.map((sortOption) => (
				...
				>
					...
					/>
					<label htmlFor={`radio-sort-option-${sortOption.sortType}`}>
						{sortOption.name}
					</label>
					{sortOption.hasSubMenu &&
						sortType?.sortType === sortOption.sortType && (
							<ul>
								{sortMenus.sortAggregations.map((sortAggregationOption) => (
									...
									>
										<input
											...
											checked={
												sortAggregationId ===
												sortAggregationOption.sortAggregation
											}
											onChange={() =>
												onSortAggregationChanged(
													sortAggregationOption.sortAggregation,
