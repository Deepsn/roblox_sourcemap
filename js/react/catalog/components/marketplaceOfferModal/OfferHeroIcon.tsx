...
export const OfferHeroIcon = ({
	modalHeroIcon,
}: OfferHeroIconProps): JSX.Element | null => {
	const assetId = parseRbxAssetId(modalHeroIcon);
	const foundationIconName =
		marketplaceOfferIconToFoundationName(modalHeroIcon);
	...
	useEffect(() => {
		...
		const iconEl =
			iconRef.current?.querySelector<HTMLElement>(".icon") ?? iconRef.current;
		if (!iconEl) {
			// eslint-disable-next-line no-console
			console.warn(
				"[Catalog.MarketplaceOfferModal] Foundation icon element not found in DOM",
			);
			return;
		}
		...
		// eslint-disable-next-line no-console
		console.log(
			"[Catalog.MarketplaceOfferModal] Foundation icon computed style",
			{
				foundationIconName,
				...
				likelyPurged:
					!svgVar || svgVar === "none" || !maskImage || maskImage === "none",
			},
		);
	}, [foundationIconName]);
		...
		<div className="marketplace-offer-modal-hero" aria-hidden>
			<div className="marketplace-offer-modal-hero-frame">
				{foundationIconName ? (
					<span
						ref={iconRef}
						className="marketplace-offer-modal-hero-icon-wrapper"
					>
						<Icon
