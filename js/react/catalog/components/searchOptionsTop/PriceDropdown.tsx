...
function PriceDropdown({
	...
	const { showFreeBreadcrumb, showCustomPriceBreadcrumb, isPriceFilterNoOp } =
		usePriceBreadcrumb(searchOptions, catalogQuery);
			...
			<React.Fragment>
				{showFreeBreadcrumb && (
					<span className="filter-display-text">
						{translate("Label.BreadCrumb.Free")}
					</span>
				)}
					...
					<span className="filter-display-text">
						{minPrice && !maxPrice ? (
							<React.Fragment>
								...
								{translate("Label.BreadCrumb.PriceAbove", {
	...
	if (!defaultCurrencyType) {
		...
		const log = JSON.stringify({
			message:
				"Unable to render PriceDropdown because defaultCurrencyType was not found!",
		});
		// Report the error
		// We need to specify container name as counters name so we can build the graphs on grafana
		AXAnalyticsService.reportAXError({
			itemName,
			counterName: "PriceDropdownError",
			log,
		});
	...
	return (
		<CatalogFilter<CurrencyType["currencyType"]>
			filter={{
				...
				renderPillContents: isCurrencyFilterSelected
					? renderPillContents
					: undefined,
			...
			updateFilterValue={(newOptionValue, textValues) => {
				...
				if (
					...
					[newMinDisplayPrice, newMaxDisplayPrice] = [
						newMaxDisplayPrice,
						newMinDisplayPrice,
					]; // Swap min and max
				}
				...
				const newMinQueryPrice =
					convertDisplayPriceToQueryPrice(newMinDisplayPrice);
				applyMinPriceToQuery(newMinQueryPrice);

				setMaxPriceInputValue(newMaxDisplayPrice);
				const newMaxQueryPrice =
					convertDisplayPriceToQueryPrice(newMaxDisplayPrice);
				...
				const currencyType = currencyTypes.find(
					(c) => c.currencyType === newSelectedCurrencyTypeId,
				);
				if (currencyType) {
