...
const CatalogFilter = <TOptionId extends number | string>({
	...
	const [selectedOptionId, setSelectedOptionId] = useState<TOptionId>(
		filter.selectedOptionId,
	);
	const isDefaultOptionSelected =
		filter.selectedOptionId === filter.defaultOptionId;

	const [selectedSecondaryOptionId, setSelectedSecondaryOptionId] = useState<
		TOptionId | undefined
	...
	const resetFilterToDefault: React.MouseEventHandler<HTMLButtonElement> =
		useCallback(
			(event: React.MouseEvent) => {
				...
				const defaultSecondaryOption =
					filter.secondaryFilterProps?.secondaryFilterOptions.find(
						(option) =>
							option.optionId ===
	...
	if (isDropdownOpen) {
		...
		filterEndIcon = isDefaultOptionSelected
			? "icon-expand-arrow"
			: "icon-close";
	}
