...
export type SalesTypeDropdownProps = {
	...
	onSalesTypeFilterChanged: (
		salesTypeFilter: SalesTypeFilter["filter"],
	) => void;
...
function SalesTypeDropdown({
	...
	if (!defaultSalesType) {
		...
		const log = JSON.stringify({
			message:
				"Unable to render SalesTypeDropdown because defaultSalesType was not found!",
		});
		// Report the error
	...
	const isDefaultSalesTypeSelected =
		salesTypeFilter?.filter === defaultSalesType.filter;
	...
	return (
		<CatalogFilter<number>
			filter={{
				...
				selectedOptionId: salesTypeFilter
					? salesTypeFilter.filter
					: defaultSalesType.filter,
