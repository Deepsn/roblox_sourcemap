...
function CatalogPage({
	...
	const { topUp, setTopUp, isSearchOnBlurEnabled, getAvatarEnrollments } =
		useAvatarEnrollments();
	...
	const [ref] = useTrackCatalogViews({
		catalogQuery,
		numberOfAppliedFilters,
		searchItemsError,
	});
	...
	const [isCurrentCategorySearchable, setIsCurrentCategorySearchable] =
		useState<boolean>();
	...
	const [shouldRedirectToVng, setShouldRedirectToVng] =
		useState<boolean>(false);
	const [buyRobuxButtonUrl, setBuyRobuxButtonUrl] = useState<string>(
		catalogConstants.buyRobuxUrl,
	);
	const [showLeaveRobloxWarningModal, setShowLeaveRobloxWarningModal] =
		useState<boolean>(false);
	const [isMobileSearchOptionsOpen, setIsMobileSearchOptionsOpen] =
		useState<boolean>(false);
	...
	const clearPrice = (keepKeyword) => {
		setCurrencyType(
			searchOptions.currencyTypes ? searchOptions.currencyTypes[0] : undefined,
		);
	...
	const getCategoryOption = (originalCategory?: Category) => {
		return UtilityService.getCategoryMenu(
	...
	const categoryClick = (
		...
		if (
			!categoryClicked.subcategories ||
			categoryClicked.subcategories.length === 0
		) {
			clearTopics();
	...
	const categoryNameClick = (
		...
		if (
			document
				?.getElementById(
					`expandable-menu-category-${categoryClicked.categoryId}`,
				)
	...
	const onSortAggregationChanged = useCallback(
		(newSortAggregation: number) => {
			const updatedSortAggregation =
				searchOptions.sortMenus?.sortAggregations.find(
		...
		[
			searchOptions.sortMenus?.sortAggregations,
			setSortAggregation,
			catalogQuery.keyword,
		],
	);
	...
	const searchOptionsProps: Omit<SearchOptionsProps, "isMobile"> = {
		...
		menuConstantsCategoryTypesCommunityCreations:
			menuConstants.categoryTypes.CommunityCreations,
	...
	return (
		...
		>
			<div className="alert-system-feedback">
				<div className={`alert alert-warning ${showError ? "on" : ""}`}>
					<span className="alert-content">
						{translate("Response.Error.Filter")}
					</span>
					<button
						type="button"
						aria-label="Close"
			...
			<div
				...
				data-identifier="direct-purchase"
			/>
			<div
				id="catalog-content"
