...
export function WrappedSearchBar(props: WrappedSearchBarProps): JSX.Element {
	...
	const [autocompleteLanguageCode, setAutocompleteLanguageCode] =
		useState<string>();
	...
	const [useAutocompleteFallback, setUseAutocompleteFallback] =
		useState<boolean>();
	...
	const getUserLocale = () => {
		...
		locale = locale.substring(
			0,
			regionChar !== -1 ? regionChar : locale.length,
		);
	...
	const onKeywordKeydown = (event: React.KeyboardEvent<HTMLInputElement>) => {
		...
		// Navigate down or handle tab key without shifting focus
			...
			const newSuggestion = document.querySelector(
				`#autocomplete-suggestion-li-${newPosition}`,
			);
		...
		(searchTerm: string) => {
			...
			CatalogAPIService.getAvatarRequestSuggestion(
				...
				function success(result) {
					...
					if (
						autocompleteSuggestionsResponse.Args.Prefix !== keywordForDisplay
					) {
						return;
					}
					...
					setAutocompleteSuggestions((prevAutocompleteSuggestions) => {
						...
						let avatarRequestSuggestionData =
							autocompleteSuggestionsResponse.Data;
						...
						avatarRequestSuggestionData.forEach((suggestion) => {
							...
							updatedAutocompleteSuggestions.textSuggestions.push(
								suggestion.Query,
							);
						});
	...
	const onAutocompleteSuggestionClicked = (
		suggestion: string,
		index: number,
	) => {
		search(suggestion);
