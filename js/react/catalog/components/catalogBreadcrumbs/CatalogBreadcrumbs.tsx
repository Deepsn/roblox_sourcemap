...
export function CatalogBreadcrumbs({
	...
	const { showCreatorBreadcrumb, isCreatorAGroup, creatorBreadcrumb } =
		useCreatorBreadcrumb(searchOptions, catalogQuery);
	...
	const showKeywordBreadcrumb =
		isKeywordCensored || (keyword && keyword.length > 0);

	const [isNonDefaultSubcategorySelected, setIsNonDefaultSubcategorySelected] =
	...
	const clearCreator = useCallback(
		(stopChaining = false, keepKeyword = false) => {
			const defaultCreator = creators?.find(
				(creator) => creator.userId === defaultCreatorId,
			);
					...
					<React.Fragment>
						...
						<li>
							<span className="text-link breadcrumb-link">
								{isKeywordCensored
									? catalogConstants.keywordSearch.censoredKey
									: keyword}
							</span>
						</li>
					</React.Fragment>
