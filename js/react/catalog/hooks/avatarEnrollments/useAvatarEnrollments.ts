...
function useAvatarEnrollments() {
	...
	const [isSearchOnBlurEnabled, setIsSearchOnBlurEnabled] =
	...
	const checkShowTopUpWidget = () => {
		...
		const showTopUp =
			Date.now() - lastClearedTimestamp > catalogConstants.topUp.resetTime;
	...
	const getAvatarShopRecommendationsAndSearchWebEnrollment = useCallback(() => {
		experimentationService
			.getABTestEnrollment<AvatarShopRecommendationsAndSearchWeb>(
				...
				experimentConstants.parameterNames
	...
	const getAvatarShopRecommendationsAndSearchEnrollment = useCallback(() => {
		experimentationService
			...
			.then(
				function success(result) {
					if (
						result?.infiniteScroll !== undefined &&
						result?.infiniteScroll !== null
					) {
	...
	const getAvatarMarketplaceGuidedFeaturesEnrollment = useCallback(() => {
		experimentationService
			...
			.then(
				function success(result) {
					if (
						result?.topUpEnabled !== undefined &&
						result?.topUpEnabled !== null
					) {
						setTopUp((prevTopUp) => ({
							...prevTopUp,
						...
						if (
							...
							if (result?.topUpVariant === "zeroBalance") {
								...
								setTopUp((prevTopUp) => ({
									...
									buttonUrl:
										"https://roblox.cashstar.com/gift-card/buy/?ref=1023gcwidget",
									giftCard: true,
								}));
							}
