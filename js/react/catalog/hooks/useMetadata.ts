...
function useMetadata() {
	const [isMetadataInitialized, setIsMetadataInitialized] =
		useState<boolean>(false);
	const [
		isPremiumPriceOnItemTilesEnabled,
		setIsPremiumPriceOnItemTilesEnabled,
	] = useState<boolean>();
	const [isPremiumIconOnItemTilesEnabled, setIsPremiumIconOnItemTilesEnabled] =
		useState<boolean>();
	const [
	...
	const [isRemoveAllSubcategoryEnabled, setIsRemoveAllSubcategoryEnabled] =
		useState<boolean>();
	...
	const getMetadataFromApi = () => {
		CatalogAPIService.getMetadataFromApi().then(
			function success(result) {
				setIsPremiumPriceOnItemTilesEnabled(
					!!result.data.isPremiumPriceOnItemTilesEnabled,
				);
				setIsPremiumIconOnItemTilesEnabled(
					!!result.data.isPremiumIconOnItemTilesEnabled,
				);
				setAutocompleteAvatarSearchNumToDisplay(
					result.data.autocompleteAvatarSearchNumToDisplay,
				);
				setIsCatalogAdsRowOnRecommendedPageEnabled(
					result.data.isCatalogAdsRowOnRecommendedPageEnabled,
				);
				setIsRemoveAllSubcategoryEnabled(
					!!result.data.isRemoveAllSubcategoryEnabled,
				);
