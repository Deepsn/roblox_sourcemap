...
export function useUrlParameterCleanup({
	...
	const cleanupUrlParameters = useCallback(() => {
		...
		if (searchOptions.isSearchOptionsLoaded) {
			...
			const rawQueryParams =
				UtilityService.getQueriesValueIntoInt(searchParams);
	...
	const constructQueriesFromURL = useCallback(
		(ignoreSearchParams?: boolean) => {
			const newCatalogQuery = initializeCatalogQuery(
				searchOptions,
				ignoreSearchParams,
			);
			...
			setMinPriceForDisplay(
				newCatalogQuery.minPrice == null ||
					newCatalogQuery.minPrice === undefined
					? ""
					: newCatalogQuery.minPrice,
			);
			setMaxPriceForDisplay(
				newCatalogQuery.maxPrice == null ||
					newCatalogQuery.maxPrice === undefined
					? ""
