...
function countDifferences(query1: CatalogQuery, query2: CatalogQuery): number {
	return (Object.keys(query1) as (keyof CatalogQuery)[])
		.filter(
			(key) =>
				key !== "topicBasedBrowsingEnabledForCategory" && key !== "keyword",
		)
		.reduce((count, key) => {
			...
			let isDifferent =
				JSON.stringify(query1[key]) !== JSON.stringify(query2[key]);
			// Only count subcategory difference if the category is the same
			if (
				key === "subcategory" &&
				isDifferent &&
				query1.category !== query2.category
			) {
				isDifferent = false;
			}
...
function useCountCatalogQueryDifferences(
	...
	const [numberOfAppliedFilters, setNumberOfAppliedFilters] =
		useState<number>(0);
	...
	useEffect(() => {
		...
		const defaultCatalogQuery: CatalogQuery = createCatalogQuery(
			searchOptionsData,
			true,
		);
