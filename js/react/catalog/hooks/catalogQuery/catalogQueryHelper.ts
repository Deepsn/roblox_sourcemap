...
export const applyCategory = (
	...
	CatalogQuery,
	"category" | "subcategory" | "topicBasedBrowsingEnabledForCategory"
> & {
		...
		if (foundTaxonomyMatch) {
			const topicBasedBrowsingEnabledForCategory: boolean =
				updatedCategory === undefined
					? false
					: catalogConstants.topics.categoriesToShowTopics.includes(
	...
	updatedCategory = categories?.find(
		(cat) => cat.categoryId === categoryIdToSearch,
	);
		...
		if (categoryIdToSearch === gearCategoryId) {
			updatedSubcategory = updatedCategory.subcategories?.find(
				(subcat) =>
					...
					(gearsId || updatedCategory?.subcategories?.[0].subcategoryId),
			);
...
const applySortMenus = (
	...
	if (sortOptions) {
		updatedSortType =
			sortTypeId == null
				? sortOptions[0]
				: sortOptions.find((sort) => sort.sortType === sortTypeId);
	} else {
	...
	if (sortAggregations) {
		updatedSortAggregation =
			sortAggregationId == null
				? sortAggregations[0]
				: sortAggregations.find(
						(sort) => sort.sortAggregation === sortAggregationId,
					);
	} else {
...
const applyCreatorValues = (
	...
	const { creators, defaultCreatorId, robloxUserId, customText } =
		searchOptions;
...
const applyCurrencyValue = (
	...
	const { currencyTypes, defaultCurrencyType, customRobuxFilterId } =
		searchOptions;
	...
	const currency: CurrencyType | undefined =
		currencyTypes.find((c) => c.currencyType === currencyTypeParam) ||
		defaultCurrencyType;
...
export const createCatalogQuery = (
	...
	const searchParams = new URLSearchParams(
		ignoreSearchParams ? "" : window.location.search,
	);
	...
	const newCatalogQuery: CatalogQuery = {
		...
		topicBasedBrowsingEnabledForCategory:
			categoryResult.topicBasedBrowsingEnabledForCategory,
		...applySortMenus(
			queryParams.SortType,
			queryParams.SortAggregation,
			searchOptions.sortMenus,
		),
		...applyCreatorValues(
			{
