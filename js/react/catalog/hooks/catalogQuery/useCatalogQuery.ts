...
function useCatalogQuery(
	...
	const numberOfAppliedFilters = useCountCatalogQueryDifferences(
		catalogQuery,
		searchOptionsData,
	);

	const [isCatalogQueryInitialized, setIsCatalogQueryInitialized] =
	...
	const initializeCatalogQuery = useCallback(
		(
			...
			const newCatalogQuery = createCatalogQuery(
				searchOptions,
				ignoreSearchParams,
			);
	...
	useEffect(() => {
		...
		if (
			currencyType &&
			currencyType.excludePriceSorts &&
			sortType &&
			sortType.isPriceRelated
		) {
	...
	useEffect(() => {
		const { defaultCategoryId, freeFilterId, defaultCurrencyType } =
			searchOptionsData;
