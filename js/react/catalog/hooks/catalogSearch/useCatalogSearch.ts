...
const useCatalogSearch = (params: CatalagSearchParams) => {
	...
	const [searchResultDict, setSearchResultDict] =
		useState<AssetsCollection | null>(null);
	const [searchResultList, setSearchResultList] = useState<string[] | null>(
		null,
	);
	const [modifiedQuery, setModifiedQuery] = useState<ModifiedQuery | null>(
		null,
	);
				...
				try {
					...
					if (elements.length === 4) {
						...
						if (key && value) {
							...
							const existingInfo: SearchItemsResponseDataSourceInfo =
								infoMap.get(trimmedKey) || {
	...
	const buildElasticSearchDebugMap = useCallback(
		(
			...
			const dataSource =
				elasticsearchDebugInfo?.searchResultDataSource || undefined;
			if (dataSource) {
				parseElasticSearchDebugInfoV1(
					dataSource,
			...
			const engagementScore =
				elasticsearchDebugInfo?.searchResultEngagementScore || undefined;
			if (engagementScore) {
			...
			const relevanceScore =
				elasticsearchDebugInfo?.searchResultRelevanceScore || undefined;
			if (relevanceScore) {
				parseElasticSearchDebugInfoV2(
					relevanceScore,
					elasticSearchDebugInfoMap,
	...
	const buildSearchResultData = useCallback(
		(
			...
			searchData?.forEach((originalItem: TItem) => {
				...
				if (
					showElasticDebugInfo &&
					elasticSearchDebugInfoMap.has(item.id.toString())
				) {
			...
			CatalogAPIService.getCatalogItemDetails(itemParamsMapKey, translate)
				.then(
					function success(details) {
						...
						UtilityService.updateSearchItemDetails(
							details,
							newSearchResultsDict,
						);
						...
						setSearchResultList((prevSearchResultList) => {
							...
							return prevSearchResultList.filter(
								(itemKey) => !missingKeys.has(itemKey),
							);
						});
						setSearchResultDict((prevDict) => ({
							...prevDict,
							...newSearchResultsDict,
						}));
					},
		...
		(items: TItem[], newTopics: Topic[]) => {
			...
			if (newTopics.length > 0) {
				newTopics.forEach((topic) => {
					const formattedTopicName = UtilityService.formatTopic(
						topic.displayName,
					);
			...
			setTopics([]);
			CatalogAPIService.postGetTopics(
				filteredItems,
				filteredTopics,
				keyword,
			).then(
						...
						function success(response) {
							...
							if (searchResult) {
								...
								if (selectedTopics.length <= 0) {
									if (
										UtilityService.isKeywordResultCensored(searchResultKeyword)
									) {
