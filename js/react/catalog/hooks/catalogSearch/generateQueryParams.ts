...
const setPriceParams = (
	...
	const { customRobuxFilterId, freeFilterId, defaultCurrencyId } =
		searchOptions;
	...
	if (currencyType != null && currencyType.currencyType !== defaultCurrencyId) {
		...
		if (currencyType.currencyType === customRobuxFilterId) {
			let minPrice =
				queryMinPrice && queryMinPrice.toString().length > 9
					? null
					: queryMinPrice;
			let maxPrice =
				queryMaxPrice && queryMaxPrice.toString().length > 9
					? null
					: queryMaxPrice;
	...
	if (category) {
		...
		if (subcategory != null) {
			if (category.categoryId === gearCategoryId) {
				const defaultSubcategoryId = parseInt(
					catalogConstants.defaults.subcategory,
					10,
				);
		...
		Object.entries(params).forEach(([key, value]) => {
			...
			if (key === "taxonomy") {
				const isDefaultTaxonomy =
					...
					(Array.isArray(value) &&
						value.length === 1 &&
						value[0] === "tZsUsd2BqGViQrJ9Vs3Wah") ||
