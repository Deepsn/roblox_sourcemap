...
const gearSubcategoryCompare = (
	leftSubcategory: Subcategory,
	rightSubcategory: Subcategory,
) => {
...
const getSearchOptions = (
	navMenuItemsResponse: NavigationMenuItemsResponse,
): SearchOptionsData => {
	...
	updatedSearchOptions.categoriesWithCreatorFilters =
		newCategoriesWithCreatorFilters;

	const navigationMenuData = { ...navMenuItemsResponse };

	const categoryEnums: CategoryEnumLibrary = {};
	navigationMenuData.categories = navMenuItemsResponse.categories.map(
		(category) => {
			...
			if (updatedCategory.subcategories) {
				...
				updatedCategory.subcategories = updatedCategory.subcategories.map(
					(subCategory) => {
						...
						subcategoryEnums[updatedSubcategory.subcategoryId] =
				...
				Object.assign(
					categoryEnums[updatedCategory.categoryId].subcategoryEnums,
					subcategoryEnums,
				);
			}
	...
	updatedSearchOptions.defaultCategory = navigationMenuData.categories.find(
		(category) =>
			category.categoryId === updatedSearchOptions.defaultCategoryId,
	);
	...
	if (navMenuItemsResponse.priceFilters !== null) {
		navigationMenuData.priceFilters = navMenuItemsResponse.priceFilters.map(
			(price) => {
	...
	navigationMenuData.creatorFilters = navMenuItemsResponse.creatorFilters.map(
		(creator) => {
			return {
				...creator,
	...
	updatedSearchOptions.defaultCreatorFilter =
		navigationMenuData.creatorFilters.find(
			(creatorFilter) =>
				creatorFilter.userId === updatedSearchOptions.defaultCreatorId,
		);

	navigationMenuData.sortMenu.sortOptions =
		navMenuItemsResponse.sortMenu.sortOptions.map((sortOption) => {
			return {
				...sortOption,
	...
	navigationMenuData.sortMenu.sortAggregations =
		navMenuItemsResponse.sortMenu.sortAggregations.map((sortAggregation) => {
			return {
				...sortAggregation,
