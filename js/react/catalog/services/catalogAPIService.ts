...
class CatalogAPIService {
	static getMetadataFromApi(): AxiosPromise<MetadataResponse> {
		return httpService.get<MetadataResponse>(
			catalogConstants.endpoints.getMetadata,
		);
	}
	...
	static getMarketplaceWidgetItems(): Promise<TItem[]> {
		...
		return httpService
			.get<MarketplaceWidgetsResponse>(
				catalogConstants.endpoints.getMarketplaceWidgets,
				{
					requestId,
				},
			)
	...
	static getCatalogItemDetails(
		...
		return httpService
			.post<CatalogItemDetailsResponse>(
				endpoints.getCatalogItemDetails,
				requestData,
			)
			.then((response) => {
				...
				const returnResult: ItemWithDetails[] =
					result && result.data
						? result.data.reduce<ItemWithDetails[]>((hydratedItems, item) => {
								...
								const itemWithRestrictions: TGenericItemDetails &
									TItemCardRestrictions = {
									...newItem,
								...
								const itemWithProperties: ItemWithDetails = {
									...
									creatorLink:
										UtilityService.buildUserLink(itemWithRestrictions),
								};
	...
	static postMarketplaceItemDetails(collectibleItemId: number) {
		...
		return httpService.post(
			catalogConstants.endpoints.postMarketplaceItemDetails,
			urlParams,
		);
	}
