...
class UtilityService {
	...
	static formatQueries(
		...
		if (
			formatQueries.taxonomy !== undefined &&
			formatQueries.taxonomy.length !== 0
		) {
	...
	static checkIfQueryParamShouldBeKeptForUrl(
		key: string,
		queryParams?: QueryParams,
	): boolean {
		...
		Object.entries(filteredParams).forEach(([key, value]) => {
			if (value !== undefined && value !== null) {
				if (value === true) {
					...
					UtilityService.checkIfQueryParamShouldBeKeptForUrl(
						key,
						filteredParams,
					)
				) {
	...
	static mapItemRestrictionIcons(
		item: TGenericItemDetails,
	): TItemCardRestrictions {
		return ItemCardUtils.mapItemRestrictionIcons(
	...
	static mapItemStatusIconsAndLabels(
		...
		return ItemCardUtils.mapItemStatusIconsAndLabels(itemStatus).map(
			(status) =>
				status.isIcon
					? {
	...
	static buildUserLink(
		item: Pick<ItemWithDetails, "creatorType" | "creatorTargetId">,
	): string {
