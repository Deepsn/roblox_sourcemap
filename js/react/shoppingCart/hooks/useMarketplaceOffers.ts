...
export type MarketplaceOfferPricing = Record<
	string,
	CartPricedItem & { offerIds: string[] }
>;
...
export function mapDiscountLines(
	...
	return (discountInformation?.discounts ?? []).map((discount) => ({
		...
		localizedAttribution:
			discount.localizedDiscountAttribution?.trim() || undefined,
	}));
}

export function isMarketplaceOfferDiscountLine(
	line: CartPricedDiscountLine,
): boolean {
	...
	(items ?? []).forEach((item) => {
		if (item.collectibleItemId && item.priceInRobux != null) {
			...
			pricedItems[item.collectibleItemId] = {
				...
				originalPrice:
					item.discountInformation?.originalPrice ?? item.regularPriceInRobux,
...
export default function useMarketplaceOffers(
	...
	const fetchCartPricing = useCallback(async () => {
		...
		try {
			...
			const selections = mapAvailableOffersToSelections(
				response.availableOffers,
			);
