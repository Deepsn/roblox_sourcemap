...
export function processTimedOptionsForCart(
	...
	const hasPermanentOption = processedOptions.some(
		(option) => option.days === 0,
	);

	if (!hasPermanentOption) {
		const timedOptionSelected = processedOptions.find(
...
export function getItemPrice(
	...
	if (
		timedOptions &&
		timedOptions.length > 0 &&
		itemDetail.timedOptions?.length
	) {
		const selectedOption =
			timedOptions.find((option) => option.selected) ?? timedOptions[0];
...
export function reconcileTimedOptions(
	...
	const previouslySelectedDays =
		cachedTimedOptions?.find((option) => option.selected)?.days ?? null;
...
export function recalculateCartTotalFromSelectedItems(
	...
	const totalPrice = items.reduce((acc, item) => {
		...
		let expectedPrice = getItemPrice(
...
export const isBundle = (item: TCartItem): boolean =>
...
export function standardizeItemDetails(
...
export function standardizeItemDetailsFromHydratedItemDetails(
	item: TDetailEntry,
): TItemDetails {
	return {
		...item,
...
export function splitItemsByCollectible(
	items: TCartItem[] = [],
): TCartItem[][] {
	const itemTypeCategorizer = (
		[legacyItems, collectibleItems]: TCartItem[][],
		item: TCartItem,
	) =>
...
export function extractLimiteds(cartState: TCartState): TExtractedLimiteds {
	const { items, itemDetails } = cartState;
	const itemTypeCategorizer = (
		{ limited1, limited2 }: TExtractedLimiteds,
		item: TCartItem,
	) => {
		const details = itemDetails[item.itemId];
		const collectibleItemId =
			item.collectibleItemId || details.collectibleItemId;
...
function doesDomElementContainQuerySelector(
	target: Element,
	selector: string,
): boolean {
...
export function isDomElementInShoppingCart(target: Element): boolean {
	const checks = {
		...
		modalBackdrop: doesDomElementContainQuerySelector(
			target,
			".modal-backdrop.in",
		),
		modal: doesDomElementContainQuerySelector(target, ".in.modal"),
		addRemoveBtn: doesDomElementContainQuerySelector(
			target,
			".add-to-cart-btn-container",
		),
		modalContainer: doesDomElementContainQuerySelector(
			target,
		...
		closestTimedOptions:
			target.closest(".shopping-cart-timed-options-dropdown") !== null,
	};
