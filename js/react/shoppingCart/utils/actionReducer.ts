...
export const setSystemFeedbackService = (
	systemFeedback: TSystemFeedbackService,
) => {
...
async function addCartItem(action: TCartAddItemAction): Promise<TCartState> {
	...
	const existingItem = cartState.items?.find(
		(i) => i.itemId && i.itemId === action.item.itemId,
	);
	...
	selectedItems[`${action.item.itemType.toLowerCase()}${action.item.itemId}`] =
		true;
...
async function addCartItems(action: TCartAddItemsAction): Promise<TCartState> {
	...
	const newItems = action.items
		.map((item) => {
			const existingItem = cartState.items?.find(
				(i) =>
					i.itemId && i.itemId === item.itemId && i.itemType === item.itemType,
			);
	...
	itemDetailsArray.forEach((itemDetails, index) => {
		newItemDetails[newItems[index].itemId] =
			itemDetails[newItems[index].itemId];
	});
