...
function SubscribeUpsellContainer({
	...
	const hasEligibleItem = useMemo(() => {
		return selectedItems.some((item) => {
			...
			return (
				price >= MIN_ELIGIBLE_PRICE &&
				price <= MAX_ELIGIBLE_PRICE &&
				!isResalePurchase(details)
			);
		});
	}, [selectedItems, itemDetails]);
	...
	const getDiscountLabel = (discountPercentage: number): string => {
		const translated =
			robloxSubscriptionTranslations.labelBlackbirdUpsellBanner({
	...
	const getSubscribeButtonText = (): string => {
		if (subscriptionStatus.hasFreeTrial) {
			const translated =
				robloxSubscriptionTranslations.actionTrialSubscription();
		...
		return (
			<div className="subscribe-upsell-container subscribe-upsell-banner">
				<div className="subscribe-upsell-header-content">
					...
					/>
					<span className="subscribe-upsell-title">
						{getDiscountLabel(upsellDiscountPercent)}
					</span>
				</div>
		...
		return (
			<div className="subscribe-upsell-container subscribe-upsell-compact">
				<div className="subscribe-upsell-header-content">
					...
					/>
					<span className="subscribe-upsell-title">
						{firstDiscount.localizedAttribution}
					</span>
				</div>
	...
	return (
		<div className="subscribe-upsell-container">
			...
			>
				...
				<span
					className={
						isExpanded ? "icon-up chevron-icon" : "icon-down chevron-icon"
					}
				/>
			</button>
