...
const { Thumbnail2d, DefaultThumbnailSize, ThumbnailTypes, ThumbnailFormat } =
	Thumbnails;
...
function checkIfItemShouldPurchaseFromReseller(itemDetail: TDetailEntry) {
	...
	if (
		...
		if (
			...
			if (
				resaleIsNotBeatenByCreator ||
				collectible.saleLocationType === "ExperiencesDevApiOnly"
			) {
				return true;
			}
		} else {
...
function buildOfferEligibleItems(
	...
	selectedItemsList.forEach((item) => {
		...
		if (
			...
			checkIfItemShouldPurchaseFromReseller(detail)
		) {
			return;
		}

		const selectedTimedOption =
			item.timedOptions?.find((option) => option.selected) ??
			item.timedOptions?.[0];
		if (
			selectedTimedOption?.days &&
			detail.timedOptions?.some(
...
function ItemPrice({
	...
	if (
		item.timedOptions &&
		item.timedOptions.length > 0 &&
		itemDetails.timedOptions?.length
	) {
		const selectedTimedOption = item.timedOptions.find((opt) => opt.selected);
		const selectedDays =
			selectedTimedOption?.days ?? item.timedOptions[0]?.days;

		const matchingTimedOption = itemDetails.timedOptions.find(
	...
	if (offerPricing) {
		...
		const preDiscountPrice =
			offerPricing.originalPrice ?? originalPrice ?? catalogPrice;
		...
		if (!isMarketPlaceEnabled && !hasResellers) {
			return (
				<div className="item-price">
					<span className="price-text text-alert">
						{itemTranslations.labelItemNotForSale()}
					</span>
				</div>
			);
		}
	...
	if (typeof expectedPrice === "number" && expectedPrice > 0) {
		return (
			<div className="item-price">
				...
				<span className="price-text text">
					{numberFormat.getNumberFormat(expectedPrice)}
				</span>
				{originalPrice !== null && (
	...
	if (isLimited) {
		const limitedUnavailable =
			...
			typeof itemDetails?.lowestPrice !== "number";
		if (limitedUnavailable) {
			return (
				<div className="item-price">
					<span className="price-text text-alert">
						{catalogTranslations.labelNoResellers()}
					</span>
				</div>
			);
		}
...
function ShoppingCartModalItem({
	...
	const showTimedOptions =
		!!item.timedOptions?.length && !!itemDetails.timedOptions?.length;
	...
	const isSelectedDurationStillOffered =
		...
		(persistedSelectedOption.days === 0 ||
			!!itemDetails.timedOptions?.some(
				(option) => option.days === persistedSelectedOption.days,
			));
	const effectiveSelectedDays = isSelectedDurationStillOffered
		? persistedSelectedOption.days
		: 0;
	...
	const offeredTimedOptions =
		itemDetails.timedOptions ?? item.timedOptions ?? [];
	const permanentTimedOption = offeredTimedOptions.find(
		(option) => option.days === 0,
	);
	const rentalTimedOptions = offeredTimedOptions.filter(
		(option) => option.days > 0,
	);
	const singleRentalTimedOption =
		rentalTimedOptions.length === 1 ? rentalTimedOptions[0] : null;
	const showTwoButtonTimedOptions =
		showTimedOptions && !!singleRentalTimedOption;
			...
			<div className="cart-item">
				...
				<div className="checkbox purchase-checkbox-container">
					<input
						...
						type="checkbox"
						checked={
							!isUnpurchasable && !!selectedItemsRecord[getCartItemKey(item)]
						}
						onChange={() => {
							onCheckboxClicked(item);
						}}
					...
					{showTwoButtonTimedOptions && singleRentalTimedOption && (
						<div className="shopping-cart-timed-options-toggle">
							...
							<button
								type="button"
								className={`timed-option-toggle-btn ${
									effectiveSelectedDays === singleRentalTimedOption.days
										? "selected"
										: ""
								}`}
								aria-pressed={
								...
								onClick={() =>
									dispatchTimedOptionSelection(singleRentalTimedOption.days)
								}
							>
								<span className="timed-option-toggle-label">
							...
							<Dropdown
								...
								placeholder={
									effectiveSelectedDays === 0
										...
										: catalogTranslations.labelTimedOptionDays(
												effectiveSelectedDays,
											)
								}
								onValueChange={(value) => {
							...
							>
								<Menu className="timed-options-dropdown-menu">
									<MenuSection className="timed-options-dropdown-menu-section">
										{(itemDetails.timedOptions || item.timedOptions)?.map(
											(timedOption) => (
												<MenuItem
              ...
              price is replaced by that state rather than shown alongside it. */}
					{isOwned && (
						<div className="item-owned">
							...
							<span className="item-owned-text">
								{catalogTranslations.labelItemOwned()}
							</span>
						</div>
					)}
					{!isOwned && isOffSale && (
						<div className="item-status-label">
							{catalogTranslations.labelOffSale()}
						</div>
					)}
					{!isOwned && !isOffSale && (
						<ItemPrice
...
function ShoppingCartModalFooter({
	...
	const offerAdjustedRemainingBalance =
		remainingBalance + totalPrice - offerAdjustedTotalPrice;
	...
	const getTransactionItemPrice = useCallback(
		(itemDetail: TDetailEntry): number => {
			if (itemDetail.collectibleItemId) {
				const offerPrice =
					marketplaceOfferPricing[itemDetail.collectibleItemId];
				if (offerPrice) {
	...
	const wasTimedOptionPurchased = useCallback(
		(itemDetail: TDetailEntry) => {
			...
			const selectedIndex = cartItem.timedOptions.findIndex(
				(option) => option.selected,
			);
			const selectedOption =
				cartItem.timedOptions[selectedIndex === -1 ? 0 : selectedIndex];
	...
	const onTransactionComplete = useCallback(
				...
				if (result.data.reason === "Success") {
					...
					if (result.data.itemData.assetId !== undefined) {
						...
						purchasedItems.push({
							...
							subType:
								itemDetail.itemType === "Bundle"
									? itemDetail.bundleType
						...
						dispatch(
							removeItemAction({
								itemId: result.data.itemData.assetId,
								itemType: "Asset",
							}),
						).catch(() => {
							console.error("Failed to remove item");
						...
						dispatch(
							removeItemAction({
								itemId: result.data.itemData.bundleId,
								itemType: "Bundle",
							}),
						).catch(() => {
							console.error("Failed to remove item");
		...
		[
			dispatch,
			getTransactionItemPrice,
			itemDetails,
			items,
			totalCartValue,
			wasTimedOptionPurchased,
		],
	);
	const parseItems = () => {
		...
		selectedItemsList.forEach((item) => {
			...
			if (item.timedOptions && item.timedOptions.length > 0) {
				timedOptionIndex = item.timedOptions.findIndex(
					(option) => option.selected,
				);
			}
			...
			const timedOptionToPass = itemStillOffersTimedOption
				? selectedTimedOption
				: undefined;
			selectedItems.push({
				id: item.itemId,
		...
		<div className="shopping-cart-footer">
			{offerSelections.length > 0 && (
				<div className="marketplace-offers-container">
					{offerSelections.map((offer) => (
						<div
							className="checkbox marketplace-offer-selection"
							key={offer.offerId}
						>
							<input
								className="input-checkbox"
								...
								onChange={(event) =>
									handleOfferCheckedChange(offer.offerId, event.target.checked)
								}
							/>
							<label
								className="checkbox-label"
								htmlFor={`shopping-cart-offer-${offer.offerId}`}
							>
			...
			<div className="cart-total-section-container">
				<div className="total-label">
					{catalogTranslations.labelTotalItems({
						itemCount: `${selectedItemsList.length}`,
					})}
				</div>
				<div className="total-price-container">
					<span className="icon-robux-16x16" />
					<span className="price-text">
						{offerAdjustedTotalPrice.toLocaleString()}
					</span>
				</div>
			</div>
				...
				<BatchBuyPriceContainer
					...
					onBuyButtonClick={() => {
						const transactionItems = selectedItemsList
							...
							.filter(
								(itemDetail): itemDetail is TDetailEntry =>
							...
							.map((itemDetail) => ({
								...
								resalePurchase:
									checkIfItemShouldPurchaseFromReseller(itemDetail),
								isLimited: isItemLimited(itemDetail),
								isTimedOptionPurchase: wasTimedOptionPurchased(itemDetail),
							}));
...
function ShoppingCartModal(props: {
	...
	const currentBalance = Math.max(
		currentUserBalance ?? currentUserBalance ?? 0,
		0,
	);
	...
	const updateTotalPrice = useCallback(
		(itemList: TCartItem[]) => {
			const totalSelectedPrice = recalculateCartTotalFromSelectedItems(
				cart,
				itemList,
			);
			...
			>
				<div className="shopping-cart-title-container">
					<h2 className="shopping-cart-title">
						...
						<span className="shopping-cart-title-item-count">
							{itemCountFormatted}
						</span>
					</h2>
				</div>
						...
						<div className="shopping-cart-items-list">
							{items.map((item) => (
								<ShoppingCartModalItem
									...
									marketplaceOfferPricing={
										marketplaceOffers.marketplaceOfferPricing
									}
				...
				<ShoppingCartModalFooter
					...
					totalCartValue={recalculateCartTotalFromSelectedItems(
						cart,
						cart.items,
					)}
