...
> = {
	featureCatalog: () =>
		resourceProvider.getTranslationResource(translationConfig.feature),
	featureItem: () =>
		resourceProvider.getTranslationResource(
			featureItemTranslationConfig.feature,
		),
	featureItemModel: () =>
		resourceProvider.getTranslationResource(itemModelTranslationConfig.feature),
	featureRobloxSubscription: () =>
		resourceProvider.getTranslationResource(
			featureRobloxSubscriptionTranslationConfig.feature,
		),
};
...
export const catalogTranslations = {
	actionSeeMoreReseller: (): string => {
		return translationResources.featureCatalog().get("Action.SeeMoreReseller", {
			...
			resellerLink: translationResources
				.featureCatalog()
				.get("Heading.Resellers", {}),
			linkEnd: "</a>",
		});
	},
	actionBuy: (): string => {
	...
	messageCollectiblesInfo: (): string => {
		return translationResources
			.featureCatalog()
			.get("Message.CollectiblesInfo", {});
	},
	labelNewCollectibles: (): string => {
		return translationResources
			.featureCatalog()
			.get("Label.NewCollectibles", {});
	},
	labelByCreatorLink: (params: Record<string, string>): string => {
		return translationResources
			.featureCatalog()
			.get("Label.ByCreatorLink", params);
	},
...
export const itemTranslations = {
	...
	labelPremiumDiscountSavings: (
		...
		return translationResources
			.featureItem()
			.get("Label.PremiumDiscountSavings", {
	...
	labelPremiumDiscountOpportunityPrompt: (premiumPrice: number): string => {
		...
		return translationResources
			.featureItem()
			.get("Label.PremiumDiscountOpportunityPrompt", {
...
export const itemModelTranslations = {
	labelLimitedQuantity: (
		...
		const unitsForConsumption = numberFormat.getNumberFormat(
			unitsAvailableForConsumption,
		);
		const total =
			typeof totalQuantity === "number"
				? numberFormat.getNumberFormat(totalQuantity)
				: null;
		return translationResources
			.featureItemModel()
			.get("Label.LimitedQuantity", {
