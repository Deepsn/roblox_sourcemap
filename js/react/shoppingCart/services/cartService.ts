...
async function fetchItemDetails(
	items: TCartItem[] = [],
): Promise<TItemDetails[]> {
	const itemsBody = items.map((i) => ({
		id: i.itemId,
		itemType: i.itemType,
	}));

	const res = await ItemDetailsHydrationService.getItemDetails(
		itemsBody,
		false,
		true,
	);
...
export async function getCollectibleResellers(
	...
	if (!itemId || !collectibleItemId)
		return { itemId, collectibleItemId, resellers: [] };
...
export async function fetchCartItemsResellers(
	...
	const resByItemId: Record<
		string,
		Array<TResellerItem | TCollectibleResellerItem>
	> = {};
	...
	const limited1Resellers = limited1.length
		? await Promise.all(
				limited1.map((item) => fetchLimitedItemResellers(item.itemId)),
			)
		: [];
	const limited2Resellers = limited2.length
		? await Promise.all(
				limited2.map((item) =>
					getCollectibleResellers(item.itemId, item.collectibleItemId),
				),
			)
		: [];
