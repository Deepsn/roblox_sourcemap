...
export const SponsoredCatalogItemsRow = ({
	placementLocation,
	translate,
}: TSponsoredCatalogItemsRowProps &
	...
	const getAdCountForCatalogPage = () => {
		...
		if (screenWidth < sizing.screenSize1Item) {
			...
			const rowWidth =
				screenWidth -
				sizing.catalogLeftNav -
				sizing.websiteLeftNav -
				sizing.extraSpace;
	...
	const getCatalogItemsDetails = useCallback(
		async (items: TSponsoredCatalogItem[]) => {
		...
		getSponsoredCatalogItems()
			.then((response: AxiosResponse<TSponsoredCatalogItemsResult>) => {
				...
				getCatalogItemsDetails(response.data.data)
					.then(
						(
							detailsResponse: AxiosResponse<TSponsoredCatalogItemsDetailsResult>,
						) => {
					...
					>
						...
						<div className="recommended-items-slider">
							<ul className="hlist item-cards recommended-items">
								{hydratedItems.map(
									(item: TCatalogSearchDetailedResponseItem, index: number) => {
										return (
											<SponsoredCatalogItemCard
												...
												premiumPricing={
													item.premiumPricing?.premiumPriceInRobux || -1
												}
												unitsAvailableForConsumption={
													item.unitsAvailableForConsumption || 0
												}
												...
												encryptedAdTrackingData={
													sponsoredCatalogItems[index].encryptedAdTrackingData
												}
			...
			return (
				<React.Fragment>
					<div
						...
						id="populated-sponsored-catalog-items"
					>
						<h2 className="sponsored-catalog-items-row-title">
							{translate("Label.Sponsored")}
						</h2>
						<div className="sponsored-catalog-items-row">
