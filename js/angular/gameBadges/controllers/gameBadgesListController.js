...
function gameBadgesListController(
	gameBadgesConstants,
	badgesLanguageService,
	...
	const shouldContinueLoadingBadges = function () {
		if (
			ctrl.badges.displayedBadgeCount >= ctrl.badges.data.length &&
			ctrl.badges.pager.canLoadNextPage()
		) {
	...
	ctrl.badges = {
		...
		pager: cursorPaginationService.createPager({
			...
			getRequestUrl: function () {
				return $filter("formatString")(gameBadgesConstants.urls.getGameBadges, {
					universeId: ctrl.universeId,
				});
			},
	...
	ctrl.seeMore = function () {
		var addCount =
			ctrl.layout.badgeCountPerPage -
			(ctrl.badges.displayedBadgeCount % ctrl.layout.badgeCountPerPage);
...
gameBadgesModule.controller(
	"gameBadgesListController",
	gameBadgesListController,
);
