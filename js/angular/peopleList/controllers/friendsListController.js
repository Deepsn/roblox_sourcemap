...
function friendsListController(
	...
	$scope.clickPlaceLink = function (friend, index) {
		const eventStreamParams =
			resources.eventStreamParams.goToGameDetailFromAvatar;
		const properties = {
			friendId: friend.id,
			position: index,
	...
	$scope.updatePresenceStatus = function (responses) {
		...
		for (let i = 0; i < responses.length; i++) {
			...
			if (
				presenceData.rootPlaceId &&
				!$scope.library.placesDict[presenceData.rootPlaceId]
			) {
		...
		if (
			placeIdsNotInPlacesDict.length > 0 &&
			angular.isFunction($scope.setPlaceDetails)
		) {
