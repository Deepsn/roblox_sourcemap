...
function socialLinksService($q, httpService, socialLinksConstants, $window) {
	...
	const mapSocialLinksErrorCode = function (responseData, errorCodeMap) {
		...
		const errorCode =
			errorCodes[0] || socialLinksConstants.errorCodes.internal.unknownError;
			...
			httpService.httpGet(urlConfig).then(
				function (responseData) {
					...
					if (targetType === socialLinksConstants.targetTypes.user) {
						result.data = [];
						Object.keys(socialLinksConstants.socialMediaTypes).forEach(
							(socialMediaType) => {
								...
								if (socialMediaUrl) {
									result.data.push({
										id: 0,
										title:
											socialLinksConstants.socialMediaTypes[socialMediaType],
										type: socialLinksConstants.socialMediaTypes[
											socialMediaType
										],
										url: socialMediaUrl,
									});
					...
					result.socialLinksVerificationStatus =
						responseData.socialLinksVerificationStatus;
	...
	const saveSocialLink = function (socialLink) {
		return $q(function (resolve, reject) {
			...
			if (socialLink.id) {
				...
				httpService
					.httpPatch(urlConfig, requestBody)
					.then(resolve, function (responseData) {
						reject(mapSocialLinksErrorCode(responseData, errorCodeMap));
					});
			} else {
