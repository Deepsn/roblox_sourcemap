...
const socialLinksConstants = {
	...
	urlRegexes: {
		...
		youtube:
			/^((http|https):\/\/)?(www\.)?youtube\.com\/(?!opensearch(\/|$))(?!.*\blogout\b)[@a-zA-Z0-9\-\/_]+$/,
		twitch: /^((http|https):\/\/)?(www\.)?twitch\.tv\/[a-zA-Z0-9\-/_]+$/,
		discord:
			/^((http|https):\/\/)?(www\.)?discord\.(gg|io|me|li)\/[a-zA-Z0-9\-_/]+$/,
		robloxGroup:
			/^(?:https?):\/\/(?:[a-z0-9-]{2,}\.)*(?:roblox(labs)?\.com\/)(?:groups\/|communities\/)(?<groupId>[\d]+)\/?(?:[A-Za-z0-9-]+)?(?:#!\/)?(?:[A-Za-z0-9]+)?$/,
		amazon:
			/^((https?):\/\/)?(www\.)?amazon\.com\/stores\/page\/[\w\-/]+(\?[\w\-/]+(=[\w\-/]*)?(&[\w\-/]+(=[\w\-/]*)?)*)?$/,
		guilded:
			/^((http|https):\/\/)?(www\.)?guilded\.gg\/((profile|teams|i|r|u)\/)?([a-zA-Z0-9-]{3,30})(\/(overview))?(\?[i|r]=?([a-zA-Z0-9-]{3,16}))?$/,
	},
