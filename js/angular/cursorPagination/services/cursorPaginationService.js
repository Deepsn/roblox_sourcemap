	...
	return {
        ...
        */
		createPager: function (options) {
			...
			var firstPageCursor =
				typeof options.firstPageCursor === "string"
					? options.firstPageCursor
					: "";
			...
			options.getDataListFromResponse =
				options.getDataListFromResponse ||
				this.getDataListFromResponseMethods.defaultV1;
			options.getNextPageCursorFromResponse =
				options.getNextPageCursorFromResponse ||
				this.getNextPageCursorFromResponseMethods.defaultV1;
			options.getErrorsFromResponse =
				options.getErrorsFromResponse ||
				this.getErrorsFromResponseMethods.defaultV1;
			options.getQueryParameters =
				options.getQueryParameters || this.getQueryParametersMethods.defaultV1;
			...
			function getCacheKey() {
				return cacheService.buildKey(
					options.getCacheKeyParameters(pageParameters),
				);
			}
			...
			function loadPage(pageNumber) {
				...
				if (
					...
					typeof nextPageCursors[cacheKey] !== "string"
				) {
				...
				httpService.httpGet(urlConfig, params).then(
					function (data) {
						currentPageNumber = pageNumber;
						nextPageCursors[cacheKey] =
							options.getNextPageCursorFromResponse(data);
						options.getDataListFromResponse(data).then(
							function (list) {
			...
			function hasCurrentPage() {
				...
				return (
					cache.getLength(cacheKey) > (currentPageNumber - 1) * options.pageSize
				);
			}
			...
			return {
				...
				hasNextPage: function () {
					var cacheKey = getCacheKey();
					if (
						cache.getLength(cacheKey) >
						currentPageNumber * options.pageSize
					) {
...
cursorPaginationModule.service(
