...
const buildWrapperFn = (
	...
	switch (spec.type) {
		case "link": {
			...
			return (content: React.ReactNode) => (
				<Link
					...
					rel={openInNewWindow ? (rel ?? "noopener noreferrer") : rel}
					className={className}
				>
					{content}
				</Link>
		...
		default: {
			...
			throw new Error(
				`Unknown wrapper type: ${(_exhaustive as WrapperConfig).type}`,
			);
		}
	}
};
...
const resolveParams = (
	...
	return Object.fromEntries(
		Object.entries(params).map(([key, value]) => [
			...
			typeof value === "string"
				? value
				: translateToStringOnly(resourceMap, intl, value),
		]),
	);
};
