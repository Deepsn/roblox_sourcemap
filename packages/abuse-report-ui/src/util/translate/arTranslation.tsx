...
export const ArTranslationContext = createContext<
	ArTranslationContextValue | undefined
>(undefined);
...
export const ArTranslationProvider = ({
	...
	const methods = useMemo(() => {
		const boundTranslate = (input: TranslateInputOrString) =>
			translate(translations, intl, input);
		...
		return {
			...
			translateToStringOnly: boundTranslateToStringOnly,
		};
	}, [translations, intl]);

	return (
		<ArTranslationContext.Provider value={methods}>
			{children}
		</ArTranslationContext.Provider>
	);
};
