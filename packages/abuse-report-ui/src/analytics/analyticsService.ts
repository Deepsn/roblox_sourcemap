...
import type { ClientActionName } from "@rbx/abuse-report-config-types";
import { EventStream } from "@rbx/core-scripts/legacy/Roblox";
import { httpService } from "@rbx/core-scripts/legacy/core-utilities";
import type { SubmissionTargetFailureReason } from "../submission/dispatchSubmission";
...
export type SubmissionTargetTelemetry =
	| {
			target_type: "default";
			client_action_name?: never;
	  }
	| {
			target_type: "clientAction";
			client_action_name: ClientActionName;
	  }
	| {
			target_type?: never;
			client_action_name?: never;
...
export type EventNameToProps = {
	...
	[EventName.Submit]: SubmissionTargetTelemetry;
	[EventName.SubmitSuccess]: SubmissionTargetTelemetry & {
		/** milliseconds spent submitting e.g. submit API latency */
		submit_api_duration: number;
		report_id?: string;
	};
	[EventName.SubmitFail]: SubmissionTargetTelemetry & {
		/** milliseconds spent submitting e.g. submit API latency */
		submit_api_duration: number;
		target_failure_reason?: SubmissionTargetFailureReason;
	};
...
export const getApiErrorMessage = (
	...
	const parts = [
		`API Error:${apiId}: ${getErrorMessage(error, defaultMessage)}`,
	];
...
export const sendRuntimeErrorEvent = (
	...
	const errorMessage =
		fullErrorMessage.length > 500
			? `${fullErrorMessage.slice(0, 500)}...`
			: fullErrorMessage;
	const trimmedStack =
		componentStack.length > 500
