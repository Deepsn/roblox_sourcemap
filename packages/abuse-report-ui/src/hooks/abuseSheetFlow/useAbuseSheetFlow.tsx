...
const getSubmissionTargetTelemetry = (
	target: SubmissionTarget,
): SubmissionTargetTelemetry => {
	switch (target.type) {
		case "default":
...
const useAbuseSheetFlow = (
	...
	const { sendEvent } = useAbuseSheetAnalytics({
		state,
		abuseVector,
		analyticsTargetId,
	});
	...
	useEffect(() => {
		...
		if (open && !firstRenderRef.current) {
			...
			dispatch({
				type: "reset",
				payload: { initialEnterTime: now, runId, startNodeId },
			});
		}
	...
	useEffect(() => {
		...
		const handleNextSimple = (next: NextSimpleValue) => {
			...
			if (typeof next === "string") {
				dispatch({
					type: "gotoNode",
					payload: { nodeId: next, enterTime: now },
				});
		...
		if (typeof next !== "string" && next.type === "submitForm") {
			...
			const targetTelemetry = getSubmissionTargetTelemetry(
				config.submission.target,
			);
	...
	const canGoBack =
		state.stack.length > 1 && state.submissionState !== "completed";
	...
	const analytics: AbuseReportAnalytics = useMemo(
		() => ({ sendEvent, EventName }),
		[sendEvent],
	);
