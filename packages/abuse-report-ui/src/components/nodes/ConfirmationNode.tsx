...
const useBlockToggle = (
	...
	const [state, setState] = useState<
		"loading" | "visible" | "hidden" | "error"
	>(blockToggle ? "loading" : "hidden");
	const [isBlocked, setIsBlocked] = useState<boolean | undefined>(undefined);
	const [isBlockRequestPending, setIsBlockRequestPending] = useState(false);
	...
	const handleBlockToggle = (checked: boolean) => {
		if (!blockToggle?.userId || isBlockRequestPending) {
			return;
		}
		setIsBlockRequestPending(true);
		setIsBlocked(checked);

		let request: Promise<void>;
		if (checked) {
			request = blockUser(blockToggle.userId)
				.then(() => {
					if (blockToggle.blockedSnackbarMessage) {
						setSnackbar({
							isVisible: true,
							message: translateToStringOnly(
								blockToggle.blockedSnackbarMessage,
							),
						});
					}
				})
			...
			request = unblockUser(blockToggle.userId)
				.then(() => {
					if (blockToggle.unblockedSnackbarMessage) {
						setSnackbar({
							isVisible: true,
							message: translateToStringOnly(
								blockToggle.unblockedSnackbarMessage,
							),
						});
					}
				})
		...
		request
			.finally(() => {
				setIsBlockRequestPending(false);
			})
			.catch(() => undefined);
	};
	...
	return (
		<React.Fragment>
			<Body>
				...
				<div className="text-body-small padding-bottom-medium">
					{translate(message)}
				</div>
				{blockToggle && blockToggleState === "visible" && (
					<div className="flex flex-col gap-small padding-xlarge bg-action-standard radius-medium">
						<Toggle
							...
							aria-busy={isBlockRequestPending}
							isChecked={Boolean(isBlocked)}
							onCheckedChange={handleBlockToggle}
							size="Medium"
							placement="End"
							hint={
								blockToggle.description
									? translateToStringOnly(blockToggle.description)
									: undefined
							}
						/>
					</div>
