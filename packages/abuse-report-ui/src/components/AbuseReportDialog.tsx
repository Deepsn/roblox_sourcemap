...
import {
	...
	useArTranslation,
} from "../util/translate/arTranslation";
...
import type { ClientActionRegistry } from "../submission/dispatchSubmission";
import {
	type AbuseReportAttributes,
	type AbuseReportRequestInput,
	getRequestAttributes,
	serializeAbuseReportAttributes,
} from "../api/abuseReportRequest";

const EMPTY_LOADERS: LoaderRegistry = {};

type AbuseReportDialogRequestInput =
	| {
	...
	| {
			attributes?: never;
			analyticsTargetId?: never;
			/** @deprecated Use attributes and analyticsTargetId instead. */
			targetIdStr: string;
	  };

type AbuseReportDialogBaseProps = {
	abuseVector: string;
	open: boolean;
	onClose: () => void;
...
export type AbuseReportDialogProps = AbuseReportDialogBaseProps &
	AbuseReportDialogRequestInput;
...
type AbuseReportDialogWithConfigBaseProps = {
	config: RawConfig;
	abuseVector: string;
	open: boolean;
	onClose: () => void;
...
export type AbuseReportDialogWithConfigProps =
	AbuseReportDialogWithConfigBaseProps & AnalyticsTargetInput;

export type PrefetchAbuseUIOptions = {
	abuseVector: string;
} & AbuseReportRequestInput;

const getAnalyticsTargetId = ({
	analyticsTargetId,
	targetIdStr,
}: {
	analyticsTargetId?: string;
	...
	if (analyticsTargetId !== undefined && targetIdStr !== undefined) {
		throw new Error("analyticsTargetId and targetIdStr are mutually exclusive");
	}

	return targetIdStr ?? analyticsTargetId;
};
 ...
 */
export const prefetchAbuseUI = (options: PrefetchAbuseUIOptions): void => {
	const { abuseVector } = options;
	const attributes = getRequestAttributes(options);
	const serializedAttributes = serializeAbuseReportAttributes(attributes);
	const locale = new Intl().getRobloxLocale();
	fetchCachedConfig(abuseVector, locale, serializedAttributes)
		.then(() => {
				...
				<LayoutSlotsProvider slots={sheetSlots}>
					...
					>
						<SheetContent
							...
							closeLabel={translateToStringOnly(
								normalizedConfig.display.closeButtonLabel,
							)}
						...
						>
								...
								utilities={
									disclosure ? (
										<DisclosureInfoButton
											ariaLabel={translateToStringOnly(
												disclosure.ariaLabel ?? "",
											)}
											closeLabel={translateToStringOnly(
												disclosure.closeButtonLabel,
											)}
											...
											buttonLabel={
												...
												translateToStringOnly(disclosure.buttonLabel)
											}
							...
							>
								{currentNode.eyebrow
									? translateToStringOnly(currentNode.eyebrow)
									: null}
							</SheetTitle>
							{currentPageElement}
						</SheetContent>
 ...
 *       attributes={{ targetId: "123" }}
 *       analyticsTargetId="123"
 *       open={open}
...
const AbuseReportDialog = (
	...
	const requestAttributes = getRequestAttributes(props);
	const serializedAttributes =
		serializeAbuseReportAttributes(requestAttributes);
	const analyticsTargetId = getAnalyticsTargetId(props);
	const analyticsTargetIdRef = useRef(analyticsTargetId);
	analyticsTargetIdRef.current = analyticsTargetId;
			...
			retryNTimes(3, () =>
				...
				.catch((err: unknown) => {
					...
					sendConfigApiErrorEvent(err, {
						...
						target_id: analyticsTargetIdRef.current,
					});
				});
		}
	...
	}, [open, abuseVector, serializedAttributes]);
		...
		>
			<ArTranslationProvider translations={config.translations}>
				<AbuseReportDialogInner
					...
					analyticsTargetId={analyticsTargetId}
					onCloseAutoFocus={onCloseAutoFocus}
					loaders={loaders}
...
export const AbuseReportDialogWithConfig = (
	props: AbuseReportDialogWithConfigProps,
): React.ReactElement => {
	const {
		config,
		abuseVector,
