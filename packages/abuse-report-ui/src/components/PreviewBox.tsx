...
const UserPreview = ({
	...
	<div className="flex items-center gap-medium padding-medium bg-action-standard radius-medium">
		...
		<div className="flex flex-col min-w-0">
			{preview.displayName && (
				<span className="text-body-medium font-bold truncate">
					{preview.displayName}
				</span>
			)}
			{preview.username && (
				// eslint-disable-next-line react/jsx-no-literals
				<span className="text-body-small content-secondary truncate">
					@{preview.username}
				</span>
			)}
		</div>
...
const PreviewBox = ({ preview }: { preview: Preview }): React.ReactElement => {
	switch (preview.type) {
		...
		default: {
			...
			throw new Error(
				`Unknown preview type: ${(exhaustiveCheck as Preview).type}`,
			);
		}
	}
};
