...
const getLocalStorageItemTrackingList = (
	itemType: string,
): Array<number> | undefined => {
	try {
		return localStorageService.getLocalStorage(
			getLocalStorageItemTrackingKey(itemType),
		) as undefined | Array<number>;
	} catch {
		return undefined;
	}
};

const addItemToLocalStorageItemTrackList = (
	itemType: string,
	id: number,
): void => {
	let itemDetailsTrackingList = getLocalStorageItemTrackingList(itemType);
	if (
		itemDetailsTrackingList === undefined ||
		itemDetailsTrackingList === null
	) {
...
export const getItemDetail = (
	...
	try {
		return localStorageService.getLocalStorage(
			getLocalStorageKey(id, itemType),
		) as undefined | TCachedItemDetailsHydratedEntry;
	} catch {
		return undefined;
	}
};
...
const cleanLocalStorageForItemType = (itemType: string): void => {
	...
	if (
		localStorageTrackingList !== undefined &&
		localStorageTrackingList !== null
	) {
		localStorageTrackingList.forEach((item) => {
			...
			if (
				...
				localStorageService.removeLocalStorage(
					getLocalStorageKey(item, itemType),
				);
			} else {
...
export const getPurchaseInfo = (
	...
	return {
		...
		purchaseFromReseller: getPurchaseFromReseller(
			itemDetail,
...
export const getTimedOptions = (
	itemDetail: TDetailEntry,
): Array<TTimedOption> | undefined => {
	if (
	...
	const timedOptionSelected = timedOptions.find(
		(timedOption) => timedOption.selected,
	);
	return [
		{
			days: 0,
