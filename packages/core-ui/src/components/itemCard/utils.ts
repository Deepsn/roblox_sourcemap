...
export const mapItemRestrictionIcons = (
	...
	if (itemRestrictions) {
		...
		if (checkIfBundle(itemType)) {
			itemCardRestrictions.isLimited = itemRestrictions.includes(
				itemRestrictionTypes.limited,
			);
			itemCardRestrictions.isRthro = itemRestrictions.includes(
				itemRestrictionTypes.rthro,
			);
			...
			if (itemCardRestrictions.isLimited) {
				...
				itemCardRestrictions.itemRestrictionIcon =
					itemRestrictionIcons.collectible;
			}
		} else {
