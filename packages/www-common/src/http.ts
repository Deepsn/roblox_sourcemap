	...
	http.setInterceptors([
		...
		(next) =>
			AsyncResult.fn(async (url, options) => {
				...
				const isFirstParty =
					url.host.endsWith(".roblox.com") ||
					url.host.endsWith(".robloxlabs.com");
				...
				if (isFirstParty && userId == null) {
					...
					options.headers.set(
						"Accept-Language",
						`${localeToUppercaseDash(locale)};q=0.01`,
					);
				...
				if (
					isFirstParty &&
					!options.headers.has("traceparent") &&
					getClient() != null
				) {
