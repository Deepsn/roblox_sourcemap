const getMetaData = () => {
	...
	const parsedDuplicationRatio =
		duplicationMeta?.dataset.duplicationRatio != null
			...
			: null;
	const duplicationRatio = Number.isNaN(parsedDuplicationRatio)
		? 0
		: parsedDuplicationRatio;
	const retryMeta = document.querySelector<HTMLElement>(
		'meta[name="page-retry-header-enabled"]',
	);
	...
	return {
		...
		duplicationRatio,
		retryAttemptHeaderEnabled:
			retryMeta?.dataset.retryAttemptHeaderEnabled === "True",
	};
};
...
const isApiSiteAvailable = (url: string) => {
	...
	if (listOfAvailableApiSites.length > 0) {
		return listOfAvailableApiSites.some(
			(apiSite) => apiSite.length > 0 && url.includes(apiSite),
		);
	}
...
export const retryAttemptHeader = (): boolean =>
	metaData.retryAttemptHeaderEnabled;
