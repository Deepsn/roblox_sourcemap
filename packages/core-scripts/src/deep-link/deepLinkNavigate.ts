...
const isItemDeeplink = (navigateSubPath: string, params: DeepLinkParams) =>
	navigateSubPath === PathPart.ItemDetails.valueOf() &&
	params.itemType &&
	params.itemId;

const getFriendshipSourceType = (
...
const buildRedirectUrlWithJoinData = (
	url: string,
			...
			return resolveShareLinksV2(params.code)
				.then((response) => {
					if (response.data.linkType === "Referral") {
						...
						// leave `/share-links` spinning. The Plus page reads the referral params and opens the
						// referral sheet inline, so the recipient lands directly on the subscribe surface.
					...
					if (
						response.data.linkType === "Experience" &&
						...
						return getUniverseIdFromPlaceId(response.data.targetId)
							.then((id: number) => {
								...
								if (
									studioLinkHandler &&
									typeof studioLinkHandler === "function"
								) {
									const studioData = {
						...
						.then((response) => {
							...
							if (
								...
								window.localStorage.setItem(
									...
									btoa(
										JSON.stringify({
											[data.placeId.toString()]: data.inviterId.toString(),
										}),
									),
								);
					...
					return resolveShareLinks(params.code, ShareLinksType.PROFILE)
						.then((response) => {
							...
							if (data.status === ProfileShareStatus.VALID) {
								...
								const friendshipSourceType =
									getFriendshipSourceType(profileShareSource);
					...
					return resolveShareLinks(params.code, ShareLinksType.SERVER)
						.then((response) => {
							...
							if (
								...
								![
									PrivateServerLinkStatus.VALID,
									PrivateServerLinkStatus.EXPIRED,
								].includes(data.status)
							) {
								return false;
							}
				...
				return resolveShareLinks(
					...
					.then((response) => {
						...
						if (data?.universeId == null) {
							...
							if (
								data.fallbackType ===
									ExperienceAffiliateDeepLinkFallbackType.PROFILE &&
								data.fallbackId
							) {
						...
						return getPlaceIdFromUniverseId(data.universeId.toString())
							.then((rootPlaceId: number) => {
								...
								// TODO: old, migrated code
								// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
								redirectUrl = buildRedirectUrlWithJoinData(
									redirectUrl,
									data.joinData!,
								);
			...
			case ShareLinksType.EXPERIENCE_DETAILS: {
				...
				return resolveShareLinks(params.code, ShareLinksType.EXPERIENCE_DETAILS)
					.then((response) => {
						...
						if (
							data?.universeId == null ||
							data.status !== ExperienceDetailsStatus.VALID
						) {
							return false;
						}
			...
			case ShareLinksType.EXPERIENCE_EVENT: {
				...
				return resolveShareLinks(params.code, ShareLinksType.EXPERIENCE_EVENT)
					.then((response) => {
						...
						let launchLink = `${UrlPart.ExperienceLauncher}placeId=${data.placeId}`;
						launchLink = buildRedirectUrlWithJoinData(
							launchLink,
							data.joinData,
						);
						// TODO: old, migrated code
