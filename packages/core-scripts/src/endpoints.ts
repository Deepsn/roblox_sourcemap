...
const getAbsoluteUrl = (relativeUrl: string): string => {
	...
	if (!relativeUrl.startsWith("/")) {
		...
		const currentDirectory = currentPath.slice(
			0,
...
const generateAbsoluteUrl = (
	...
	if (
		isRequestCrossDomain &&
		typeof urls[relativePath.toLowerCase()] !== typeof undefined
	) {
...
const isFirstPartyDomain = (host: string): boolean =>
	...
	firstPartyDomains.some((domain) => host.endsWith(domain));
