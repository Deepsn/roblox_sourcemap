...
export const displayName = (): string | null =>
	userDataset()?.displayname ?? null;

export const createdDateTime = (): string | null =>
	userDataset()?.created ?? null;
...
export const hasVerifiedBadge = (): boolean =>
	userDataset()?.hasverifiedbadge === "true";
...
export const authenticatedUser = (): AuthenticatedUser | null => {
	...
	const id =
		dataset.userid == null ? null : Number.parseInt(dataset.userid, 10);
	return {
		isAuthenticated: true,
		...
		hasVerifiedBadge: dataset.hasverifiedbadge === "true",
	};
};
