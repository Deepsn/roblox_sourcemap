...
const announceElectionResults = (isMaster) => {
	...
	for (const listener of electionListeners) {
		try {
			...
			log(
				`Error running subscribed election result handler: ${JSON.stringify(e)}`,
			);
		}
	}
};
...
const pingMasterAndInitiateElectionIfNotActive = () => {
	...
	if (
		isThisTabMaster === true ||
		Date.now() - masterLastResponseTime <= masterIdleTimeBuffer
	) {
		return;
	}
	masterNodeReply = "";
...
const purgeElectionDetails = () => {
	...
	if (
		electionTime &&
		Date.now() - lastElectionTimeInMs > electionDetailsPurgeInterval
	) {
		localStorage.removeItem(keys.electionInProgress);
	}
...
const nominateAsEligible = () => {
	...
	const masterId = localStorage.getItem(keys.masterId);
	subscribeToEvents();
	const masterLastResponseTimeString = localStorage.getItem(
		keys.masterLastResponseTime,
	);
	if (
		!masterLastResponseTimeString ||
		masterLastResponseTimeString.length === 0
	) {
