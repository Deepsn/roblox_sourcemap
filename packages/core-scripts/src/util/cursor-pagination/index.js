...
export class CursorPager {
	...
	get pagingParameters() {
		return { ...this._firstPagePagingParameters };
	}
	...
	get hasNextPage() {
		...
		if (
			this._cache.getLength(cacheKey) >
	...
	setPagingParametersAndLoadFirstPage(newPagingParameters) {
		...
		this._currentPageNumber = 1;
		this._indexCursors = {};
		this._firstPagePagingParameters = { ...newPagingParameters };
		this._pagingParameters = { ...newPagingParameters };
	...
	reloadCurrentPage() {
		...
		const cacheKey = this._getCacheKey();
		let highestIndex = 0;
		const indexCursors = this._indexCursors;
		const currentIndex = this.currentPageNumber * (this._pageSize - 1);
		...
		const invalidationIndex =
			Math.floor(currentIndex / this._loadPageSize) * this._loadPageSize;
	...
	_loadPage(pageNumber, id) {
		...
		return new Promise((originalResolve, originalReject) => {
			...
			const cacheKey = cursorPager._getCacheKey();
			const items = cursorPager._cache.getPage(cacheKey, pageNumber);
	...
	_loadNextPageIntoCache(cacheKey, id) {
		...
		return new Promise((resolve, reject) => {
			cursorPager._indexCursors[cursorPager._cache.getLength(cacheKey)] =
				cursorPager._pagingParameters.cursor;

			// The page number according to getItems (based on loadPageSize, not pageSize)
			const loadPageNumber = Object.keys(cursorPager._indexCursors).length;

			cursorPager
				._getItems({
					...cursorPager._pagingParameters,
...
export { default as cursorPaginationConstants } from "./constants";
export { default as PaginationCache } from "./PaginationCache";
