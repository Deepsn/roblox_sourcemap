...
const classicThemeEnabledForNonPlus = () => {
	...
	if (
		new URLSearchParams(window.location.search).get("deviceThemeOverride") ===
		"Classic"
	) {
		return true;
	}
...
const initialTheme = () => {
	...
	if (
		classList.contains("age-kids-variant1-theme") ||
		classList.contains("age-kids-theme")
	) {
		return "kids";
	}
