...
const realtimeStateTracker = function (
	...
	const updateCurrentState = (namespace, sequenceNumber) => {
		if (
			...
			!currentRecordedState.namespaceSequenceNumbersObj
		) {
			currentRecordedState = {
				namespaceSequenceNumbersObj: {},
			};
		}
		currentRecordedState.namespaceSequenceNumbersObj[namespace] =
			sequenceNumber;
		...
		if (localStoragePersistenceEnabled) {
			localStorage.setItem(
				getRealTimeStateKey(),
				JSON.stringify(currentRecordedState),
			);
		}
	};
	...
	const updateSequenceNumber = (namespace, sequenceNumber) => {
		...
		if (
			...
			latestNotificationInfo.namespaceSequenceNumbersObj[namespace] >
				sequenceNumber
		) {
			pushToEventStream(
				eventStreamEvents.RealTimeUpdateLatestSequenceNumber,
