...
const batchRequestFactory = new BatchRequestFactory<
	UserDataRequest,
	FriendResult
>();
...
const generateUserDataStoreMethod = (methodName: MethodType) => {
	...
	return (
		...
		return userDataProcessor.queueItem(
			userDataRequest,
			requestKey,
			cacheProperties,
		);
...
export default {
	...
	getUser: (userId: number): Promise<UserObject> =>
		userBatchRequestProcessor.queueItem({ userId }),
