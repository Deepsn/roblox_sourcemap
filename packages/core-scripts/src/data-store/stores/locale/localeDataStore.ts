...
const getLocales =
	(): AxiosPromise<LocaleProvider.ApiArrayResponseSupportedLocaleLocus> =>
		localeApiInstance.v1LocalesGet({ withCredentials: true });

const getUserLocale =
	(): AxiosPromise<LocaleProvider.UserLocalizationLocusLocalesResponse> =>
...
const setUserLocale = (
	localeCode: string,
): AxiosPromise<LocaleProvider.SuccessResponse> => {
	const payload: LocaleProvider.SetSupportedLocaleForUserRequest = {
		supportedLocaleCode: localeCode,
	};
	return localeApiInstance.v1LocalesSetUserSupportedLocalePost(payload, {
		withCredentials: true,
	});
};
...
const getLocalesWithCache = (
	...
	requestDataFromCacheOrNetwork(
		getLocales,
		localeStorageCacheKeys.getLocales,
		cacheDurationInMs,
	);
