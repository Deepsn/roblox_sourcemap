...
export default class International {
	...
	getMonthsI18n(monthFormat, modifier) {
		const format =
			validMonthFormats.indexOf(monthFormat) > -1 ? monthFormat : "short"; // default to short
	...
	d(dateObj, options) {
		...
		if (typeof options === "string" || options === undefined) {
			formatOptions =
				deprecatedDateFormats[options] || deprecatedDateFormats.short; // default to short
		...
		return this.getDateTimeFormatter().getCustomDateTime(
			dateObj,
			formatOptions,
		);
	}
	...
	n(number, options) {
		...
		const definedNumberFormats = numberConstants.getDefinedNumberFormats(
			this.currency,
		);
		...
		if (typeof options === "string" || options === undefined) {
			formatOptions =
				definedNumberFormats[options] || definedNumberFormats.decimal; // default to decimal
		} else if (typeof options === "object") {
			formatOptions = options;
		} else {
			throw new TypeError(
	...
	getWeekdaysList(dayFormat) {
		const format =
			validDayFormats.indexOf(dayFormat) > -1 ? dayFormat : "short"; // default to short
		...
		const days = daysInWeek.map(
			(index) => new Date(pivotYear, pivotMonth, index),
		);
