...
export default class DateTimeFormatter {
	...
	getFullDate(date, delimiter = defaultDelimiter) {
		...
		const secondHalfOfFullDate = this.getCustomDateTime(
			date,
			fullDateFormat[1],
		);
	...
	getCustomDateTime(date = new Date(), options = undefined) {
		const formatter = new Intl.DateTimeFormat(this.locale, options);
		try {
		...
		if (Object.keys(this.dateOrdering).length === 0) {
			...
			if (formatter.formatToParts) {
				...
				const filteredParts = parts.filter(
					(part) => part.type !== filteredOutPartType,
				);
	...
	getDefaultDateOrdering() {
		...
		dateTypes = dateTypes.sort((a, b) => a.index - b.index);
