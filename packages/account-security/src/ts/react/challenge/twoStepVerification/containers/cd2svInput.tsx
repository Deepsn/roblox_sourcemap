...
const CD2SVInput: React.FC<Props> = ({
	...
	const [shouldDebounceRetry, setShouldDebounceRetry] =
	...
	ackErrorButtonClassName = ackErrorButtonClassName.concat(
		" ",
		marginBottomClassName,
	);
			...
			requestService.twoStepVerification
				...
				.then((result) => {
					if (result.isError) {
						...
						if (
							result.error ===
							TwoStepVerification.TwoStepVerificationError.CHALLENGE_DENIED
						) {
							setModalTitleText(resources.Label.TwoStepVerification);
