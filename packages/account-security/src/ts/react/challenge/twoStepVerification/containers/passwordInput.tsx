...
const PasswordInput: React.FC<Props> = ({
	...
	const verifyCode = async () => {
		...
		const result = await requestService.twoStepVerification.verifyPasswordCode(
			userId,
			{
				challengeId,
				actionType,
			...
			<React.Fragment>
				<BodyElement>
					...
					<p className={marginBottomXLargeClassName}>
						{resources.Label.EnterPassword} {maybeDelayedText ?? ""}
					</p>
					...
					<InputControl
						...
						// Optional parameters:
						autoComplete="off"
						placeholder={resources.Label.PasswordPlaceholder}
						hideFeedback
						concealInput
						autoFocus
					/>
