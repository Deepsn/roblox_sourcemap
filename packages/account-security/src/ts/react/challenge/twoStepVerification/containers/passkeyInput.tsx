...
const PasskeyInput: React.FC<Props> = ({
	...
	const verifyCode = async () => {
		...
		const options = await requestService.twoStepVerification.getPasskeyOptions(
			userId,
			{
				challengeId,
				actionType,
		...
		if (DeviceMeta && DeviceMeta().isInApp) {
			...
			try {
				code = shouldConvertToStandardBase64
					? fido2Util.formatCredentialAuthenticationResponseApp(
							credentialString,
						)
					: credentialString;
			} catch {
			...
			const credential = await navigator.credentials
				.get({
					...
					publicKey: fido2Util.formatCredentialRequestWeb(
						JSON.stringify(makeAssertionOptions),
					),
				})
				.catch(() => {
					handleError(
		...
		const result =
			await requestService.twoStepVerification.verifyPasskeyCredential(userId, {
				challengeId,
				actionType,
				code,
			});
	...
	// Focus the "Verify"button when the component mounts.
		...
		metadata && (
			<BodyElement>
				...
				<p className={marginBottomSmallClassName}>
					{resources.Label.VerifyWithPasskey}
				</p>
