...
const AuthenticatorInput: React.FC<Props> = ({
	...
	const verifyCode = async () => {
		...
		const result =
			await requestService.twoStepVerification.verifyAuthenticatorCode(userId, {
				challengeId,
				actionType,
				code,
			});
	...
	const codeValid =
		codeError === null && code.length === metadata?.authenticatorCodeLength;
				...
				<BodyElement>
					...
					<InputControl
						...
						placeholder={resources.Label.CodeInputPlaceholderText(
							metadata.authenticatorCodeLength,
						)}
