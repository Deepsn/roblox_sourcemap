	...
	const getIntegrityToken = useCallback(async () => {
		try {
			if (DeviceMeta && DeviceMeta().isInApp) {
				...
				const integrityResultJson =
					await hybridResponseService.getNativeResponse(
				...
				eventService.sendChallengeInvalidatedEvent(
					ERROR_CONSTANTS.invalidNativeResponse,
				);
			...
			console.error(error);
			eventService.sendChallengeInvalidatedEvent(
				ERROR_CONSTANTS.getNativeResponseException,
			);
			metricsService.fireChallengeInvalidatedEvent();
		}
