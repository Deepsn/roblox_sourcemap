	...
	(): QueryParametersForTwoStepVerification | null => {
		...
		> = {
			...
			clientSupports2svRecovery:
				queryParameters[QueryParameterKey.CLIENT_SUPPORTS_2SV_RECOVERY]!,
			recoverySessionId:
				queryParameters[QueryParameterKey.RECOVERY_SESSION_ID]!,
		};
...
export type QueryParametersForRostile = z.infer<
	typeof QueryParametersForRostileValidator
>;

/**
 * Reads query parameters to render a hybrid rostile challenge.
 */
export const readQueryParametersForRostile =
	(): QueryParametersForRostile | null => {
		const queryParameters = UrlParser.getParametersAsObject();
		const queryParametersRenamed: Record<
			keyof QueryParametersForRostile,
			string
		> = {
			challengeId: queryParameters[QueryParameterKey.CHALLENGE_ID]!,
		...
		const result = QueryParametersForRostileValidator.safeParse(
			queryParametersRenamed,
		);
		if (!result.success) {
			console.error(LOG_PREFIX, result.error);
...
export const readQueryParametersForPrivateAccessToken =
	(): QueryParametersForPrivateAccessToken | null => {
		const queryParameters = UrlParser.getParametersAsObject();
		const queryParametersRenamed: Record<
			keyof QueryParametersForPrivateAccessToken,
			string
		> = {
			challengeId: queryParameters[QueryParameterKey.CHALLENGE_ID]!,
		};

		const result = QueryParametersForPrivateAccessTokenValidator.safeParse(
			queryParametersRenamed,
		);
		if (!result.success) {
			console.error(LOG_PREFIX, result.error);
...
export const readQueryParametersForDeviceIntegrity =
	(): QueryParametersForDeviceIntegrity | null => {
		const queryParameters = UrlParser.getParametersAsObject();
		const queryParametersRenamed: Record<
			keyof QueryParametersForDeviceIntegrity,
			string
		> = {
			challengeId: queryParameters[QueryParameterKey.CHALLENGE_ID]!,
		...
		const result = QueryParametersForDeviceIntegrityValidator.safeParse(
			queryParametersRenamed,
		);
		if (!result.success) {
			console.error(LOG_PREFIX, result.error);
...
export type QueryParametersForBiometric = z.infer<
	typeof QueryParametersForBiometricValidator
>;
/**
 * Reads query parameters to render a hybrid biometric challenge.
 */
export const readQueryParametersForBiometric =
	(): QueryParametersForBiometric | null => {
		const queryParameters = UrlParser.getParametersAsObject();
		const queryParametersRenamed: Record<
			keyof QueryParametersForBiometric,
			string
		> = {
			challengeId: queryParameters[QueryParameterKey.CHALLENGE_ID]!,
		...
		const result = QueryParametersForBiometricValidator.safeParse(
			queryParametersRenamed,
		);
		if (!result.success) {
			console.error(LOG_PREFIX, result.error);
 ...
 */
export const readQueryParametersForProofOfSpace =
	(): QueryParametersForProofOfSpace | null => {
		const queryParameters = UrlParser.getParametersAsObject();
		const queryParametersRenamed: Record<
			keyof QueryParametersForProofOfSpace,
			string
		> = {
			challengeId: queryParameters[QueryParameterKey.CHALLENGE_ID]!,
		...
		const result = QueryParametersForProofOfSpaceValidator.safeParse(
			queryParametersRenamed,
		);
		if (!result.success) {
			console.error(LOG_PREFIX, result.error);
