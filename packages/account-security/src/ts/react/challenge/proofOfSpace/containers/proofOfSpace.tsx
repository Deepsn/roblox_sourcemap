...
import {
	SolverParameters,
	SolverType,
} from "../../../../common/algorithm/solverCommon";
...
const ProofOfSpace: React.FC = () => {
	...
	const [puzzleSolution, setPuzzleSolution] =
		useState<GraphPebblingPuzzleAnswer | null>(null);
	...
	const checkProgress = () => {
		...
		if (
			curProgress.current <
			lastProgress.current + PROGRESS_CHECK_MINIMUM_PROGRESS
		) {
	...
	const solvePuzzle = () => {
		...
		if (worker.current == null) {
			...
			fallbackSolver.current.runAsync(
				...
				(error: string) => {
					...
					console.error(
						LOG_PREFIX,
						"exception caught in fallback solver:",
						error,
					);
					timeoutComputationAndCloseModal();
				},
		...
		worker.current.onmessage = (
			event: MessageEvent<GraphPebblingPuzzleMessage>,
		) => {
	...
	const checkAnswer = async () => {
		...
		const result = await requestService.proofOfSpace.verifyPuzzle(
			challengeId,
			puzzleSolution,
		);
