...
export const parseChallengeSpecificProperties: ParseChallengeSpecificProperties =
	(challengeId, challengeTypeRaw, challengeMetadataJson) => {
...
export const renderChallenge: RenderChallenge = async ({
	...
	challengeBaseProperties.onChallengeCompleted = async (data) => {
		const result = await requestService.genericChallenge.continueChallenge(
			sharedParameters?.genericChallengeId ||
				challengeSpecificProperties.challengeId,
			data.challengeType,
			JSON.stringify(data.metadata),
		);

		if (result.isError) {
			metricsService.fireContinueFailureEvent(
		...
		if (
			!Object.values(ChallengeType).includes(
				result.value.challengeType as ChallengeType,
			)
		) {
			if (
				...
				// eslint-disable-next-line no-console
				console.warn(
					"[old middleware] new challenge type found but no new fallback registered!",
				);
				challengeBaseProperties.onChallengeInvalidated({
			...
			const maybeChallengeSpecificProperties =
				challengeBaseProperties.newParseChallenge(
		...
		void renderChallenge({
			...
			.then((success) => {
				if (!success) {
					...
					metricsService.fireRenderFailureEvent(
						nextChallengeSpecificProperties.challengeType,
					);
					// eslint-disable-next-line no-console
	...
	if (sharedParameters?.shouldAnalyze === true) {
		...
		const onChallengeCompletedWithoutFingerprint =
	...
	switch (challengeSpecificProperties.challengeType) {
		...
		case ChallengeType.FORCE_AUTHENTICATOR: {
			...
			const fullParameters: ForceActionRedirectInterface.ChallengeParameters = {
				forceActionRedirectChallengeType:
					ForceActionRedirectInterface.ForceActionRedirectChallengeType
						.ForceAuthenticator,
				...challengeBaseProperties,
				...challengeMetadata,
			};
		...
		case ChallengeType.PRIVATE_ACCESS_TOKEN: {
			...
			return Promise.resolve(
				PrivateAccessToken.renderChallenge(fullParameters),
			);
		}

		case ChallengeType.DEVICE_INTEGRITY: {
			const { challengeId, challengeType, challengeMetadata } =
				challengeSpecificProperties;
