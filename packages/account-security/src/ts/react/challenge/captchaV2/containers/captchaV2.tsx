...
const CaptchaV2: FC = () => {
	...
	// required.
	const [blockResponse, setBlockResponse] =
		useState<CaptchaV2BlockResponse | null>(null);
	...
	const verifyRef = useRef<(isRetry: boolean) => Promise<void>>(() =>
		Promise.resolve(),
	);
	const verify = useCallback(
		async (isRetry: boolean): Promise<void> => {
			...
			if (result.errorStatusCode === HttpResponseCodes.forbidden) {
				...
				const block = requestService.captchaV2.parseBlockResponse(
					result.errorRaw,
				);
				if (block !== null) {
	...
	if (renderInline) {
		...
		return (
			<div className="margin-top-medium flex flex-col">{challengeBody}</div>
		);
	}
