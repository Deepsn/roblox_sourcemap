...
export class GraphPebblingPuzzleSolver {
	...
	constructor(params: GraphPebblingPuzzleParameters) {
		...
		this.totalComputedNodes =
			(this.layers + this.merkleLayers + 1) * this.layerSize - 1;
	...
	initPebbling(batchSize: number): void {
		...
		for (
			let i = 0;
			i < batchSize && this.curLayer.getHead() < this.layerSize;
			++i
		) {
	...
	pebble(batchSize: number): void {
		...
		for (
			...
			const pred1 =
				this.curLayer.getHead() | (1 << (this.layers - this.pebblingLayer));
			const pred2 =
				this.curLayer.getHead() & ~(1 << (this.layers - this.pebblingLayer));
	...
	createMerkleTree(batchSize: number): void {
		...
		let readIndex =
			(this.merkleTree.getHead() - Math.ceil(this.merkleTree.length / 2)) * 2;

		// Repeatedly hash siblings and push to end (note all siblings are contiguous).
		for (
			let i = 0;
			i < batchSize && this.merkleTree.getHead() < this.merkleTree.length;
			i++
		) {
	...
	open(nodeId: number): number[] {
		const merkleStartDelta =
			(this.layers - this.merkleLayers + 1) * this.layerSize;
		...
		for (let i = 0; i < this.layers + Math.log2(this.merkleLayers); i++) {
			...
			curMerkleIndex =
				curMerkleIndex % 2 === 0 ? curMerkleIndex + 1 : curMerkleIndex - 1;
			opening.push(this.merkleTree.get(curMerkleIndex));
			// Parent of current node.
			curMerkleIndex =
				this.merkleTree.length -
				Math.floor((this.merkleTree.length - curMerkleIndex) / 2);
		}
		return opening;
	}
	...
	answer(): GraphPebblingPuzzleAnswer | null {
		...
		for (let i = 0; i < this.rounds; i++) {
			...
			const pred1 =
				(challenge | (1 << (this.layers - nodeLayer))) - this.layerSize;
			const pred2 =
				(challenge & ~(1 << (this.layers - nodeLayer))) - this.layerSize;
			// Open predecessors.
	...
	progress(): number {
		// Includes init step.
		const pebblingProgress =
			this.pebblingLayer * this.layerSize + this.curLayer.getHead();
