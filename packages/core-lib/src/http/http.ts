...
let fetchWithInterceptors: FetchFunction = retryInterceptor(
	fetchWithoutInterceptors,
);
...
export const setInterceptors = (
	...
	fetchWithInterceptors = interceptors
		.toReversed()
		.reduce(
			(chain, interceptor) => interceptor(chain),
			fetchWithoutInterceptors,
		);
};
...
): AsyncResult<DeserializedJson, FetchJsonError> =>
	fetch(url, options).andThen(fromJson);
...
export const get = <T>(
	...
	options?: RequestOptions,
): AsyncResult<T, FetchJsonError<T>> =>
	fetch(url, options).andThen(fromSchema(schema));
...
export const post = <T>(
	...
	schema: ZodMiniType<T>,
	options?: RequestOptions,
): AsyncResult<T, PostJsonError<T>> =>
	postResponse(url, data, options).andThen(fromSchema(schema));
