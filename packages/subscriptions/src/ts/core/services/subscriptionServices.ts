...
export const getSubscriptionsStatuses = async (
	...
	try {
		const { data } = await httpService.get<GetSubscriptionStatusesResponse>(
			serviceConstants.url.getSubscriptionStatuses(
				params.subscriptionProductTargetKeys,
			),
		);
		return data;
	} catch (error) {
...
export const purchaseWithRobux = async (
	...
	const { data } = await httpService.post<PurchaseWithRobuxResponse>(
		urlConfig,
		{
			priceInRobux,
		},
...
export const getSubscriptionMetadata =
	async (): Promise<SubscriptionMetadata> => {
...
export const getUserSubscriptions = async (
	...
	while (hasMore) {
		const request: GetUserSubscriptionsRequest = {
			resultsPerPage: 20,
			cursor,
			expirationTimestampMsStart: expirationTimestampStart
				? expirationTimestampStart.getTime()
				: 0,
			expirationTimestampMsEnd: expirationTimestampEnd
				? expirationTimestampEnd.getTime()
				: 0,
		};
		// No way to run in parallel since we need to get the cursor to do the next request
		// eslint-disable-next-line no-await-in-loop
		const result = await httpService.get<GetUserSubscriptionsResponse>(
			urlConfig,
			request,
		);
		const rows = result.data?.subscriptions ?? [];
		const convertedSubscriptions: UserSubscription[] = rows.map(
			(subscription) => ({
				...
				paymentProvider: normalizeSubscriptionPaymentProvider(
...
export const cancelUserSubscription = async (
	...
	const urlConfig = serviceConstants.url.cancelSubscription(
...
export const dismissSubscriptionNotification = async (
	...
	const urlConfig = serviceConstants.url.dismissSubscriptionNotification(
		subscriptionTargetKey,
	);
	await httpService.post(urlConfig);
};
...
export const getSubscriptionResubscribeEligibility = async (
	...
	try {
		const { data } =
			await httpService.get<GetSubscriptionResubscribeEligibilityResponse>(
 ...
 */
export const listSubscriptionsV2 = async (
