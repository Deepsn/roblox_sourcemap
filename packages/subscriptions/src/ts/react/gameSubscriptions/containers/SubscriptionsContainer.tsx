	...
	useEffect(() => {
		if (
			...
			(deviceMeta.getDeviceMeta()?.isUniversalApp &&
				deviceMeta.getDeviceMeta()?.isDesktop) ||
		...
		const fetchData = async () => {
			...
			const { subscriptionProductsInfo } = await getSubscriptions({
				subscriptionProductType:
					SubscriptionProductType.DEVELOPER_SUBSCRIPTION_PRODUCT,
				subscriptionProviderId: parseInt(computedUniverseId, 10) ?? 0,
			});

			if (subscriptionProductsInfo.length > 0) {
				const targetKeys = subscriptionProductsInfo.map(
					(info) => info.subscriptionTargetKey,
				);
