...
export const purchaseWebSubscription = async (
	...
	try {
		...
		if (
			response.invalidReason !== undefined &&
			...
			if (
				...
				!Object.values(ViolationLabel).includes(
					extractedFailureReason as ViolationLabel,
				) ||
				expirationTimeInMinutesString === undefined
			) {
				return;
			}
			const failureReason: ViolationLabel =
				extractedFailureReason as ViolationLabel;
			const expirationTimeInMinutes = parseInt(
				expirationTimeInMinutesString,
				10,
			);
		...
		if (
			...
			onRedirectAction();
			window.location.href = buildCreditCheckoutUrl(
