	...
	Record<
		...
		Partial<
			Record<
				...
				Partial<Record<SubscriptionInputType, string>>
			>
		>
	>
> = {
	[SubscriptionPurchaseEventType.USER_INPUT]: {
		[SubscriptionViewName.PURCHASE_MODAL]: {
			[SubscriptionInputType.SUBSCRIBE]:
				"GameSubscriptionsPurchaseModalSubscribeClicked",
			[SubscriptionInputType.CANCEL]:
				"GameSubscriptionsPurchaseModalCancelClicked",
		},
...
const sendCounterEvent = (event: SubscriptionPurchaseEvent) => {
	...
	const purchaseType =
		event.purchaseFlowEventType as SubscriptionPurchaseEventType;
