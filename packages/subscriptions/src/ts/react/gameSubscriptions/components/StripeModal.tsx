	...
	const verifyPaymentProfileCreationPoll = async () => {
		try {
			const response = await verifyPaymentProfileCreation(
				providerPaymentProfileId.current,
			);
			if (response) {
				return true;
			}
		...
		async (fn: () => Promise<boolean>, interval: number, times: number) => {
			try {
				...
				if (times === 0) {
					systemFeedbackService.warning(
						translate(
							FeatureSubscriptions.ErrorStripeSavedCardSomethingWentWrong,
						),
					);
					setIsLoading(false);
					closeStripeModal();
	...
	const onFormSubmit = useCallback(async () => {
		...
		try {
			...
			if (response?.setupIntent?.status === "succeeded") {
				providerPaymentProfileId.current =
					response?.setupIntent?.payment_method?.toString() ?? "";
				...
				switch (response.error.code) {
					...
					case STRIPE_ERROR_CODES.CARD_DECLINED:
						systemFeedbackService.warning(
							translate(FeatureSubscriptions.ErrorStripeCardDeclined),
						);
						closeStripeModal();
						break;
					default:
