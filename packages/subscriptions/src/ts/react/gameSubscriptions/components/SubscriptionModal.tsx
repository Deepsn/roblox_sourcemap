...
const SubscriptionModal = ({
	...
	const buySubscription = useCallback(
		(e: React.MouseEvent<HTMLButtonElement>, paymentProvider: string) => {
			...
			if (isRobux) {
				...
				const showError = (msg?: string) => {
					systemFeedbackService.warning(
						msg || translate(FeatureSubscriptions.ErrorGenericError),
					);
					handleDone();
				};
				...
				void purchaseRobuxWebSubscription(
					...
					(serviceError: HttpServiceError) => {
						const { errorMessage } = serviceError.data as {
							errorMessage: string;
						};
			...
			void purchaseWebSubscription(
				...
				(serviceError: HttpServiceError) => {
					const { clientHint, errorCode } =
						serviceError.data as SubscriptionPurchaseErrorResponse;
					...
					switch (errorCode) {
						...
						case SubscriptionErrorCodes.VPC_REQUIRED:
							...
							updateModalContent(
								...
								translate(
									CommonVerifiedParentalConsent.DescriptionSpendingRestrictionWithSettings,
								),
						...
						case SubscriptionErrorCodes.UNSUPPORTED_LOCALE:
							updateModalContent(
								...
								translate(
									SubscriptionErrorCodesToModalContent[errorCode] ?? "",
								) ?? translate(FeatureSubscriptions.ErrorGenericError),
								translate("Action.OK"),
								true,
								() => closeErrorModal,
	...
	const footerElement = useMemo(() => {
		...
		let primaryButtonTranslationKey =
			FeatureSubscriptions.ActionSubscribeWithCreditDebitCard;
		const secondaryButtonTranslationKey =
			FeatureSubscriptions.ActionSubscribePayAnotherWay;
		...
		if (primaryPaymentProviderType === PaymentProvider.CREDITBALANCE) {
			primaryButtonTranslationKey =
				FeatureSubscriptions.ActionSubscribeWithRobloxCredit;
		}
		return (
			<div>
				<Button
				...
				<Button
					...
					onClick={(e) =>
						buySubscription(e, secondaryPaymentProviderType || "")
					}
				>
					{translate(secondaryButtonTranslationKey) ||
						secondaryPaymentProviderType}
				</Button>
			</div>
		);
	}, [
		...
		>
			...
			<Modal.Body>
				<div className="modal-body-sub-info">
					...
					<span className="font-header-2 text-emphasis sub-info sub-text-normal">
						{provider}
					</span>
					<span className="font-header-1 text-emphasis sub-info sub-text-title">
						{name}
					</span>
					<span className="sub-info">
						<span className="font-header-2 text-emphasis sub-text-normal">
							{displayPrice}
						</span>
						<span className="font-header-2 text-default sub-text-normal">
							{cadence}
						</span>
					</span>
