...
const SubscriptionCard = ({
	...
	const [primaryPaymentProviderType, setPrimaryPaymentProviderType] =
		useState("");
	...
	const discountInformation = useMemo(() => {
		if (!userBasePriceInRobux || !priceDiscountDetails?.length || !priceInRobux)
			return null;
		...
		return {
			...
			totalDiscountAmount,
			totalDiscountPercentage: Math.round(
				(totalDiscountAmount / userBasePriceInRobux) * 100,
			),
	...
	const [ItemPurchase, itemPurchaseService] = useMemo(
		() =>
			USE_UNIFIED_PURCHASE
				? createItemPurchase({ forceUnifiedModal: true })
				: [null, null],
		[],
	);
		...
		<div className="subscription-card-item bg-shift-200">
			...
			>
				...
				<div className="subscription-card-text">
					<span className="text-title-medium content-emphasis subscription-card-title">
						{name}
					</span>
					{description && (
			...
			<div className="subscription-card-overlays">
				{USE_UNIFIED_PURCHASE && ItemPurchase ? (
					<ItemPurchase
						...
						expectedCurrency={1}
						expectedSellerId={
							parseInt(subscription.subscriptionProviderId, 10) || 0
						}
						...
						subscriptionTitle={translate(
							FeatureSubscriptions.HeadingGetSubscription,
						)}
					...
					<SubscriptionModal
						...
						cadenceDisclaimer={translate(
							FeatureSubscriptions.MessageMonthlyCadenceDisclaimer,
						)}
