...
const StripeForm = ({
	...
	const [addressInfoComplete, setAddressInfoComplete] =
		useState<boolean>(false);

	const stripeFormDisclosure = {
		__html: translate(
			FeatureSubscriptions.MessageStripeAddPaymentMethodDisclosure,
			{
	...
	const handleEmailChange = useCallback(
		(event: React.ChangeEvent<HTMLInputElement>) => {
			...
			setEmailError(
				inputtedEmail !== "" && emailRegex.exec(inputtedEmail) === null,
			);
	...
	return (
		<div>
			...
			<div>
				<div
					className={`form-group form-has-feedback ${emailError ? "form-has-error" : ""}`}
				>
					<input
