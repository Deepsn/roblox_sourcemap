...
function isRemoteParentRequestConfig(
	value: unknown,
): value is RemoteParentRequestConfig {
	return (
		typeof value === "object" &&
		value !== null &&
		"translate" in value &&
...
export const RemoteParentRequestNode: NodeComponent = ({
	...
	const [
		...
		setIsChildSubjectToParentalControls,
	] = useState<boolean | undefined>();
	const [requestCreatedWithoutEmail, setRequestCreatedWithoutEmail] =
		useState(false);
	...
	const [legallySensitiveData, legallySensitiveActions] =
		useLegallySensitiveContent(consentName, "vpc-upsell-web");
	...
	const prepareEmailEntry = useCallback(async (): Promise<void> => {
		try {
			setIsChildSubjectToParentalControls(
				await api.isChildSubjectToParentalControls(),
			);
		} catch {
			...
			<div className="gap-small flex flex-col">
				...
				<p
					...
					dangerouslySetInnerHTML={{
						__html: legallySensitiveCopy.description ?? "",
					}}
				/>
				<TextInput
					type="email"
					...
					autoComplete="email"
					aria-label={
						legallySensitiveCopy.textboxLabel ?? translate("Label.ParentEmail")
					}
					placeholder={
						legallySensitiveCopy.placeholderText ??
						translate("Label.EmailCapitalized")
					}
					value={email}
					onChange={(event) => {
