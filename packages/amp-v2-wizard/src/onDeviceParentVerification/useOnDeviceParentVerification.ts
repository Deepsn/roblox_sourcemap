	...
	useEffect(() => {
		...
		const startEmbedded = (inquiryId: string) => {
			...
			personaClient = new PersonaClient({
				...
				onError: () => {
					settle({
						outcome: VendorOutcome.Failed,
						failure: VendorSessionFailure.VendorError,
					});
				},
			});
		};
		...
		const startHosted = (started: StartOnDeviceParentVerificationResponse) => {
			if (started.verificationLink == null) {
				settle({
					outcome: VendorOutcome.Failed,
					failure: VendorSessionFailure.StartFailed,
				});
				return;
			}
		...
		const startCheck = async () => {
			try {
				const started = await startOnDeviceParentVerification(
					sessionId,
					method,
				);
				if (disposed) {
