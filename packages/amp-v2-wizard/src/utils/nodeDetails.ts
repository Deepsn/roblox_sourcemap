 ...
 */
export function asConsentRows(
	value: unknown,
): AgreementConsentRow[] | undefined {
	if (!Array.isArray(value)) {
		return undefined;
	}
	const items: unknown[] = value;
 ...
 */
export function asMethodOptions(
	value: unknown,
): VerificationMethodOption[] | undefined {
	if (!Array.isArray(value)) {
		return undefined;
	}
	const items: unknown[] = value;
	...
	for (const item of items) {
		...
		if (id !== undefined && label !== undefined) {
			const description =
				"description" in item ? asText(item.description) : undefined;
