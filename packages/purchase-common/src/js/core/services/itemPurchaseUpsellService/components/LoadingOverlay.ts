export default class LoadingOverlay {
	...
	public show = (message?: string): this => {
	...
	private readonly updateOrCreateMessage = (message?: string): void => {
		if (!this.overlayDiv) {
			return;
		}
		if (!message) {
	...
	private readonly getOrCreateContentDiv = (
	...
	private readonly getOrCreateOverlayDiv = (): HTMLDivElement => {
		if (this.overlayDiv) {
	...
	private readonly getOrCreateSpinnerSpan = (
