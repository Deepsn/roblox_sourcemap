...
function autoPurchaseFlow(
	...
	const titleText = translationResource.get(
		LANG_KEYS.insufficientRobuxHeadingNew,
		{},
	);
	function onAccept() {
	...
	if (shouldShowUnifiedPurchaseModal) {
		...
		const collectibleItemId =
			itemDetail.buyButtonElementDataset?.collectibleItemId ?? null;
		const rentalOptionDays =
			itemDetail.buyButtonElementDataset?.rentalOptionDays ?? null;
