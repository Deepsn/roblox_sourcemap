...
import { writeUpsellOfferIds } from "../common/upsellOffersStore";
...
export default function generateCookieForAutoPurchase(
	...
	// Selected marketplace offers; persisted alongside the cookie's discounted
	// expectedPrice so the auto-purchase charges the agreed price.
	offerIds?: string[],
) {
	...
	const purchaseMetadata = `${itemPurchaseObj.expectedCurrency},${itemPurchaseObj.expectedPrice},${
		itemPurchaseObj.expectedSellerId
	},${itemPurchaseObj.userassetId ?? ""}`;
	...
	writeUpsellOfferIds(upsellUuid, offerIds);
