...
const trustedUrlConfig = (): Pick<
	UrlConfig,
	"retryable" | "withCredentials"
> => ({
	retryable: true,
...
export async function getTrustedFriendStatus(
	userId: number,
): Promise<TrustedFriendStatusResponse> {
...
export async function acceptTrustedFriendRequest(
	userId: number,
): Promise<unknown> {
	const urlConfig: UrlConfig = {
...
export async function sendTrustedFriendRequest(
	userId: number,
	source?: string,
): Promise<unknown> {
