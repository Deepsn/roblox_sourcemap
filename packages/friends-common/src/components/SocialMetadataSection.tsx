...
const ORIGIN_DISPLAY_MAP: Partial<
	Record<FriendRequestOriginSource, FriendRequestOriginDisplay>
> = {
	...
	[FriendRequestOriginSource.PhoneContactImporter]: {
		...
		translationKey:
			trustedFriendsTranslationKeys.friendRequestOriginPhoneContactImporter,
	},
	[FriendRequestOriginSource.PlayerSearch]: {
		iconName: "icon-filled-magnifying-glass",
		translationKey:
			trustedFriendsTranslationKeys.friendRequestOriginPlayerSearch,
	},
};
...
function profileInsightDateTimeToUnixSeconds(
	value: unknown,
): number | undefined {
...
function parseProfileInsights(
	insights: ProfileInsightJson[],
): ParsedSocialInsights {
	...
	for (const insight of insights) {
		...
		if (origin?.friendRequestOriginSource) {
			friendRequestOriginDisplay =
				ORIGIN_DISPLAY_MAP[
					origin.friendRequestOriginSource as FriendRequestOriginSource
				];
		}
	}
...
const SocialMetadataSection = ({
	userId,
}: SocialMetadataSectionProps): React.JSX.Element => {
	...
	useEffect(() => {
		...
		const parsed = parseProfileInsights(
			Array.isArray(profileInsights) ? profileInsights : [],
		);
		setInsights(parsed);
			...
			{visibleDetails.length && !isFetchingInsights ? (
				<ul className="flex flex-row flex-wrap gap-medium justify-start items-center list-none margin-none padding-none">
					{visibleDetails.map(({ id, iconName, label }) => (
						<li
							key={id}
							className="inline-flex max-width-full flex-row items-center gap-xsmall"
						>
							<Icon
								name={iconName}
								size="XSmall"
								className="content-default shrink-0"
							/>
							<span className="text-body-medium content-default min-width-0">
								{label}
							</span>
						</li>
					))}
