...
const TrustedFriendsModal = ({
	...
	const { buttonConfig, performPrimaryAction } =
		useGetTrustedFriendsModalButtons({
			userId,
			linkTokens,
			trustedFriendAction,
			onClose,
	...
	const copyEntry = useMemo(
		() => trustedFriendsModalVariantToText[modalVariant],
		[modalVariant],
	);
...
const TrustedFriendsModalActionsPreCheck = ({
	...
	const {
		...
		queryFn: () =>
			getTrustedFriendAction(userId, { friendRequestToken: linkTokens }),
		enabled: open,
	});
	...
	const modalVariant = useMemo(():
		...
		if (
			!trustedFriendAction ||
			!isMappedTrustedFriendAction(trustedFriendAction)
		) {
			return "invalid";
		}
	...
	const shouldShowModal = !(
		!open ||
		modalVariant === "pending" ||
		modalVariant === "invalid"
	);
