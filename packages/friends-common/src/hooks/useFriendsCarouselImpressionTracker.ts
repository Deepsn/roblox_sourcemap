...
const useFriendsCarouselImpressionTracker = (
	...
	const buildItemImpressionsProperties: TBuildItemImpressionsEventProperties =
		useCallback(
			(impressedIndexes: number[]) => {
				if (friendsList) {
					...
					return {
						...
						[ItemImpressionsMetadata.ItemPositions]:
							filteredImpressedIndexes.map((index) => index + 1),
						[ItemImpressionsMetadata.PositionsInTopic]:
							filteredImpressedIndexes.map((index) => index + 1),
						// Friends Carousel is one row, so all items have a row number of 1
						[ItemImpressionsMetadata.RowNumbers]: filteredImpressedIndexes.map(
							() => 1,
						),

						[UserImpressionsMetadata.Presences]: filteredImpressedIndexes.map(
							(index) =>
								getUserPresenceType(
						...
						[UserImpressionsMetadata.PresenceUniverseIds]:
							filteredImpressedIndexes.map(
								(index) => friendsList[index]?.presence.universeId ?? -1,
							),
						[UserImpressionsMetadata.FriendStatuses]:
							filteredImpressedIndexes.map(() => FriendStatus.Friend),
