...
async function startTrustedFriendVpcUpsell(
	targetUserId: number,
): Promise<void> {
	try {
		await AccessManagementUpsellV2Service.startAccessManagementUpsell({
			...
			ampFeatureCheckData: [
				{ name: "targetUser", type: "UserId", value: String(targetUserId) },
			],
			isAsyncCall: false,
			usePrologue: true,
...
export function useGetTrustedFriendsModalButtons({
	...
	const performPrimaryAction = useCallback(async () => {
		...
		const runHandler = async (
			handlerType: TrustedFriendPrimaryHandlerEnum,
		): Promise<void> => {
			switch (handlerType) {
