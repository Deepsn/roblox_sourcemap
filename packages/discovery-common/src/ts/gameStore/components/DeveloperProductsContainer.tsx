...
const DeveloperProductsContainer = ({
	translate,
}: WithTranslationsProps): JSX.Element => {
	const { universeId = "" } = metadataConstants.metadataData() || {};

	const [developerProducts, setDeveloperProducts] = useState<
		TDeveloperProduct[] | undefined
	>(undefined);
	...
	const [resultsPerPage, setResultsPerPage] = useState(
		CARDS_PER_ROW_MAX * ROWS_PER_PAGE,
	);
	...
	useEffect(() => {
		const updateResultsPerPage = () => {
			...
			const newResultsPerPage =
				Math.min(Math.max(cardsToFit, CARDS_PER_ROW_MIN), CARDS_PER_ROW_MAX) *
				ROWS_PER_PAGE;
			...
			if (developerProducts?.length) {
				const newTotalPages = Math.ceil(
					developerProducts.length / newResultsPerPage,
				);
...
export default withTranslations(
	...
	developerProductsTranslationConfig,
);
