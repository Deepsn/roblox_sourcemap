...
const useEventRsvpStatus = (
	...
	const [userHasRsvpd, setUserHasRsvpd] = useState(
		eventItem.userRsvpStatus === RSVP_STATUS.GOING,
	);
	...
	const rsvpSuccessMessage = useMemo(() => {
		...
		return getTranslationStringForKeyWithFallback(
			translate,
		...
		eventItem.eventTime.startUtc,
		eventItem.displayTitle,
		eventItem.title,
		translate,
	]);
	...
	const handleToggleRsvpClick = useCallback(
		async (e: React.MouseEvent<HTMLButtonElement>) => {
			const newRsvpStatus = userHasRsvpd
