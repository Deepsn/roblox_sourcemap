...
const EventsListContainer = ({
	...
	const { playabilityStatus } = usePlayabilityStatus(
	...
	const filteredEventList = useMemo(() => {
		return eventList.filter(
			(e) => tryParseDate(e.eventTime.endUtc) > new Date().getTime(),
		);
	}, [eventList]);
	...
	const isSeeMoreBtnVisible = useMemo(() => {
		return (
			filteredEventList.length > defaultItems + itemsPerExpansion * expansions
		);
	...
	useLayoutEffect(() => {
		const updateItemsPerRowThrottled = throttle((feedWidth: number) => {
			setItemsPerRow(
				getNumTilesPerRow(feedWidth, 0, TComponentType.ExperienceEventsTile),
			);
		}, 100);
	...
	const displayedEventList = useMemo(() => {
		return filteredEventList
			.sort((a, b) =>
				tryParseDate(a.eventTime.startUtc) < tryParseDate(b.eventTime.startUtc)
					? -1
					: 1,
			)
	...
	return (
		<React.Fragment>
			<div className="container-header">
				<h3>
					{getTranslationStringForKeyWithFallback(translate, "eventsTitle")}
				</h3>
			</div>
			...
			{isSeeMoreBtnVisible && (
				<button
					type="button"
					aria-label={getTranslationStringForKeyWithFallback(
						translate,
						"seeMore",
					)}
