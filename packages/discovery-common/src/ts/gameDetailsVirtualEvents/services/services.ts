...
export const getEventDetailsRes = async (eventId: string): Promise<unknown> => {
	...
	const endpoint = endpointsFactory.getVirtualEventsDetailsEndpoint(
		eventId,
		isUserAuthenticated,
	);
...
export const getTotalRsvpCounters = async (
	eventId: string,
): Promise<RsvpCounters> => {
	const endpoint =
		endpointsFactory.getVirtualEventsTotalRsvpCountersEndpoint(eventId);
...
export const postRsvpStatus = async (
	eventId: any,
	status: string,
): Promise<unknown> => {
...
export const getEventsForUniverseId = async (
	universeId: any,
