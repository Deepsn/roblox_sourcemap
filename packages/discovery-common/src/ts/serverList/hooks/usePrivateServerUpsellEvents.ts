...
const PrivateServerUpsellEventsContext =
	createContext<PrivateServerUpsellEvents>({
...
function sendUpsellEvent(
	context: PrivateServerEventContext,
	universeId: number,
): void {
	sendEventWithTarget(
...
export function PrivateServerUpsellEventsProvider({
	...
	useEffect(() => {
		...
		if (checkoutReturnQuery.isLoading || checkoutReturnQuery.data === undefined)
			return;
		...
		hadPendingCheckout,
		checkoutReturnQuery.isLoading,
		checkoutReturnQuery.data,
		dimensions,
	]);

	const trackFreeWithPlusClick = useCallback(() => {
		sendUpsellEvent(
			PrivateServerEventContext.PLUS_UPSELL_FREE_WITH_PLUS_CLICK,
			universeId,
		);
	}, [universeId]);
	...
	const trackSubscribeBannerClick = useCallback(() => {
		sendUpsellEvent(
			PrivateServerEventContext.PLUS_UPSELL_SUBSCRIBE_BANNER_CLICK,
			universeId,
		);
	}, [universeId]);
	...
	const trackSubscribeSheetClick = useCallback(() => {
		sendUpsellEvent(
			PrivateServerEventContext.PLUS_UPSELL_SUBSCRIBE_SHEET_CLICK,
			universeId,
		);
	...
	const trackSheetDismiss = useCallback(() => {
		sendUpsellEvent(
			PrivateServerEventContext.PLUS_UPSELL_SHEET_DISMISS,
			universeId,
		);
	...
	return React.createElement(
		PrivateServerUpsellEventsContext.Provider,
		{ value },
		children,
	);
}
