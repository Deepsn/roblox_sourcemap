			...
			<React.Fragment>
				<span className="text-body-medium content-emphasis flex items-center gap-xsmall">
					...
					<span className="flex flex-row items-center gap-[2px]">
						<Icon name="icon-filled-robux" size="Medium" />
						<span>
							{translate(resources.pricePerMonthText, { price: String(price) })}
						</span>
					</span>
				</span>
				...
				const errorData =
					error != null && typeof error === "object" && "data" in error
						? // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
							(
								error as {
									data?: { errors?: { userFacingMessage?: string }[] };
								}
							).data
						: undefined;
				const errorMsg =
					errorData?.errors?.[0]?.userFacingMessage ??
					translate(resources.purchaseError);
				setSubmitError(errorMsg);
			});
	}, [
		autoGeneratedServerName,
		createServer,
		onClose,
		onServerCreated,
		translate,
			...
			<SheetBody>
				<div className="padding-x-xlarge padding-y-medium gap-y-xlarge flex flex-col">
					...
					<div className="flex flex-col gap-y-medium">
						...
						>
							{translate(
								isPlusUser ? resources.createText : resources.subscribeText,
							)}
						</Button>
