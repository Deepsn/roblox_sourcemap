...
const PrivateServersSection = ({
	...
	const { trackSubscribeBannerClick, trackUpsellBannerShown } =
		usePrivateServerUpsellEvents();

	/** `null` while loading — hide Plus entrypoints until GUAC resolves (avoids fail-open flicker). */
	const [appPolicyDisablePlusEntrypoints, setAppPolicyDisablePlusEntrypoints] =
	...
	const canCreate =
		isPlusUser ||
		canCreatePrivateGameServer(serversWithOwner, privateServerLimit);

	const plusUpsellBanner = !isPlusUser &&
		canCreateServer &&
		!hideRobloxPlusEntrypoints && (
					...
					return (
						<ServerListItem
							...
							isJoinDisabled={
								...
								isExpired ||
								(!!joinRestricted &&
									(server.playing || server.players?.length || 0) === 0)
							}
