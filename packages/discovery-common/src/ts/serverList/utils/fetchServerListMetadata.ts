...
const fetchServerListMetadata = async (
	...
	try {
		...
		const playabilityStatusPromise = bedev1Services.getPlayabilityStatus(
			universeId.toString(),
		);

		const privateServerSettings =
			await bedev2Services.getPrivateServerSettings(universeId);
		...
		const placeDetailsPromise = bedev1Services.getPlaceDetails(
			placeId.toString(),
		);
		...
		const [placeDetails, canUserManagePlace, playabilityStatus] =
			await Promise.all([
				placeDetailsPromise,
		...
		if (!placeDetails || !playabilityStatus) {
			...
			return Promise.reject(
				new Error("Place details or playability status is undefined"),
			);
		}

		const canCreateServer =
			privateServerSettings.privateServerData.isAvailable &&
			playabilityStatus.isPlayable;

		const preopenCreatePrivateGame = getShouldPreopenCreateVip(
		...
		return {
			...
			privateServerProductId:
				privateServerSettings.privateServerData.privateServerProductId,
			privateServerLimit:
				privateServerSettings.privateServerData.privateServerLimit,
