...
const GameTilePlayerCountPill = ({
	...
	return (
		<div
			className={playerCountClassName}
		...
		>
			...
			>
				{featureTypes.map(
					(type) =>
						featureIconMap[type] && (
							<span key={type} className={featureIconMap[type]} />
						),
				)}
			</div>
		</div>
...
const GameTileOverlayPill = ({
	...
	if (gameTilePillData) {
		return (
			<GameTilePillsContainer pills={gameTilePillData} isFocused={isFocused} />
		);
	}
	...
	if (
		...
		(playerCountStyle === TPlayerCountStyle.Always ||
			playerCountStyle === TPlayerCountStyle.Hover)
	) {
		return (
