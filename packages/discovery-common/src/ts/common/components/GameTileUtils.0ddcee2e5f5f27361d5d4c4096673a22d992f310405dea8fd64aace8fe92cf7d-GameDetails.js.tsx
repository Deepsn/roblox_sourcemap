...
const { keyBoardEventCode, numberOfInGameAvatarIcons, numberOfInGameNames } =
	common;
export type TBuildEventProperties = (
	gameData: TGameData,
	id: number,
) => TGameDetailReferral;
...
export const GameTileHiddenTileMessage = ({
	message,
}: {
	message: string;
}): JSX.Element => <span className="text-body-medium">{message}</span>;
...
export const GameTileUndoButton = ({
	...
	const sendGameTileHiddenAction = useGameTileHiddenAction(
		universeId,
		topicId,
		page,
	);
	...
	const handleClick = useCallback(
		(e: React.MouseEvent) => {
			...
			e.stopPropagation();
			sendGameTileHiddenAction(
				GameTileHiddenActionType.GameTileHiddenUndone,
				hiddenReason,
			);
			onUndo?.();
		},
...
const GameTileAvatarThumbnail = ({
	user,
}: {
	user: TGetFriendsResponse;
}): JSX.Element => {
	return (
		<Thumbnail2d
...
const GameTileFacepile = ({
	...
	const numIconsToShow = coverCountText
		? numberOfInGameAvatarIcons - 1
		: numberOfInGameAvatarIcons;
...
export const ShimmedPlayerInteractionModalComponent = ({
	...
	game,
	translate,
}: TShimmedPlayerInteractionModalComponentProps &
...
export const getNumTilesPerRow = (
	...
	const { minTileWidth, columnGap, minTilesPerRow, maxTilesPerRow } =
		displayConfig;
	...
	const itemsPerRow = Math.min(
		maxTilesPerRow,
		Math.max(minTilesPerRow, numItemsFit),
	);
