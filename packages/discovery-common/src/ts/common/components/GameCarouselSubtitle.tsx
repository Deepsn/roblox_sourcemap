...
const GameCarouselSubtitle: React.FC<Props> = ({
	...
	const [countdownTimeRemaining, setCountdownTimeRemaining] = useState<
		number | undefined
	>(
		endTimestampInt !== undefined
			? endTimestampInt - Math.floor(Date.now() / 1000)
			: undefined,
	);
	useEffect(() => {
		if (endTimestampInt !== undefined) {
			setCountdownTimeRemaining(
				endTimestampInt - Math.floor(Date.now() / 1000),
			);
	...
	const renderSubtitleWithLink = useMemo(() => {
		if (formatSubtitleLink && subtitleLink && subtitle) {
			...
			if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
				...
				const linkText = subtitle.slice(
					startIdx + linkStartDelimiter.length,
					endIdx,
				);
	...
	return subtitle ? (
		<div className="sort-subtitle-container">
			<span className="font-sort-subtitle text-default">
