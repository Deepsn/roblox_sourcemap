...
export default function usePlaceIdOverride(
	...
	const [validatedPlaceIdOverride, setValidatedPlaceIdOverride] = useState<
		number | null
	>(null);
	const [isResolvingPlaceId, setIsResolvingPlaceId] = useState(
		!!placeIdOverride,
	);
	...
	useEffect(() => {
		...
		bedev1Services
			...
			.then((placeDetails) => {
				if (
					placeDetails?.universeId != null &&
					String(placeDetails.universeId) === universeId
				) {
