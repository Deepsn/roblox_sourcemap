...
export const GameCarouselHorizontalScroll = ({
	...
	const [isScrollBackDisabled, setIsScrollBackDisabled] =
		useState<boolean>(true);
	const [isScrollForwardDisabled, setIsScrollForwardDisabled] =
		useState<boolean>(true);
	...
	const [carouselScrollRef, carouselScrollWidth] =
		useElementWidthResizeObserver();

	const [carouselWindowRef, carouselWindowWidth] =
		useElementWidthResizeObserver();
	...
	const numGamesFitted = useMemo(() => {
		...
		if (carouselWindowWidth !== undefined && gameTileWidth !== undefined) {
			return Math.max(
				1,
				Math.floor(
					(carouselWindowWidth + gameTileWidthOffset) /
						(gameTileWidth + gameTileWidthOffset),
				),
			);
		}
	...
	useEffect(() => {
		...
		if (isLoadingMoreGames) {
			...
			Math.abs(carouselLeftValue) +
				carouselWindowWidth +
				wideTileHoverGrowWidthPx >=
				carouselScrollWidth
		) {
	...
	const checkLoadMoreGames = useCallback(() => {
		...
		if (
			cursorIndex + lookAheadGames >= gameData?.length &&
			loadMoreGames &&
			!isLoadingMoreGames
		) {
			loadMoreGames();
	...
	const onScrollToPrev = useCallback(() => {
		if (!isScrollBackDisabled) {
			...
			setCarouselLeftValue((prevLeftValue) =>
				Math.min(prevLeftValue + scrollDistance, 0),
			);
			...
			setCarouselLeftValue((prevLeftValue) => {
				...
				if (carouselScrollWidth !== undefined) {
					...
					const scrollerOffset: number =
						hideScrollBackWhenDisabled && isScrollBackDisabled
							? scrollerWidth
							: 0;
					return (
						Math.max(prevLeftValue - scrollDistance, -1 * carouselScrollWidth) +
						scrollerOffset
					);
				}
	...
	const getIsGameOnScreen = useCallback(
		(gameIndex: number) => {
			return (
				gameIndex >= cursorIndex && gameIndex < cursorIndex + numGamesFitted
			);
		},
	...
	const listContainerClassName = useMemo(() => {
		return classNames({
			...
			"game-carousel wide-game-tile-carousel scrollable-carousel":
				isWideTileCarousel,
