...
const CarouselContainer = ({
	...
	const { isPlayable, playabilityStatus, isFetchingPlayability } =
	...
	const filteredAssets: TCarouselItem[] = React.useMemo(() => {
		...
		const nonFailedAssets = assets.filter(
			(item) => !failedItemIds.has(item.id),
		);

		if (!isEligibleForVideoPreview) {
			return nonFailedAssets.filter(
