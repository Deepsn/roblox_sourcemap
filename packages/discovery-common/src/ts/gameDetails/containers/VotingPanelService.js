...
const VotingPanelService = (function () {
	...
	var loadVotingService = function (rootNodeSelector, placeId) {
		...
		if (rootNode?.length === 0) {
			throw new Error(
				"VotingPanelService.loadVotingService: Root node not found",
			);
		}
	...
	const checkPolicyAndLoad = function (
		...
		callBehaviour("app-policy")
			.then((response) => {
				enableLikesCounts =
					response?.EnableAggregateLikesFavoritesCount ?? false;
			})
			.catch(() => {
	...
	var getModalProperties = function (modalType) {
		...
		var votingPanelResources = translationProvider.getTranslationResource(
			"Feature.VotingPanel",
		);
		...
		var accountsPageLink =
			...
			accountPageUrl +
			"'>" +
			langMap["Label.AccountPageTitle"] +
			"</a>";
		var registerPageLink =
			"<a href='" +
			registerPageUrl +
			"'>" +
			langMap["Label.LoginOrRegisterPageTitle"] +
			"</a>";
		...
		var modalProperties = {
			...
			EmailOrPhoneIsVerified: {
				...
				bodyContent: votingPanelResources.get(
			...
			GuestUser: {
				...
				bodyContent:
					...
					"</div><div>" +
					intl.f(langMap["Label.GuestUserMessage"], {
