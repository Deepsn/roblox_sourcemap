...
function PlayButtonContents({ attributionId }: TPlayButtonProps): JSX.Element {
	...
	const [{ shouldDefaultToDownloadButton, isMarketingCampaignReferred }] =
		React.useState(() => {
			const queryParams = parseQueryString(window.location.search);
			return {
				shouldDefaultToDownloadButton:
					queryParams.downloadButtonDefault === "true",
	...
	const isDownloadButtonExperimentEligible =
		...
		!shouldDefaultToDownloadButton;
	const {
		isDownloadButtonOverrideEnabled,
		isLoading: isDownloadButtonExperimentLoading,
	...
	const placeId = validatedPlaceIdOverride
		? String(validatedPlaceIdOverride)
		: rootPlaceId;
	...
	if (
		isFetchingPolicy ||
		isResolvingPlaceId ||
		isDownloadButtonExperimentLoading
	) {
		return <Loading />;
	}
	...
	const playButtonBaseEventProperties = {
		[SessionInfoType.DiscoverPageSessionInfo]:
			referralParams.discoverPageSessionInfo,
		[SessionInfoType.GameSearchSessionInfo]:
			referralParams.gameSearchSessionInfo,
		...
		[EventStreamMetadata.PlaceIdOverride]:
			validatedPlaceIdOverride ?? undefined,
	};
		...
		<React.Fragment>
			...
			<ContextualMessage
				...
				shouldShowNoticeAgreementIfPlayable={
					experienceDetailsNoticeType ===
					ExperienceNoticeType.InExperiencePurchase
				}
