...
const useGameDescriptionImpressionTracker = (
	descriptionRef: RefObject<HTMLPreElement>,
	{
		...
		referralSessionInfo,
		descriptionText,
	}: GameDescriptionImpressionTrackerOptions,
): void => {
	const { isLoading: isEventListLoading } =
		useEventDetailsForUniverseId(universeId);
	const isEventListSettled = !isEventListLoading;
	const isDescriptionTextEmpty =
	...
	const onEmit = useCallback(
		(threshold: number) => {
			const eventStreamParams = {
				...
				[EventStreamMetadata.ComponentType]:
					GAME_DESCRIPTION_IMPRESSION_COMPONENT_TYPE,
			...
			const eventParams =
				eventStreamConstants[EventType.ExpandableTextImpression](
					eventStreamParams,
				);
