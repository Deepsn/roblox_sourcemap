	...
	switch (item.type) {
		...
		default:
			window.EventTracker?.fireEvent(
				carouselErrorCounters.MediaGalleryMediaChangedUnknownItemType,
			);
			return {
				[EventStreamMetadata.IsVideo]: false,
				[EventStreamMetadata.IsYoutubeVideo]: false,
...
const useMediaCarouselAnalytics = (
	...
	const sendMediaGalleryMediaChangedEvent = useCallback(
		(newIndex: number, previousIndex: number, isAutoAdvance: boolean) => {
			...
			if (!nextItem) {
				window.EventTracker?.fireEvent(
					carouselErrorCounters.MediaGalleryMediaChangedMissingItem,
				);
				return;
			}
			...
			const derivedEventParams: TMediaGalleryAction = {
				...baseEventParams,
				...assetParams,
			};
			const eventStreamParams =
				eventStreamConstants.mediaGalleryMediaChanged(derivedEventParams);
