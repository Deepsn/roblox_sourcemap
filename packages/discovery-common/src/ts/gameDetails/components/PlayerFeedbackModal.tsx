...
import {
	sendPlayerFeedbackEvent,
	voteTypeToString,
} from "../utils/playerFeedbackUtils";
import {
	CommonUIMessages,
...
const PlayerFeedbackModal: React.FC<PlayerFeedbackModalProps> = ({
	...
	const [disabledReason, setDisabledReason] = useState(
		translate(PlayerFeedbacksVoteForm.MessageMinLengthCharacters, {
			count: minLength,
		}),
	);
	...
	const reportFooter: string = useMemo(() => {
		let footerText = translate(
			PlayerFeedbacksVoteForm.MessageReportAbuseInsteadLong,
		);
		footerText = footerText.replace(
			"<u>",
	...
	useEffect(() => {
		sendPlayerFeedbackEvent(
			"playerFeedbackFormModalMounted",
			placeId,
			voteType,
		);
	}, [placeId, voteType]);
	...
	const onCloseButtonPress = useCallback(() => {
		sendPlayerFeedbackEvent(
			"dismissPlayerFeedbackFormModal",
			placeId,
			voteType,
		);
		hideModal();
	...
	const onTextboxChange = useCallback(
		(event: React.ChangeEvent<HTMLTextAreaElement>) => {
			if (event.target.value.length > maxLength) {
				setDisabledReason(
					translate(PlayerFeedbacksVoteForm.MessageMaxLengthCharacters, {
						count: maxLength,
					}),
				);
	...
	const handleSubmit = useCallback(async () => {
		sendPlayerFeedbackEvent(
			"clickPlayerFeedbackFormModalSubmit",
			placeId,
			voteType,
		);
		try {
			...
			hideBannerUntilReload();
			showSuccessMessage(
				translate(PlayerFeedbacksVoteForm.MessageSubmitSuccess),
			);
		} catch (error) {
	...
	return (
        ...
        directly from the Roblox backend, with NO UGC text in the string */}
				...
				<div
					...
					dangerouslySetInnerHTML={{ __html: modalFooter }}
				/>
				{voteType === ReviewCategoryType.Downvote && (
					// eslint-disable-next-line react/no-danger
					<div
						className="font-caption-body"
						dangerouslySetInnerHTML={{ __html: reportFooter }}
					/>
				)}
			</div>
		</Modal>
