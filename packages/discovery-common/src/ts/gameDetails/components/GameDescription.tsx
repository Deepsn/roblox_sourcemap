...
const GameDescription = ({
	...
	const [userLanguageName, setUserLanguageName] = React.useState<
		string | undefined
	>(undefined);
	...
	useEffect(() => {
		...
		if (name && sourceName && name !== sourceName) {
			try {
				const gameTitleContainer = document.querySelector(
					".game-title-container",
				);
				if (gameTitleContainer) {
					const gameTitleElement = gameTitleContainer.querySelector("h1");
					if (gameTitleElement) {
						gameTitleElement.textContent = isSwappedToSource
							? sourceName
							: name;
					}
				}
	...
	return (
		<React.Fragment>
			...
			{isEligibleForSwap && (
				...
				>
					<span className="swap-translated-content font-bold">
						{swapButtonText}
					</span>
