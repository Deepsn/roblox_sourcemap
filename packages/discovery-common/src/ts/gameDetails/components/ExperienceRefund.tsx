...
import {
	gameDetailsPage,
	homePage,
} from "../../common/constants/configConstants";
...
const RefundPolicyText: React.FC<TRefundPolicyTextProps> = ({ policyData }) => {
	...
	const policyTextLinkRegex = new RegExp(
		`${linkStartDelimiter}|${linkEndDelimiter}`,
	);
	...
	const [beforeLink, linkText, ...afterLink] =
		policyText.split(policyTextLinkRegex);
...
const ExperienceRefund: React.FC<TExperienceRefundProps> = ({
	gameDetails,
	translate,
}) => {
								...
								onClick={() => {
									if (rootPlaceId) {
										...
										window.EventTracker?.fireEvent(
											gameDetailsPage.requestRefundError,
										);
									}
								}}
							>
