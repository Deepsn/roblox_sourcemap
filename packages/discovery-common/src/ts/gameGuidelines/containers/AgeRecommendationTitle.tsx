...
export const AgeRecommendationTitle = ({
	...
	const [contentContainsStrongLanguage, setContentContainsStrongLanguage] =
		useState(false);
	const [
		communicationContainsStrongLanguage,
		setCommunicationContainsStrongLanguage,
	] = useState(false);
	const [experienceGuidelines, setExperienceGuidelines] =
		useState<ExperienceGuidelines | null>(null);
	const [headerDisplayName, setHeaderDisplayName] = useState<string | null>(
		null,
	);

	const { universeId = "" } = metadataConstants.metadataData() || {};

	const getCommunicationStrongLanguageSettingForUniverse =
		useCallback(async () => {
			try {
				const atfsResponse =
					await assetTextFilterSettingsService.getAssetTextFilterSettings(
						universeId,
					);
				setCommunicationContainsStrongLanguage(
					atfsResponse?.Profanity === true,
				);
			} catch {
	...
	const getAgeGuidelinesForUniverse = useCallback(async () => {
		...
		try {
			const egsResponse =
				await experienceGuidelinesService.getAgeRecommendation(universeId);
			const ageRecommendation =
				egsResponse.data as AgeRecommendationDetailsResponse;
			...
			let descriptorDisplayNames =
				ageRecommendation.ageRecommendationDetails.descriptorUsages
      ...
      */
			if (!descriptorDisplayNames) {
				if (
					...
					descriptorDisplayNames =
						gameGuidelinesConstants.SuitableForAllAgesText;
				} else {
					descriptorDisplayNames = "";
				}
			}
			...
			setContentContainsStrongLanguage(
				strongLanguageDescriptor?.contains === true,
			);
		} catch {
			setExperienceGuidelines(null);
	...
	const updateState = useCallback(async () => {
		...
		getAgeGuidelinesForUniverse,
		getCommunicationStrongLanguageSettingForUniverse,
	]);
	...
	return (
		<React.Fragment>
			{isDisplayAgeRecommendationDetails &&
				igrsRating &&
				(igrsRating as string) !== "" && (
					<div data-testid="igrs-rating-label-container">
						<IgrsRatingLabel
							igrsRating={igrsRating}
							igrsRatingDisplayMessage={
								experienceGuidelines?.igrsRatingDisplayMessage
							}
							translate={translate}
						/>
					</div>
