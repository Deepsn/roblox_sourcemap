...
export type GameInstanceQueryParams = Record<
	string,
...
export default {
	...
	getAccountSettingsGuacPolicy:
		(): Promise<AccountSettingsGuacPolicy | null> => {
			return callBehaviour<AccountSettingsGuacPolicy>(
				"account-settings-ui",
			).catch(() => null);
		},
	getCurrentUserBalance: (userId: number) => {
		...
		return http
			.get<UserCurrencyResponse>(urlConfig)
