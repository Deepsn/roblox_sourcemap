...
import attributionUtils, {
	AttributionType,
} from "../../../ts/common/utils/attributionUtils";
	...
	const sendPlayGameClickedEvent = () => {
		const privateServerParams =
			serverListType === serverListTypes.Vip.key
				...
				: {};
		const properties = {
			attributionId: attributionUtils.getAttributionId(
		...
		return () => {
			...
			if (accessCode) {
				...
				window.Roblox.GameLauncher?.joinPrivateGame(
					...
					"",
					joinAttemptId,
					window.Roblox.GameLauncher?.isJoinAttemptIdEnabled()
						? gamePlayIntentContext
						: undefined,
				);
				return;
