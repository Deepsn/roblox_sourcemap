...
const urlConstants = {
	getUserProfileUrl: (userId: number) =>
		getAbsoluteUrl(`/users/${userId}/profile`),
	...
	getShutdownGameInstanceUrl: () =>
		getAbsoluteUrl(`${matchmakingApi}/v1/game-instances/shutdown`),
	getPrivateServerConfigUrl: (privateServerId: number, serverListId?: number) =>
		getUrlWithQueries(`/private-server/configure`, {
			privateServerId,
			serverListId,
		}),
	getVipServerUrl: (vipServerId: number) =>
		`${gamesApi}/v1/vip-servers/${vipServerId}`,
	createVipServerUrl: (universeId: number) =>
		`${gamesApi}/v1/games/vip-servers/${universeId}`,
	updateVipServerSubscriptionUrl: (vipServerId: number) =>
		`${gamesApi}/v1/vip-servers/${vipServerId}/subscription`,
	createPrivateServerUrl: (universeId: number) =>
		`${gamesApi}/v1/games/vip-servers/${universeId}`,
	privateServerHelpUrl: (locale: string) =>
		getUrlWithLocale("/info/vip-server", locale),
	getUserSettingsApiUrl: () =>
		`${apiGatewayUrl}/user-settings-api/v1/user-settings`,
