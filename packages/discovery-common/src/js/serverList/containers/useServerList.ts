...
type GameServerPlayer = {
	[K in Exclude<keyof GameServerPlayerResponse, "playerToken">]:
		| GameServerPlayerResponse[K]
		| null;
} & Pick<GameServerPlayerResponse, "playerToken">;

export type GameServer = Omit<
	GameServerResponse,
	"id" | "players" | "playerTokens"
> & {
	id?: string | null;
...
function useServerList(
		...
		async (params: GameInstanceQueryParams = {}, clearServersList = false) => {
			...
			try {
				const {
					data: {
						data: rawInstances,
						nextPageCursor: nextPageCursorData,
						gameJoinRestricted,
					},
				} = await getGameServers(
					placeId,
					clearServersList ? "" : nextPageCursor,
					params,
				);
				...
				await Promise.all(
					instances.map(async (instance) => {
						...
						if (
							...
							try {
								const { data } =
									await serverListService.getVipServer(vipServerId);
				...
				setServers(
					clearServersList ? instances : combinedServers(servers, instances),
				);
				...
				if (params.startTime !== undefined) {
					...
					window.EventTracker?.endSuccess(
	...
	const removeServerAtIndex = useCallback(
		(index: number) => {
			if (isBusy) {
				throw Error(
					"Cannot remove server from list while a request is in flight",
				);
			}
