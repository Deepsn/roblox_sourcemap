...
const { resources, playerTruncationSize, serverListTypes } =
	serverListConstants;
...
function GameInstanceCard({
			...
			.then(
				() => {
					systemFeedbackService.success(
						translate(resources.shutdownServerSuccess),
					);
					onShutdownServerSuccess();
				},
				() => {
					systemFeedbackService.warning(
						translate(resources.shutdownServerError),
					);
				},
			)
			.finally(() => {
	...
	const canShutdown =
		isActiveGameInstance && ((!isVipServer && canManagePlace) || canConfigure);
	...
	const serverIDText =
		translate(resources.serverIdText, { serverId: shortId }) ||
		`ID: ${shortId}`;
			...
			<div className={`card-item card-item-${cssKey}server`}>
				...
				<div
					className={classNames(
						`rbx-${cssKey}game-server-details`,
						"game-server-details",
						{
				...
				>
					...
					{isPrivateServer && (
						<RenewPrivateGame
							{...{
								...
								isPaymentCancelled:
									!subscription?.active && subscription?.price > 0,
					...
					{serverListType === serverListTypes.public.key && (
						<div className="server-id-text text-info xsmall">
							{serverIDText}
						</div>
					)}
				</div>
