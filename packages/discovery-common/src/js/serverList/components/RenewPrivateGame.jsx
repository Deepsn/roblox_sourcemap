...
function RenewPrivateGame({
	...
	const itemPurchaseDataElement = document.getElementById(
		"ItemPurchaseAjaxData",
	);
	const userBalanceFromDataAttribute = itemPurchaseDataElement?.getAttribute(
		"data-user-balance-robux",
	);
	const [userRobuxBalance, setUserRobuxBalance] = useState(
		userBalanceFromDataAttribute ?? null,
	);
				...
				.then(
					...
					({ data }) => {
						if (data.errors?.length > 0) {
							...
							systemFeedbackService.warning(
								translate(resources.renewSubscriptionError) ||
