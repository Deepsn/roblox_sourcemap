...
function FriendsInServerLabel({ translate, players }) {
	...
	const friendsText = useMemo(() => {
		...
		const firstFriendLinkHtmlString = renderToString(
			getLinkifiedFriendName(friends[0]),
		);
		if (friends.length === 1) {
		...
		const secondFriendLinkHtmlString = renderToString(
			getLinkifiedFriendName(friends[1]),
		);
		if (friends.length === 2) {
