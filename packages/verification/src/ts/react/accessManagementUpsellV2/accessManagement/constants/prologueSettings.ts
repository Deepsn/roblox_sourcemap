...
/** v2 TextScreenNode renders plain text; web prologue copy may include `<br />` markup. */
export function prologueDescriptionForTextScreen(html: string): string {
	...
	if (
		...
		switch (true) {
			case featureName === featureConstants.TriggerTrustedFriendVPCRecourse:
				prologueReasonText = translate(
					PrologueConstants.Description.AddFriendVpc,
				);
				break;
...
export function buildVpcPrologueCopy(
	featureName: string,
	translate: TranslateFunction,
	recourseParameters: Record<string, string> | null | undefined,
	expChildModalType?: string,
): { title: string; description: string; confirmLabel: string } {
	return {
		title: getPrologueTranslatedTitle(
			featureName,
			PrologueConstants.Title.AskYourParent,
			translate,
		),
		description: prologueDescriptionForTextScreen(
			getPrologueTranslatedBodyText(
				featureName,
				PrologueConstants.Description.Vpc,
				translate,
		...
		confirmLabel: translate(PrologueConstants.Action.AskNow),
	};
}
