...
function AccessManagementContainer({
	...
	const [featureSpecificParams, setFeatureSpecificParams] = useState<
		TFeatureSpecificData | undefined
	>();
	...
	const [vpcV2PolicyEnabled, setVpcV2PolicyEnabled] = useState<
		boolean | undefined
	>(undefined);
	// Request types rolled back to the v1 wizard even when the v2 policy is enabled.
	// Undefined while GUAC is loading, so an excluded request type does not flash the v2 path.
	...
	async function onAccessManagementCustomEvent(
		...
		vpcHandoffRequest.current = {
			isAsyncCall,
			requestDetails: (ampRecourseData as Record<string, string> | null) ?? {},
			closeCallback,
			usePrologue,
		};
		if (usePrologue) {
	...
	useEffect(() => {
		const handleEvent =
			onAccessManagementCustomEvent as unknown as EventListener;
	...
	useEffect(() => {
		if (!isVpcHandoffCandidate || vpcExcludedRequestTypes !== undefined) {
			return;
		}
		vpcV2Handoff
			.getExcludedRequestTypes()
			.then(setVpcExcludedRequestTypes, () => setVpcExcludedRequestTypes([]));
	}, [isVpcHandoffCandidate, vpcV2Handoff, vpcExcludedRequestTypes]);
				...
				case Recourse.ParentLinkRequest: {
					return (
						<ParentalRequestContainer
							...
							isPrologueUsed={isPrologueUsed ?? false}
