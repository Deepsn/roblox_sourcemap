...
export const fetchFeatureAccess = createAsyncThunk(
	...
	async (
		...
		try {
			const { featureName, ampFeatureCheckData, successfulAction, namespace } =
				arg;
...
export const accessManagementSlice = createSlice({
	...
	reducers: {
		...
		setAmpFeatureCheckData: (
			state,
			action: PayloadAction<ExtraParameter[]>,
		) => {
		...
		setVerificationStageRecourse: (
			state,
			action: PayloadAction<RecourseResponse>,
		) => {
		...
		builder
			...
			.addCase(fetchFeatureAccess.fulfilled, (state, action) => {
				...
				if (ampResponse.access === Access.Actionable) {
					...
					state.verificationStageRecourse = ampResponse.recourses[0] ?? null;
				}
			})
...
export const selectAccessManagement = (state: RootState) =>
	state.accessManagement;
export const selectShowUpsell = (state: RootState) =>
	state.accessManagement.showUpsell;
export const selectFeatureName = (state: RootState) =>
	state.accessManagement.featureName;
export const selectAmpFeatureCheckData = (state: RootState) =>
	state.accessManagement.ampFeatureCheckData;
export const selectFeatureAccess = (state: RootState) =>
	state.accessManagement.featureAccess;
export const selectCurrentStage = (state: RootState) =>
	state.accessManagement.stage;
export const selectVerificationStageRecourse = (state: RootState) =>
	state.accessManagement.verificationStageRecourse;
export const selectPrologueStatus = (state: RootState) =>
	state.accessManagement.prologueUsed;
export const selectNamespace = (state: RootState) =>
	state.accessManagement.namespace;
