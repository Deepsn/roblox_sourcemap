...
const ParentalRequestContainer = ({
	...
	onHidecallback: () => void;
	onOutcome?: (didSend: boolean) => void;
	...
	const reportOutcome = (didSend: boolean) => {
		if (outcomeReportedRef.current || !onOutcome) {
			return;
		}
	...
	const handleDismissWithoutSend = () => {
		reportOutcome(false);
		onHidecallback();
	};
	...
	const multipleParentSuccessMessage = (
		<p>{translate(emailSentConfirmation.pluralParentBody)}</p>
	);
	const emailNotSentSuccessMessage = (
		<p>{translate(emailSentConfirmation.emailNotSentBody)}</p>
	);
	const confirmationMessage = useMemo(() => {
		if (!isEmailSent) {
			return emailNotSentSuccessMessage;
		}
		return parentEmail.length === 1
			? oneParentSuccessMessage
			: multipleParentSuccessMessage;
		...
		<div>
			{recourseAction === Recourse.ParentConsentRequest && (
				<ParentalRequestBroadcast
					...
					onHideCallback={handleDismissWithoutSend}
					value={value}
				/>
			)}
...
export default withTranslations(
	...
	parentalRequestTranslationConfig,
);
