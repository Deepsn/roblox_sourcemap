...
// Fields are populated once a vendor session starts; null-initialized here.
const VendorVerificationDataInitialState = {
	...
	qrCode: null,
	loading: false,
} as unknown as VendorVerificationData;
...
} as unknown as IDVerificationState;
...
const initialState = {
	...
	loading: null,
} as unknown as VerificationState;

export const selectIDVState = (state: RootState) =>
	state.verification.IDVerificationState;
...
export const startIDVerification = createAsyncThunk(
	...
	async (
		...
		try {
			...
			const recourseToReport = ageEstimation
...
export const fetchIDVerificationStatus = createAsyncThunk(
	...
	async (token: string, thunkAPI) => {
		try {
			...
			const recourseToReport = isAgeEstimation
				? Recourse.AgeEstimation
				: Recourse.GovernmentId;
			reportEvent(ReportEvent.VerificationFailed, recourseToReport, {
				error: JSON.stringify(error),
...
export const verificationSlice = createSlice({
	...
	reducers: {
		...
		setIDVerificationState: (
			state,
			action: PayloadAction<IDVerificationState>,
		) => {
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(startIDVerification.fulfilled, (state, action) => {
				...
				let completionPageState: PageText | null = null;
				...
				state.IDVerificationState = {
					...state.IDVerificationState,
					vendorVerificationData,
					completionPageState: completionPageState as PageText,
					page,
					loading: false,
				...
				if (isAgeEstimation) {
					...
					switch (vendorData.sessionStatus) {
						...
						case VerificationStatusCode.Stored:
							page = IDVPage.Complete;
							completionPageState = getPageStateConstants(
								VerificationViewState.SUCCESS_GENERIC,
							);
							break;
						case VerificationStatusCode.Started:
							...
							reportEvent(
								...
								{
									session:
										state.IDVerificationState.vendorVerificationData
											.sessionIdentifier,
								},
							);
							break;
						case VerificationStatusCode.Submitted:
							reportEvent(
								ReportEvent.verificationInProgress,
								Recourse.GovernmentId,
								{
									session:
										state.IDVerificationState.vendorVerificationData
