...
export type GETRequestConfig<C extends string = string> =
	CommonRequestConfig<C> & {
		method: "GET";
	};

export type POSTRequestConfig<C extends string = string> =
	CommonRequestConfig<C> & {
		method: "POST";
		data: object;
	};

type RequestData<C extends string = string> =
	| GETRequestConfig<C>
	| POSTRequestConfig<C>;
...
function statusCodeFromError(error: unknown): string {
	if (isAxiosError(error)) {
		return (
			error.code ?? error?.response?.status.toString() ?? "UnknownAxiosError"
		);
	}
	return "UnknownError";
}
