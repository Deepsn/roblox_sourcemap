...
const getServerTiming = (
	...
	const requestStartMs = entries.find(
		(entry) => entry.name === "rbxReqStart",
	)?.duration;
	const requestEndMs = entries.find(
		(entry) => entry.name === "rbxReqEnd",
	)?.duration;
...
function getNavTimingMilestones({
	...
	return NAV_TIMING_TO_SPEC.reduce<BridgeMilestone[]>(
		(metrics, [field, name, phases]) => {
			const value = navTiming[field] as number;

			const subPhaseValues = phases?.reduce<BridgeSubphase[]>(
				(subphases, subphase) => {
					...
					return [
						...subphases,
						{ name: subphase, deltaMs: deltaMs(subphaseValue) },
					];
				},
				[],
			);
			...
			if (name === "PrimaryDataReady" && serverTiming && subPhaseValues) {
				subPhaseValues.push(
					{
						name: "serverRequestStart",
						deltaMs: serverDeltaMs(serverTiming.requestStartMs),
					},
					{
						name: "serverRequestEnd",
						deltaMs: serverDeltaMs(serverTiming.requestEndMs),
					},
				);
			...
			return [
				...metrics,
				{ name, id, deltaMs: deltaFromStart, subphases: subPhaseValues },
			];
		},
		[],
	);
