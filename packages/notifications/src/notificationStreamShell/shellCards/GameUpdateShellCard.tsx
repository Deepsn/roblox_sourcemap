...
export const GameUpdateShellCard = ({
	...
	const primary =
		resolved[0] ?? placeholderModel(universes[0]?.UniverseId ?? 0);
	...
	const viewModel: GameUpdateViewModel = {
		...
		eventDate:
			primary.createdOn != null
				? new Date(primary.createdOn).toISOString()
				: eventDate,
		isInteracted,
		canLaunch,
	...
	const sendEvent = (eventName: string): void =>
		sendGameUpdateEvent(eventName, eventParams());
	...
	const followMutation = useMutation<unknown, unknown, boolean>({
		mutationFn: (following: boolean) => {
			...
			return following
				? httpService.post(config, {})
				: httpService.delete(config, {});
		},
		onSuccess: (_data, following) => {
			...
			if (!primary.updateMessage) {
				queryClient
					...
					.catch((error: unknown) =>
						reportNotificationStreamError("gameUpdateRefetch", error),
					);
			}
		},
		onError: (error: unknown) =>
			reportNotificationStreamError("gameUpdateFollow", error),
	});
	...
	const goToDetails = (): void => {
		...
		if (primary.rootPlaceId) {
			window.location.href = gameDetailsHref(
				primary.rootPlaceId,
				primary.createdOn,
			);
		}
		...
		<GameUpdateNotification
			...
			handlers={{
				...
				onReport: () => {
					...
					openAbuseReport(
						abuseReportHref(primary.universeId, window.location.href),
					);
				},
				...
				onMetaActionsOpenChange: (open: boolean) =>
					sendEvent(
						open ? streamEvents.openMetaActions : streamEvents.closeMetaActions,
					),
			}}
		/>
	);
