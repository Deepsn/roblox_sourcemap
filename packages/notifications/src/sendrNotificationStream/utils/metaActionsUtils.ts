...
const flattenAllMetaActions = (
	metaActions: VisualItemMetaAction[],
): Action[] => {
	const actions = metaActions.reduce<Action[]>(
		(actionsList, currentMetaAction) => {
...
const getIdempotenceKeyFromState = (
	currentState: PageState | undefined,
): string => {
	if (currentState?.visualItems.metaAction?.length) {
		const allAction = flattenAllMetaActions(
			currentState.visualItems.metaAction,
		);
