...
const getFirstDeepLinkNavigationPath = (
...
const shouldRetryDeeplink = (action: Action): boolean => {
	const firstDeepLinkNavigationPath = getFirstDeepLinkNavigationPath(
		action.path,
	);
...
export const SendrNotification = ({
	...
	const [showDeeplinkFailureModal, setShowDeeplinkFailureModal] =
		useState(false);
	...
	const handleActions = async (visualItem: InteractibleVisualItem) => {
		...
		for (const action of actions) {
			...
			let isSuccess = true;
			const senderId =
				notificationData.content?.clientEventsPayload?.sender_userid;
	...
	return (
		<div
			...
			className={`sendr-notification-background ${
				dismissed
					? "sendr-notification-dismissed"
					: "sendr-notification-visible"
			}`}
		>
			<DeeplinkFailModal
