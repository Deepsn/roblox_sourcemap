...
const ReportNotificationModalContent: FC<ReportNotificationModalProps> = ({
	handleClose,
}) => {
	const translate = useNotificationLocalization();
	const { displayState: notificationProps, notificationData } =
		useSelectedNotification();
		...
		() => [
			{
				...
				action: () => {
					sendEventStreamEvent(
						NOTIFICATION_EVENTS.ReportNotificationCancelled,
						"click",
						{
							sendrVersion:
								notificationData?.content.minVersion?.toString() ?? "",
	...
	const generateIdempotenceKey = useCallback(async () => {
		...
		const keyWithDate = `${key}${new Date().toDateString()}`
			.split(" ")
			.join("");
	...
	const reportedNotification = useMemo(
		() =>
			notificationProps ? (
				<NotificationView {...notificationProps} isReadOnly />
			) : null,
		[notificationProps],
	);
	...
	const onReportSubmit = useCallback(
		async (data: AbuseReportFormData) => {
			sendEventStreamEvent(
				NOTIFICATION_EVENTS.ReportNotificationSubmit,
				"click",
				{
					sendrVersion: notificationData?.content.minVersion?.toString() ?? "",
					notifType: notificationData?.content.notificationType ?? "",
			...
			try {
				...
				sendEventStreamEvent(
					NOTIFICATION_EVENTS.ReportNotificationSubmitSuccess,
					"click",
					{
						sendrVersion:
							notificationData?.content.minVersion?.toString() ?? "",
				...
				sendEventStreamEvent(
					NOTIFICATION_EVENTS.ReportNotificationSubmitFailed,
					"click",
					{
						sendrVersion:
							notificationData?.content.minVersion?.toString() ?? "",
				...
				return (
					<React.Fragment>
						<DialogBody className="flex flex-col gap-y-small">
							...
							<div className="reported-notification">
								{reportedNotification}
							</div>
							{body}
						</DialogBody>
						{apiErrorMsg ? (
							<div className="text-error notification-api-error">
				...
				<React.Fragment>
					...
					{apiErrorMsg ? (
						<Grid
							item
							xs={12}
							classes={{ root: "text-error notification-api-error" }}
						>
							{translate(apiErrorMsg)}
						</Grid>
					) : null}
