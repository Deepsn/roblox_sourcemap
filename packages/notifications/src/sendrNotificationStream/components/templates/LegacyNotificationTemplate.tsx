...
export const LegacyNotificationTemplate = ({
	...
	const textBodyItem: VisualItemText | null =
		visualItems[VisualItemType.TextBody]?.[0] || null;
	let eventTimeString;
	const hasMetaActions = visualItems[VisualItemType.MetaAction]?.[0];
	const textButtonItems: Array<VisualItemButton> =
	...
	const handleBaseInteraction = (e: React.MouseEvent<HTMLButtonElement>) => {
		if (
			textBodyItem?.actions &&
			textBodyItem.actions.length > 0 &&
			handleActions
		) {
	...
	return (
		<div
			className={`legacy-notif-base ${isMetaActionsOverlay ? "meta-actions-overlay" : ""}`}
		>
			<button
			...
			<div className="notif-content-container">
				...
				<div className="small text text-content">
					{textBodyItem?.title && (
						<span className="notif-title-text text-emphasis">
							{formatText(textBodyItem.title)}
						</span>
					)}
				...
				{hasMetaActions && !isReadOnly && (
					...
					>
						<span
							className={
								isMetaActionsOverlay ? "icon-close" : "icon-more-gray-vertical"
							}
						/>
					</button>
				)}
