...
export const MetaActionsListBase = ({
	...
	for (
		let metaActionsIndex = 0;
		metaActionsIndex < metaActions.length;
		metaActionsIndex++
	) {
		const item = metaActions[metaActionsIndex]!;
		const { actionIcon } = item;
		const iconName = isMetaActionIcon(actionIcon)
			? metaActionIconMap[actionIcon]
			: undefined;
		...
		listElements.push(
			...
			>
				{iconName ? (
					<React.Fragment>
						<Icon name={iconName} />
						<span className="meta-action-button-text">
	...
	return (
		...
		>
			...
			/>
			<div className="meta-actions-overlay meta-action-button-group">
				{listElements}
			</div>
		</div>
	);
