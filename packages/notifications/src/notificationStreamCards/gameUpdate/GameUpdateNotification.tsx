...
const GameUpdateThumbnail = ({
	...
	return (
		<a
			...
			href={
				viewModel.rootPlaceId ? `/games/${viewModel.rootPlaceId}` : undefined
			}
...
export const GameUpdateNotification = ({
	...
	if (viewModel.state === "unfollowed") {
		return (
			<Notification
				...
				style={cardStyle}
				title={undefined}
				description={translate("Message.UnfollowedGame", {
	...
	if (viewModel.state === "aggregated") {
		...
		const description = aggregatedText
			.split(AGG_TOKEN_SPLIT)
			.map((part, index) => {
		...
		return (
			<Notification
				...
				style={cardStyle}
				media={
					<GameUpdateThumbnail viewModel={viewModel} onClick={openDrilldown} />
				}
				title={undefined}
			...
			description={
				<React.Fragment>
					<a
						...
						href={
							viewModel.rootPlaceId
								? `/games/${viewModel.rootPlaceId}`
								: undefined
						}
						title={viewModel.fullGameName ?? viewModel.gameName}
						onClick={(e) => {
					...
					{showNotPlayable && (
						<span
							className="text-caption-body not-playable-message"
							style={{ display: "block" }}
						>
