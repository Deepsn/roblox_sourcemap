...
export const markGameUpdateInteracted = (
	...
	httpService
		...
		.catch((error: unknown) =>
			reportNotificationStreamError("gameUpdateInteracted", error),
		);
};
...
const fetchGameUpdate = batchQuery<
	number,
	LatestGameUpdate[],
	LatestGameUpdate | undefined
>(
	...
	(universeIds) =>
		httpService
			...
			.then(({ data }) => data ?? []),
	(updates, universeId) =>
		updates.find((update) => update.universeId === universeId),
);

const fetchGameUpdates = async (
	universeIds: number[],
): Promise<LatestGameUpdate[]> => {
...
const fetchFollowedUniverseIds = async (userId: number): Promise<number[]> => {
	const { data } = await httpService.get<
		GameFollowing[] | { data: GameFollowing[] }
...
const fetchPlaceDetail = batchQuery<
	number,
	PlaceDetail[],
