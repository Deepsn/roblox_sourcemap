...
const { notificationApi, gamesApi, followingsApi, websiteUrl } =
	environmentUrls;
...
export const getLatestGameUpdatesUrlConfig = (
	universeIds: number[],
): UrlConfig => ({
...
export const multiGetPlaceDetailsUrlConfig = (
	placeIds: number[],
): UrlConfig => ({
...
export type GameUpdateNsPage =
	(typeof GAME_UPDATE_NS_PAGES)[keyof typeof GAME_UPDATE_NS_PAGES];
...
export const gameUpdateInteractedUrlConfig: {
	url: string;
...
export const followUniverseUrlConfig = (
	userId: number,
	universeId: number,
): UrlConfig => ({
...
export const openAbuseReport = (url: string): void => {
	...
	const globals =
		(window as unknown as { Roblox?: AbuseReportGlobals }).Roblox ?? {};
