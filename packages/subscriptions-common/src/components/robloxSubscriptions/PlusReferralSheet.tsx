 ...
 */
const buildPlusReferralReturnUrl = (
	code: string,
...
const PlusReferralSheetBody: FC<PlusReferralSheetBodyProps> = ({
	...
	const { handle: referrerHandle, isLoading: isReferrerLoading } =
		useReferrerHandle(isPitch ? referrerUserId : undefined);

	const joinHeading = translate(
		"Heading.ReferralRecipientJoin",
		undefined,
		"Join Plus, get",
	);
	const joinPlusLabel = translate(
		"Action.ReferralJoinPlus",
		undefined,
		"Join Roblox Plus",
	);
				...
				{face === "pitch" ? (
					...
					<SheetBody className="gap-y-large padding-top-large padding-bottom-medium medium:padding-top-xlarge medium:padding-bottom-large flex flex-col items-center text-center">
						...
						<p className="text-body-small medium:text-body-medium content-emphasis margin-none">
							{translate(
								UNREDEEMABLE_COPY[face].key,
								undefined,
								UNREDEEMABLE_COPY[face].fallback,
							)}
						</p>
					</SheetBody>
				)}
...
const PlusReferralSheet: FC<PlusReferralSheetProps> = ({
	...
	const resolvedSubscribePeriodType =
		subscribePrice !== undefined
			? subscribePeriodType
			: lookedUpSubscribePeriodType;
	const resolvedSubscribeFeatureConfig =
		subscribeFeatureConfig ?? lookedUpSubscribeFeatureConfig;
