...
import {
	CompactBenefitList,
	LargeScreenBenefitList,
} from "./SubscriptionBenefitLists";
...
const RobloxSubscriptionWidget: FC<RobloxSubscriptionWidgetProps> = ({
	...
	useEffect(() => {
		...
		if (!apiProductType) {
			trackCounter(
				"Error_RobloxSubscriptionWidget_InvalidSubscriptionProductType",
			);
		}
	...
	const offers = Array.isArray(primaryProduct.offers)
		? primaryProduct.offers
		: [];
	const hasFreeTrial = offers.some((o) => o.freeTrial != null);

	const productBenefits = Array.isArray(primaryProduct.benefits)
		? primaryProduct.benefits
		: [];
