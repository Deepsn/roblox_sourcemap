...
const BENEFIT_FIELD_TRANSFORMS: Record<string, FieldTransform> = {
	...
	periodType: {
		mapValue: (v) => PERIOD_TYPE_TO_PLURAL[String(v)] ?? String(v),
	},
};
	...
	for (const [key, value] of Object.entries(benefit)) {
		...
		if (typeof value === "object") {
			for (const [field, fieldValue] of Object.entries(
				...
				const paramValue = transform?.mapValue
					? transform.mapValue(fieldValue)
					: fieldValue;
...
export function CompactBenefitList({
	benefits,
	translate,
}: CompactBenefitListProps) {
	const compactBenefits = benefits
		.filter((b) => isCompactBenefitIcon(b.iconName))
		.sort(
			(a, b) =>
				compactBenefitIconSortKey(a.iconName) -
				compactBenefitIconSortKey(b.iconName),
		);
		...
		<Fragment>
			{compactBenefits.map((b, index) => (
				<BenefitItem
					...
					label={translate(
						b.benefitTranslationKey,
						getBenefitTranslationParams(b),
					)}
				/>
			))}
		</Fragment>
...
export function LargeScreenBenefitList({
	benefits,
	translate,
}: LargeScreenBenefitListProps) {
	...
	const sortedBenefits = benefits.toSorted(
		(a, b) =>
			largeBenefitIconSortKey(a.iconName) - largeBenefitIconSortKey(b.iconName),
	);
