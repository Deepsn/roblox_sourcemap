...
export const GameBadgesList = ({
	...
	const fetchGameBadges = useCallback(
		async (
			...
			if (
				isInitial ||
				(numberToShowRef.current >= badgesLengthRef.current && cursor)
			) {
				try {
					const { nextPageCursor, data } =
						await gameBadgesService.getGameBadges(
							badgeUniverseId,
							cursor,
							limit,
						);
	...
	return (
		<div className="stack badge-container game-badges-list">
			<div className="container-header">
				<h3 data-testid="game-badges-heading">
					{translate(translationMap.HeadingGameBadges)}
				</h3>
			</div>
