...
(function (window, undefined) {
	...
	function deepGet(object, property) {
		...
		for (
			...
			parts.length > 0;
			object = object[property], property = parts.shift()
		) {
	...
	function getResourceUrl(name) {
		...
		return (
			config.paths[name] ||
			config.baseUrl + config.modulePath + "/" + name.replace(".", "/") + ".js"
		);
	}
	...
	function loadResource(nameOrUrl, onload) {
		if (
			!isString(nameOrUrl) ||
			config.externalResources.toString().indexOf(nameOrUrl) >= 0
		)
			return onload();
		var name = getResourceName(nameOrUrl);
		if (resourceMap[name] === undefined) {
			resourceMap[name] = { loaded: false, depsLoaded: true, listeners: [] };
	...
	return {
		...
		realTimeNotifications: {
			...
			presenceBulkNotifications: {
				...
				types: { presenceChanged: "PresenceChanged" },
			},
		},
		presenceTypes: { offline: 0, online: 1, inGame: 2, inStudio: 3 },
		keyCodes: { enter: 13 },
		events: {
...
if (typeof Roblox.EventStream === "undefined") {
	Roblox.EventStream = (function () {
		var targetTypes = { DEFAULT: 0, WWW: 1, STUDIO: 2, DIAGNOSTIC: 3 };
		...
		function sendEventWithTarget(
			...
			if (serviceWorkerMode && targetType !== targetTypes.DEFAULT) {
				throw (
					"TargetType '" +
					targetType +
					"' is not supported in Service Worker mode"
				);
			}
			if (!eventName || !context || baseUrl === "") {
				return;
			}
			var defaultParams = getEventParameters(
				eventName,
				context,
				additionalProperties,
			);
...
if (typeof Roblox.UserInteractionsEvent === "undefined") {
	Roblox.UserInteractionsEvent = (function () {
		...
		var fireEventStreamEvent = function (context) {
			if (
				!Roblox.EventStream ||
				!Roblox.EventStream.isInitialized ||
				!Roblox.EventStream.isInitialized()
			) {
				return false;
			}
...
Roblox.UserService = (function () {
	...
	function getUserData() {
		...
		Roblox.CurrentUser.hasVerifiedBadge =
			metaTag.dataset.hasverifiedbadge === "true";
	}
	function init() {
		if (metaTag) {
			getUserData();
...
Roblox.MetaDataValues = (function () {
	...
	function getPageName() {
		return (
			metaTagPageName &&
			metaTagPageName.dataset &&
			metaTagPageName.dataset.internalPageName
		);
	}
...
Roblox.Utilities.ExponentialBackoff = function (
	regularBackoffSpecification,
	fastBackoffPredicate,
	fastBackoffSpecification,
) {
	var self = this;
	var attemptCount = 0;
	...
	function getDelay() {
		...
		if (attemptCount === 0) {
			...
			return (
				rawDelay + getRandomness(rawDelay, spec.FirstAttemptRandomnessFactor())
			);
		} else {
			...
			return (
				rawDelay +
				getRandomness(rawDelay, spec.SubsequentDelayRandomnessFactor())
			);
		}
	}
...
Roblox.Utilities.ExponentialBackoffSpecification = function (options) {
	options = options || {};
	var firstAttemptDelay =
		typeof options.firstAttemptDelay === "number"
			? options.firstAttemptDelay
			: 5000;
	var firstAttemptRandomnessFactor =
		typeof options.firstAttemptRandomnessFactor === "number"
			? options.firstAttemptRandomnessFactor
			: 0.5;
	var subsequentDelayBase =
		typeof options.subsequentDelayBase === "number"
			? options.subsequentDelayBase
			: firstAttemptDelay * 2;
	var subsequentDelayRandomnessFactor =
		typeof options.subsequentDelayRandomnessFactor === "number"
			? options.subsequentDelayRandomnessFactor
			: firstAttemptRandomnessFactor;
	var maximumDelayBase =
		typeof options.maximumDelayBase === "number"
			? options.maximumDelayBase
			: 5 * 60 * 1000;
...
Roblox &&
	...
	Roblox.BundleDetector.bundleDetected("headerinit");
