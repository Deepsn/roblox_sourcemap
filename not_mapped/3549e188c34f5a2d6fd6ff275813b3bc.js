...
+(function (t) {
	...
	})(jQuery); // leancore/BootstrapNamespacing.js

$.fn.bootstrapModal = $.fn.modal; // leancore/libs/underscore-min.js
...
}).call(this); // GoogleAnalytics/GoogleAnalyticsEvents.js
	...
	FireEvent: function (args) {
		if (window._gaq) {
			if (!window.GoogleAnalyticsDisableRoblox2) {
				...
				GoogleAnalyticsEvents.LocalEventLog.push(
					makeGoogleAnalyticsLogObject(eventsArray),
				);
			}
			...
			GoogleAnalyticsEvents.LocalEventLog.push(
				makeGoogleAnalyticsLogObject(eventsArrayB),
			);
		}
	},
	...
	TrackTransaction: function (orderId, priceTotal) {
		if (window._gaq) {
			...
			if (!window.GoogleAnalyticsDisableRoblox2) {
				...
				GoogleAnalyticsEvents.LocalEventLog.push(
					makeGoogleAnalyticsLogObject(transArray),
				);
			}
	...
	TrackTransactionItem: function (orderId, sku, name, category, price) {
		if (window._gaq) {
			...
			if (!window.GoogleAnalyticsDisableRoblox2) {
				_gaq.push(itemArray);
				GoogleAnalyticsEvents.LocalEventLog.push(
					makeGoogleAnalyticsLogObject(itemArray),
				);
				_gaq.push(trackTransArray);
				GoogleAnalyticsEvents.LocalEventLog.push(
					makeGoogleAnalyticsLogObject(trackTransArray),
				);
			}
...
GoogleListener.handleEvent = function (event, data) {
	...
	switch (event.type) {
		...
		default:
			console.log(
				"GoogleListener - Event registered without handling instructions: " +
					event.type,
			);
			return false;
	}
...
Roblox.SubmitButton = (function () {
	...
	function init() {
		$(buttonCssSelector).each(function (idx, element) {
			...
			var clickCallback = getClickHandler(
				button,
				callbackObject,
				canMultiClick,
			);
	...
	function getToggleButtonHandler(button, clickCallback) {
		return function (event, enabled) {
			if (enabled) {
				...
				button
					.addClass(buttonEnabledClass)
					.removeClass(buttonDisabledClass)
					.removeClass(disabledClass);
				button.on("click", clickCallback);
			} else {
				button
					.addClass(buttonDisabledClass)
					.addClass(disabledClass)
					.removeClass(buttonEnabledClass);
				button.data(clickableCssSelector, "false");
				button.off("click");
			}
		};
	}
	return { init: init, submitToggleEvent: submitToggleEvent };
})();
$(function () {
...
if (typeof Roblox.DeveloperConsoleWarning === "undefined") {
	Roblox.DeveloperConsoleWarning = (function () {
		...
		return { showWarning: showWarning };
	})();
...
Roblox.BootstrapWidgets = (function () {
	...
	function SetupAccordion() {
		$('[data-toggle="collapsible-element"]').on(
			"show.bs.collapse",
			function (e) {
	...
	function ToggleSystemMessage(
		...
		if (typeof alertElm !== "undefined") {
			...
			timeoutSlideDown =
				typeof timeoutSlideDown === "undefined" ? 200 : timeoutSlideDown;
			timeoutSlideUp =
				typeof timeoutSlideUp === "undefined" ? 3000 : timeoutSlideUp;
			setTimeout(function () {
				if (clone) {
					clone.addClass("on");
				} else {
	...
	function TruncateParagraph(lineHeight, numberOfLines) {
		...
		paraOverflowToggleElement.each(function () {
			...
			if (clone.height() <= maxHeight || clone.height() <= elem.height()) {
				elem
					.removeClass(paraOverflowToggleSelector)
		...
		var bindToggleClick = function () {
			$(this).bind("click touchstart", function () {
				...
				if ($(this).text() === moreTitle) {
					paraOverflowToggleElement
						.removeClass(paraHeightSelector)
						.addClass(paraOverflowToggleOffSelector);
					$(this).text(lessTitle);
				} else {
					paraOverflowToggleElement
						.removeClass(paraOverflowToggleOffSelector)
	...
	function TruncateContent() {
		...
		contentOverflowToggleElement.each(function () {
			...
			var containerToggleElement = $(
				".toggle-content[data-container-id='" + containerId + "']",
			);
			var showMoreGradientEnd = $(
				".show-more-end[data-container-id='" + containerId + "']",
			);
			showMoreGradientEnd.removeClass("hide");
			if (
				clone.height() <= elem.height() ||
				!containerToggleElement.is(":visible")
			) {
				elem
					.removeClass(contentOverflowToggleSelector)
	...
	function ToggleContent() {
		...
		var bindToggleClick = function () {
			...
			$(this).bind("click", function () {
				...
				var contentOverflowToggleElement = $(
					"#" + contentOverflowToggleElementId,
				);
				if ($(this).text() === moreTitle) {
					contentOverflowToggleElement
						.removeClass(contentHeightSelector)
					...
					contentOverflowToggleElement
						.removeClass(contentOverflowToggleOffSelector)
	...
	function SetupCarousel(carouselId) {
		...
		$(carouselId).carousel({ interval: 6000, pause: "hover" });
	}
	...
	function SetupVerticalMenu() {
		...
		var hoverOverSublist = dropdown.find(
			"li " + submenuContainerSelector + "[hover=true]",
		);
		...
		listItems.on("mouseover touchstart", function () {
			...
			if (delay !== "never" && (dir === 1 || delay === "always")) {
				window.setTimeout(function () {
					...
					var currentlyActiveMenu = dropdown.find(
						"li[hover=true] " + submenuContainerSelector,
					);
					subLists.hide();
...
Roblox.CookieUpgrader.upgrade = function (name, options) {
	...
	try {
		...
		if (cookieValue != null) {
			...
			var deletedCookieData = { cookieName: name, cookieValue: cookieValue };
		...
		cookie +=
			"expires=" + options.expires(originalCookieValue).toUTCString() + "; ";
		...
		if (typeof GoogleAnalyticsEvents != "undefined") {
			GoogleAnalyticsEvents.FireEvent([
				"CookieUpgrader",
				"ExceptionDuringConvertOf" + name,
				exp.message,
			]);
		}
	}
};
Roblox.CookieUpgrader.getExpirationFromCookieValue =
	function getExpirationFromCookieValue(propertyName, originalCookieValue) {
		var acquisitionTimeRegex = new RegExp(
			propertyName + "=(\\d+)\/(\\d+)\/(\\d+)",
		);
		var originalCookieTimeMatches =
			originalCookieValue.match(acquisitionTimeRegex);
		var startTime = new Date().getTime();
		if (
			originalCookieTimeMatches != null &&
			originalCookieTimeMatches.length != 0
		) {
			var originalCookieTime = new Date(
				originalCookieTimeMatches[3],
				originalCookieTimeMatches[1] - 1,
				originalCookieTimeMatches[2],
			);
...
Roblox.Lang["ControlsResources"] = Roblox.Lang["CommonUI.Controls"]; // Thumbnails/thumbnailMetrics.js
var Roblox = Roblox || {};
Roblox.ThumbnailMetrics = (function () {
	var metricsBaseUrl =
			(Roblox.EnvironmentUrls && Roblox.EnvironmentUrls.metricsApi) ||
		...
		loadStates = { complete: "complete", timeout: "timeout" },
		initDone = false,
		queue = [],
		logProbability = 0.0;
	...
	function logThumbnailTimeout(thumbnailType) {
		...
		$.ajax({
			...
			crossDomain: true,
			xhrFields: { withCredentials: true },
			data: { loadState: loadStates.timeout, thumbnailType: thumbnailType },
		});
	}
	init();
	return {
...
function fitStringToWidthSafeText(str, width, className) {
	var safeName = fitStringToWidthSafe(str, width, className).replace(
		"&hellip;",
		"...",
	);
	return safeName;
...
$(function () {
	...
	var pauseBetweenRequests = imageRetryDataElement
		? imageRetryDataElement.data("image-retry-timer")
		: 1500;
	var retryMaxTimes = imageRetryDataElement
		? imageRetryDataElement.data("image-retry-max-times")
		: 10;
	var gaLoggingPercent = imageRetryDataElement
		? imageRetryDataElement.data("ga-logging-percent")
		: 0;
	var GoogleAnalyticsEvents_FireEvent =
		(window.GoogleAnalyticsEvents && GoogleAnalyticsEvents.FireEvent) ||
		function () {};
	...
	function getImgWithSrc(options) {
		...
		var imgWithSrc =
			img.length === 1 ? img : options.el.find("img.original-image");
		return imgWithSrc;
	}
	...
	function logSuccess(options) {
		...
		Roblox.ThumbnailMetrics &&
			Roblox.ThumbnailMetrics.logFinalThumbnailTime(gap);
	...
	$.fn.loadRobloxThumbnails = function () {
		return this.each(function () {
			...
			imgWithSrc.one("load", function () {
				...
				Roblox.ThumbnailMetrics &&
					Roblox.ThumbnailMetrics.logFinalThumbnailTime(
						imgLoadTime,
						"2dThumbnailOnLoad",
					);
			});
...
Roblox &&
	...
	Roblox.BundleDetector.bundleDetected("leanbase");
