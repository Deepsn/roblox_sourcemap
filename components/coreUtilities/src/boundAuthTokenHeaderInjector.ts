...
try {
	...
	if (hba.boundAuthTokenWhitelist) {
		...
		const batWhitelistWrapper = JSON.parse(
			hba.boundAuthTokenWhitelist,
		) as BatWhitelistWrapper;
		...
		for (const apiSampling of batWhitelist) {
			if (apiSampling.apiSite === JQUERY_PSEUDO_API_SITE_KEY) {
				...
				jQueryRolloutPerMillion = Math.max(
					0,
					parseInt(apiSampling.sampleRate, 10) || 0,
				);
				break;
			}
		}
...
const initializeBoundAuthTokensForJQuery = (): void => {
	$.ajaxPrefilter((options: JQueryAjaxSettings) => {
		...
		const originalXhr = (options.xhr ??
			$.ajaxSettings.xhr) as () => XMLHttpRequest;
