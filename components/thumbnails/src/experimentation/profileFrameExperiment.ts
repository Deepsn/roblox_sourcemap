...
export const PROFILE_FRAME_EXPERIMENT_KEY =
	"friendsCarouselProfileFrameEnabled";
...
const resolveTreatment = (): Promise<boolean> => {
	...
	const pending = (async () => {
		const layerValues =
			await ExperimentationService.getAllValuesForLayer(PROFILE_FRAME_LAYER);
...
export const useProfileFrameExperiment = (): boolean => {
	const [isInTreatment, setIsInTreatment] = useState(
		isProfileFrameInTreatmentFromCache,
	);
