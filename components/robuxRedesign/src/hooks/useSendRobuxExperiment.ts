...
const isControlVariant = (variant: unknown): boolean =>
	variant === 0 || variant === "0";
const isTreatmentVariant = (variant: unknown): boolean =>
	variant === 1 || variant === "1";

/**
 * Fetches the Send Robux experiment assignment without logging exposure.
 */
const getSendRobuxExperimentAssignment =
	async (): Promise<SendRobuxExperimentAssignment> => {
		try {
			const service = await waitForExperimentationService();
			const values = await service.getAllValuesForLayer(
				SEND_ROBUX_EXPERIMENT_LAYER,
			);
			...
			trackCounter("SendRobuxExperimentEvaluated", {
 ...
 */
export function useSendRobuxExperiment(
	enabled = true,
): SendRobuxExperimentState {
