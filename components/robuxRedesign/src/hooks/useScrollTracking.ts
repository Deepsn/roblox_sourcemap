...
const TRACKING_SELECTOR = TRACKING_IDENTITY_ATTRS.map(
	(attr) => `[${attr}]`,
).join(", ");
	...
	const trackingHandlers = useMemo(
		(): { attr: string; record: (target: Element) => boolean }[] => [
			...
			},
			{
				...
				record: (target) => {
					const subscriptionProductId = target.getAttribute(
						SUBSCRIPTION_PRODUCT_ID_ATTR,
					);
					if (
						!subscriptionProductId ||
					...
					const subscriptionProductType =
						target.getAttribute(SUBSCRIPTION_PRODUCT_TYPE_ATTR) ?? "";
					seenSubscriptionProductsRef.current.set(
						subscriptionProductId,
						subscriptionProductType,
					);
					return true;
