...
import {
	isCardProviderPayload,
	isPayPalProviderPayload,
...
export type QuickPay = {
	...
	process3DSForQuickPay: (
		ev: MessageEvent<QuickPayIframeMessage>,
...
export function useQuickPay({
	...
	const [legalDisclosureTranslationKey, setLegalDisclosureTranslationKey] =
		useState<string>("");
	const [paymentProfiles, setPaymentProfiles] = useState<PaymentProfile[]>([]);
	const [preparePaymentProviderPayload, setPreparePaymentProviderPayload] =
		useState<PreparePaymentProviderPayload | null | undefined>();
	const [shouldRedirectToPurchase, setShouldRedirectToPurchase] =
		useState<boolean>(false);
	...
	const firePaymentMethodSelectedCounter = useCallback(
	...
	const fetchQuickPayMetadata = useCallback(async () => {
		...
		const eligibleProfiles = profiles.filter(
			...
			(profile) =>
				profile.isQuickPayEnabled === undefined ||
				profile.isQuickPayEnabled === true,
		);
	...
	const getRedirectUrl = (
		payload: ProcessPaymentProviderPayload,
	): string | undefined =>
	...
	const preparePaymentForQuickPay = useCallback(
		async (
			...
			const isPayPalPayment = isPayPalProviderPayload(
				paymentProfile.providerPayload,
			);
			...
			const data = await preparePayment(
				...
				.catch((err: unknown) => {
					...
					if (isGenericChallengeAbandonedError(err)) {
						...
						getAxiosResponseFailureReason(err) === "RemovedPaymentProfile"
					) {
	...
	const processPaymentForQuickPay = useCallback(
		async (
			...
			const paymentProviderType = isPayPalPayment
				? "BraintreePayPal"
				: "StripeCard";
			...
			if (!data) {
				...
				if (selectedProduct) {
					await preparePaymentForQuickPay(
						selectedProduct,
						isBonusItem,
	...
	const process3DSForQuickPay = useCallback(
		async (
			...
			if (
				ev.data.type !== "3DS-authentication-complete" ||
				!ev.data.url ||
				!stripe
			) {
				return;
