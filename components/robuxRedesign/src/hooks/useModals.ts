...
export type Modals = {
	purchaseDisabled: {
		...
		isOpen: boolean;
		openModal: (
			vpcOptimization: boolean,
			userDisablePurchases: boolean,
		) => void;
...
export function useModals(): Modals {
	...
	const [isPurchaseWarningModalOpen, setIsPurchaseWarningModalOpen] =
		useState<boolean>(false);
	const [isPurchaseDisabledModalOpen, setIsPurchaseDisabledModalOpen] =
		useState<boolean>(false);
	const [isQuickPayModalOpen, setIsQuickPayModalOpen] =
		useState<boolean>(false);
	const [isQuickPay3DSModalOpen, setIsQuickPay3DSModalOpen] =
		useState<boolean>(false);
	const [isRobuxGiftingModalOpen, setIsRobuxGiftingModalOpen] =
		useState<boolean>(false);
	const [isRedirectErrorModalOpen, setIsRedirectErrorModalOpen] =
		useState<boolean>(false);
	const [
		isSamsungPaymentMethodsModalOpen,
		setIsSamsungPaymentMethodsModalOpen,
	] = useState<boolean>(false);
	...
	const [
		...
		setSamsungPaymentMethodsProductIsBonus,
	] = useState<boolean>(false);
	const [
		samsungPaymentMethodsProductIsSubscription,
		setSamsungPaymentMethodsProductIsSubscription,
	] = useState<boolean>(false);
	const [samsungPaymentMethodsProduct, setSamsungPaymentMethodsProduct] =
		useState<Product | undefined>();

	const [hasUserDisabledPurchases, setHasUserDisabledPurchases] =
		useState(false);
	...
	const [
		...
		setIsFirstTimePurchaseConsentModalOpen,
	] = useState(false);
	const [
		firstTimePurchaseConsentCallback,
		setFirstTimePurchaseConsentCallback,
	...
	const openFirstTimePurchaseConsentModal = useCallback(
		(callback: () => void) => {
