...
export function useBuyRobuxPage(
	...
	const [bonusItemId, bonusItemDisplayName, bonusItemRootPlaceId] =
		useMemo(() => {
			const bonusItem = buyRobuxPageData.sections.find(
				(section) => section.personalizedBonus,
			...
			buyRobuxPageData.sections.reduce<LimitedTimeBonusItemFields>(
				(acc, section) => {
					for (const b of section.limitedTimeBonus?.limitedTimeBonuses ?? []) {
						...
						acc.ids.push(id);
						if (meta?.backgroundImage2dUrl)
							acc.bannerImageUrls.push(meta.backgroundImage2dUrl);
						if (meta?.creatorDisplayName)
							acc.creatorDisplayNames.push(meta.creatorDisplayName);
						if (meta?.translationKey)
							acc.displayNames.push(meta.translationKey);
	...
	const giftingUrl = useMemo(
		() =>
			buyRobuxPageData.sections.find(({ robuxGift }) => robuxGift?.giftingUrl)
				?.robuxGift?.giftingUrl ?? "",
		[buyRobuxPageData.sections],
	);
	...
	const subscriptionProductIds = useMemo(
		() =>
			buyRobuxPageData.sections.flatMap(
				(section) =>
					section.subscriptionV2?.products.map(
						(p) => p.subscriptionProductId,
					) ?? [],
			),
		[buyRobuxPageData.sections],
	);
