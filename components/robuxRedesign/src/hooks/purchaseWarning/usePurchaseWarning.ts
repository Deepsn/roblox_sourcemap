...
export function usePurchaseWarning(): {
	...
	useEffect(() => {
		...
		if (rootDataset) {
			setIsPurchaseWarningModalEnabled(
				rootDataset.isScaryModalEnabled === "true",
			);
		}
	}, []);
	...
	const isStoppedByPurchaseWarning = useCallback(
		async (
			...
			const is13To17ScaryModalEnabled =
				!isOnDesktop && !isSubscriber && !isSubscriptionProduct;
			...
			if (
				...
				(action ===
					PurchaseWarningAction.ParentalConsentWarningPaymentModal13To17 &&
					isInApp)
			) {
