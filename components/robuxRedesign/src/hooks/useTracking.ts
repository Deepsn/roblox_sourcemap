...
export type Tracking = {
	...
	trackStatus: (
		viewName: VIEW_NAME,
		status: PURCHASE_EVENT_STATUS,
		viewMessage?: string,
	) => void;
};

type VIEW_NAME = NonNullable<
	Parameters<typeof pfas.sendUserPurchaseFlowEvent>[2]
>;
type PURCHASE_EVENT_TYPE = NonNullable<
	Parameters<typeof pfas.sendUserPurchaseFlowEvent>[3]
>;
type VIEW_MESSAGE = NonNullable<
	Parameters<typeof pfas.sendUserPurchaseFlowEvent>[4]
>;

type PURCHASE_EVENT_STATUS = NonNullable<
	Parameters<typeof pfas.sendUserPurchaseStatusEvent>[1]
>;
...
export function useTracking(): Tracking {
	...
	const { seenProducts, seenSections, seenSubscriptionProducts } =
		useScrollTracking();
	...
	useEffect(() => {
		if (
			...
			seenSubscriptionProducts.length > 0
		) {
