...
export function usePurchaseEligibility(): {
	...
	const [vpcOptimizationEnabled, setVPCOptimizationEnabled] = useState<
		boolean | undefined
	>();
	...
	const fetchPurchasingDisabledBySelf =
		useCallback(async (): Promise<boolean> => {
	...
	const showVPCOptimization = useCallback(async (): Promise<void> => {
		const {
			texasU18VPCOptimizationEnabled,
			texasU18VPCOptimizationEnabledForEveryone,
	...
	const startAccessManagementUpsellFlow =
	...
	const isUserEligibleForPurchase = useCallback(async (): Promise<boolean> => {
		...
		if (userPurchaseSetting.expirationTimeInMinutes) {
			...
			const timeoutInHours = Math.ceil(
				userPurchaseSetting.expirationTimeInMinutes / 60,
			);
		...
		if (
			...
			userPurchaseSetting.failureReason ===
		...
		if (
			...
			openPurchaseDisabledModal(
				Boolean(vpcOptimizationEnabled),
				purchasingDisabledBySelf,
			);
			return false;
		}
