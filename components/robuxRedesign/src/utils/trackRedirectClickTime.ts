...
export type RedirectClickTimeBucket =
	| (typeof REDIRECT_CLICK_TIME_BUCKETS)[number]["label"]
	| "10+";

export function getRedirectClickTimeBucket(
	timeSincePageLoadMs: number,
): RedirectClickTimeBucket {
	const bucket = REDIRECT_CLICK_TIME_BUCKETS.find(
		({ maxMs }) => timeSincePageLoadMs <= maxMs,
	);
