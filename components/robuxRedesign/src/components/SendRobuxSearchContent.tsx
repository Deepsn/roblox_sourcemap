...
export function SendRobuxSearchContent({
	...
	const { searchText, setSearchText, users, isSearching, clearSearch } =
		useUserSearch({
	...
	const filteredFriends = useMemo(() => {
		...
		return friends.filter(
			(friend) =>
				normalizeFriendSearchText(friend.combinedName ?? "").includes(
	...
	const myFriendsHeading = isLoggedIn
		? translate("Label.MyFriendsWithCount", friendsCount)
		: "";
	...
	const showFriendsSection =
		isLoggedIn && (isLoading || filteredFriends.length > 0);
	const showNoResults =
				...
				<div className="flex flex-col padding-bottom-large">
					{showFriendsSection && (
						...
						<div
							...
							data-testid="send-robux-friends-section"
						>
							<h2
