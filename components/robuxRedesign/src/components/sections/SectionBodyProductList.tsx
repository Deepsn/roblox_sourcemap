...
} from "../ProductItem/ProductItemDefault";
...
export function SectionBodyProductList({
	...
	const hasCollapsible = useMemo(
		() => products.some((product) => product.isCollapsible),
		[products],
	);
		...
		<Fragment>
			{products.map((product) => (
				<ItemComponent
					...
					buttonProps={{
						className:
							"min-width-[90px] small:min-width-[120px] medium:min-width-[200px]",
					}}
				/>
			))}
			{hasCollapsible && (
