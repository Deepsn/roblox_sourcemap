...
export function OmniSearch({ onSelectUser }: OmniSearchProps) {
	...
	const showOverlay =
		users.length > 0 ||
		(!isSearching && searchText.trim().length >= USER_SEARCH_MIN_CHARACTERS);
	...
	const activeDescendantId =
		focusedIndex >= 0 && users[focusedIndex]
			? `user-${users[focusedIndex].contentId}`
			: undefined;
		...
		<div ref={searchAreaRef} className="relative">
			...
			{showOverlay && (
				<div
					...
					style={{ top: "calc(100% + 0.25rem)", zIndex: 10 }}
				>
					<OmniSearchResults
						users={users}
