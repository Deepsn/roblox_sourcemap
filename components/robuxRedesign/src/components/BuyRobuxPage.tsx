...
export function BuyRobuxPage() {
	...
	const reduceSections = useCallback(
		(sections: ReactElement[], section: Section) => {
			...
			if (redirect) {
				element = (
					<RedirectProductList {...baseProps} redirectOptions={redirect} />
				);
				...
				subscriptionV2.products.forEach((product) => {
					product.benefits?.forEach((benefit) => {
						if (benefit.plusDiscountSubscriptionBenefits) {
							...
							benefit.plusDiscountSubscriptionBenefits.periodType =
								product.periodType;
						}
					});
				});
				element = (
					<SubscriptionV2 {...baseProps} subscriptionV2={subscriptionV2} />
				);
			} else if (giftCard) {
				element = <GiftCard {...baseProps} giftCard={giftCard} />;
			} else if (limitedTimeBonus) {
				element = (
					<LimitedTimeBonusSection
						{...baseProps}
	...
	return (
		<div className="flex flex-col items-center">
			...
			<Header
				transfersSection={transfersSection}
				robuxGiftSection={robuxGiftSection}
			/>
			<div
				className={classNames(
					"buy-robux-content padding-x-large small:padding-x-xlarge",
					{
						"is-in-app": isInApp,
					},
				)}
