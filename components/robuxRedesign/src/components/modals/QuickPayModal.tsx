...
import {
	convertProviderPayloadMoneyToDecimal,
	formatAmount,
...
import {
	QuickPay,
	QuickPayIframeMessage,
} from "../../hooks/quickPay/useQuickPay";
...
function QuickPayModal({
	...
	const handleProcessPayment = useCallback(
		async (providerPayload: PreparePaymentProviderPayload) => {
			...
			const { error, isLoading: loading } =
				await processPaymentForQuickPay(providerPayload);
			if (error) {
	...
	const [amountTotal, taxAmountExclusive] = useMemo(() => {
		...
		return [
			convertProviderPayloadMoneyToDecimal(
				preparePaymentProviderPayload.amountTotal,
			),
			convertProviderPayloadMoneyToDecimal(
				preparePaymentProviderPayload.taxAmountExclusive,
			),
		];
	}, [preparePaymentProviderPayload]);
			...
			<Modal.Body>
							...
							<div className="bonus-item-card-small">
								...
								<div className="description-container">
									<div className="description-title">
										{ltbItemMetadata
											? translate(ltbItemMetadata.translationKey) ||
												"Bonus Item"
											: (bonusItemDisplayName ??
												translate(
													collectibleBonusItemMetadata?.translationKey ?? "",
												))}
									</div>
									<div className="description-subtitle">
										{ltbItemMetadata?.creatorDisplayName ? (
											<div className="flex flex-row gap-xsmall self-stretch items-center justify-start margin-top-[8px]">
												...
												{ltbItemMetadata.creatorIsVerified && (
				...
				<div className="price-info space-between-container">
					<div className="subtotal">
						{translate("Message.OneClickPay.Subtotal")}
					</div>
					<span
				...
				<div className="space-between-container">
					<div className="total-due">
						{translate("Message.OneClickPay.TotalDue")}
					</div>
					{preparePaymentProviderPayload ? (
						<span
				...
				<div className="use-a-different-payment-method">
					...
					<div
						role="button"
						tabIndex={0}
						onClick={handleUseDifferentPaymentMethodClick}
					>
				...
				<div className="margin-top-small">
					<QuickPayLegalDisclosure
						translationKey={legalDisclosureTranslationKey}
					/>
				</div>
			</Modal.Body>
			<Modal.Footer>
...
export const QuickPayModalWithLoading = (props: QuickPay) => {
	...
	return (
		<div>
			{props.isQuickPayLoading && (
				<FullScreenLoading ariaLabel={translate("Label.Loading")} />
			)}
			<QuickPayModal {...props} />
		</div>
	);
