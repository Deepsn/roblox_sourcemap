...
export function QuickPaySelectCardDropdown({
	...
	const { trackQuickPayClickPaymentMethodDropdown } =
		useContext(TrackingContext);

	const { translate } = useTranslation();

	const [showSelectionDropdown, setShowSelectionDropdown] =
		...
		({ CardNetwork, Last4Digits, ExpMonth, ExpYear }: Card) => {
			...
			return (
				<div className="payment-method-container">
					<span
						className={`payment-method-image card-icon ${getCardNetworkIcon(CardNetwork)}`}
					/>
					<span className="card-number">
						<span className="card-asterisk-mask">{ASTERISK_MASK}</span>
						<span className="payment-profile-label">{Last4Digits}</span>
					</span>
					<span className="card-exp-key">
						{translate("QuickPay.Expiration")}
					</span>
				...
				<Fragment>
					...
					>
						{paymentProfiles
							.filter(
								(paymentProfile) =>
									paymentProfile.id !== selectedPaymentProfile.id,
							)
							.map((paymentProfile) => (
								<button
									type="button"
