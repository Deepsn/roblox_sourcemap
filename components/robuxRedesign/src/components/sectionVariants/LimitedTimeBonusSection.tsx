	...
	return (
		...
		>
			...
			<Button
				...
				{...redirectProps}
				icon={
					redirectUrl ? "icon-regular-arrow-up-right-from-square" : undefined
				}
			>
				<PriceTag
...
function BonusItemCard({
	...
	const bannerHeightClass =
		totalCards > 1
			? "min-height-[120px]"
			: "min-height-[186px] medium:min-height-[204px]";
	...
	return (
		<div
			className="self-stretch radius-large flex flex-col"
			...
			<div className={`relative bg-shift-200 ${bannerHeightClass}`}>
				...
				<div
					className={classNames(
						"relative z-10 padding-xlarge flex height-full",
						bodyLayoutClass,
						{
...
export function LimitedTimeBonusSection({
	...
	const cards = useMemo(() => {
		...
		>((acc, item) => {
			const { collectibleItemMetadata, gamePassMetadata } =
				item.displayableBonus;
			if (!collectibleItemMetadata && !gamePassMetadata) {
				trackCounter("BonusItemMissingMetadata", {
					productType: "LimitedTimeBonus",
				});
				return acc;
			}
			...
			<div className="self-stretch flex flex-col gap-xlarge medium:gap-xxlarge">
				{cards.map(({ item, products }) => (
					<BonusItemCard
						...
						redirectUrl={
							limitedTimeBonus.isRedirect && redirect?.url
								? redirect.url
								: undefined
						}
