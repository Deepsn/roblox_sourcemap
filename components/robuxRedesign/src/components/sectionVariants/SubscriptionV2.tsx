...
export function SubscriptionV2({
	...
	const { paymentSession, breakpoint, redirect } =
	...
	const isFreeTrial = useMemo(
		() =>
			products.some(
				(p) => p.offers?.some((o) => Boolean(o.freeTrial)) ?? false,
			),
		[products],
	);
	...
	return (
		<Section
			data-testid="section-subscription-v2"
			{...getSectionTrackingProps(sectionBase)}
		>
			<SectionHeader>
			...
			{isInApp && (
				<SheetRoot
					open={isLearnMoreSheetOpen}
					onOpenChange={handleLearnMoreSheetOpenChange}
				>
					<LearnMoreSheet />
				</SheetRoot>
			)}
