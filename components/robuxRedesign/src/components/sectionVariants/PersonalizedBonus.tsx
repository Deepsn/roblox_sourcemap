...
const Banner = ({ personalizedBonus }: BannerProps) => {
	...
	if (productType === productTypeToValueMap.PRODUCT_TYPE_COLLECTIBLE_ITEM) {
		...
		productType === productTypeToValueMap.PRODUCT_TYPE_GAME_PASS_PRODUCT
	) {
		if (!gamePassMetadata) {
			trackCounter("BonusItemMissingMetadata", { productType });
			return null;
		...
		<Section {...getSectionTrackingProps(sectionBase)}>
			<div>
				...
				{personalizedBonus.bodyTranslationKey && (
					<SectionSubHeader>
						{translate(personalizedBonus.bodyTranslationKey)}
					</SectionSubHeader>
				)}
			</div>
			<SectionBody
				isPrimary={isPrimary}
				banner={<Banner personalizedBonus={personalizedBonus} />}
			>
