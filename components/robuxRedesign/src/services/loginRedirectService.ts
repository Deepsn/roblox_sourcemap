...
export const LOGIN_REDIRECT_PATHNAME_LAYER =
	"Payments.LoginRedirect.ExposureBased";
...
export class LoginRedirectService {
	...
	async getExperimentBasedPathname(): Promise<{
		pathname: string;
		...
		if (!userId) {
			return {
				pathname: DEFAULT_LOGIN_REDIRECT_PATHNAME,
				isInExperiment: false,
			};
		}

		const deviceMeta = getDeviceMeta();
		const appType = (deviceMeta?.appType ?? "unknown")
			.trim()
			.replace(/\s+/g, "_");
		...
		if (
			...
			iosMajorVersion < MIN_SUPPORTED_IOS_MAJOR_VERSION
		) {
		...
		const res = await service.getAllValuesForLayer(
			LOGIN_REDIRECT_PATHNAME_LAYER,
		);
		...
		if (typeof variant === "number") {
			...
			return {
				pathname:
					variant === 1
						? NEXTJS_LOGIN_REDIRECT_PATHNAME
