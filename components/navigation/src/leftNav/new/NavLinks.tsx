...
const ReferralNavItem = ({
	...
	const [showNewBadge, setShowNewBadge] = useState(() =>
		shouldShowReferralNewBadge(entry),
	);
	...
	const cardContent = (
		<Fragment>
			...
			<span className="min-width-0 text-truncate-end grow-1 text-align-x-left">
				{label}
			</span>
			{showNewBadge ? (
				<Badge
					label={translate("Label.New", undefined, "New")}
					variant="Contrast"
				/>
			) : null}
		</Fragment>
 ...
 */
const BlackbirdJoinReferralNavItem = ({
	referral,
}: {
	...
	return (
		<nav>
			<ul className="flex flex-col gap-small">
				...
				{pendingReferral ? (
					<BlackbirdJoinReferralNavItem referral={pendingReferral} />
				) : null}
				<NavItem
					path="/home"
