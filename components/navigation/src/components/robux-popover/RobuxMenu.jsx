...
function RobuxMenu({
	...
	const robuxBadgeStr = mapRobuxBadgeTypeToStr(robuxBadgeType);
	...
	useEffect(() => {
		...
		if (
			...
			creditDisplayConfig !==
				layoutConstant.creditDisplayConfigVariants.hideCreditAndRobux
		) {
	...
	return (
		<Fragment>
			<div className={isWalletDisplayed ? "" : "wallet-hidden"}>
				...
				{creditDisplayConfig !==
					layoutConstant.creditDisplayConfigVariants.control && (
					<li className="dropdown-wallet">
						<Link className="dropdown-wallet-section">
