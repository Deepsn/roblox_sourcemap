					...
					responseError: (rejection) => {
						...
						if (challengeAvailable) {
							const retryRequest = (
								...
								rejection.config.headers[GENERIC_CHALLENGE_ID_HEADER] =
									challengeIdInner;
								// eslint-disable-next-line no-param-reassign
								rejection.config.headers[GENERIC_CHALLENGE_TYPE_HEADER] =
									challengeTypeRaw;
								// eslint-disable-next-line no-param-reassign
							...
							if (Migrate.isSupportedByGrasshopper(challengeTypeRaw)) {
								return interceptChallenge({
									...
									legacyGenericRender:
										window.Roblox.AccountIntegrityChallengeService?.Generic
											.renderChallenge,
								});
							}
					...
					const retryRequest = (rejection) => {
						...
						if (retryService.isExponentialBackOffEnabled) {
							if (!rejectionConfig.exponentialBackOff) {
								rejectionConfig.exponentialBackOff =
									retryService.exponentialBackOff();
							}
						...
						$log.debug(
							`---- rejection.config.url ------${rejectionConfig.url}`,
						);
						$log.debug(
							`---- incrementalTimeout ------${rejectionConfig.incrementalTimeout}`,
						);
						...
						response: (response) => {
							...
							if (config.retryable && config.exponentialBackOff) {
								...
								const retryAttemptCount =
									config.exponentialBackOff.GetAttemptCount();
								$log.debug(
