...
const getShowIdentityVerificationFlow = async (
	...
	if (
		!playButtonOverlayWebFlag &&
		!(voiceOptInWebFlag || cameraOptInWebFlagO13)
	) {
		return {
			elegibleToSeeVoiceUpsell: false,
			universePlaceVoiceEnabledSettings: defaultUniverseSettings,
	...
	if (ExperimentationService.getAllValuesForLayer) {
		shouldShowUpsellO13 = Boolean(
			(await ExperimentationService.getAllValuesForLayer(avatarChatUpsellLayer))
				.ShouldShowUpsell,
		);
...
const PlayButtonContents = ({
	...
	const [elegibleToSeeVoiceUpsell, setElegibleToSeeVoiceUpsell] = useState<
		boolean | undefined
	>(undefined);
	const [showVoiceOptIn, setShowVoiceOptIn] = useState<boolean | undefined>(
		undefined,
	);
	const [showAvatarVideoOptIn, setShowAvatarVideoOptIn] = useState<
		boolean | undefined
	>(undefined);
	const [requireExplicitVoiceConsent, setRequireExplicitVoiceConsent] =
		useState<boolean | undefined>(undefined);
	const [useCameraU13Design, setUseCameraU13Design] = useState<
		boolean | undefined
	>(undefined);
	const [useVoiceUpsellV2Design, setUseVoiceUpsellV2Design] = useState<
		boolean | undefined
	>(undefined);
	...
	useEffect(() => {
		const fetchShowIdentityVerificationFlow = async () => {
			try {
				...
				setElegibleToSeeVoiceUpsell(
					showIdentityVerificationFlow.elegibleToSeeVoiceUpsell,
				);
				setShowVerification(
				...
				setShowAvatarVideoOptIn(
					showIdentityVerificationFlow.showAvatarVideoOptInOverlay,
				);
				...
				setUseVoiceUpsellV2Design(
					showIdentityVerificationFlow.useVoiceUpsellV2Design,
				);
			} catch {
				window.EventTracker?.fireEvent(
					counterEvents.PlayButtonShowIdentificationError,
				);
				...
				onClick={async (e) => {
					...
					if (status === PlayabilityStatus.Playable) {
						if (elegibleToSeeVoiceUpsell && isExperienceVoiceEnabled) {
							...
							const ixpPromise =
								ExperimentationService.getAllValuesForLayer(layerName);
							ixpPromise
								.then(() => {
...
export const PlayButton = (props: TPlayButtonProps) => {
	return (
