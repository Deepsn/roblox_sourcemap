...
const ConsentTool = ({
	...
	const isAnalyticsCookieAccepted =
		consentCookieHandler.isAnalyticsCookieAccepted();
	const [acceptAnalyticsCookie, updateConsent] = useState<boolean>(
	...
	const privacyPolicyUrl = urlService.getUrlWithLocale(
		typeof urlConstants.privacyPolicyUrl === "string"
	...
	const supportUrl =
		typeof urlConstants.supportUrl === "string" ? urlConstants.supportUrl : "";
	...
	const GAToggle = (
		<div>
			...
			<Toggle
				...
				onToggle={() => {
					updateConsent(!acceptAnalyticsCookie);
				}}
			/>
		</div>
	);
	...
	return (
		<Modal
			...
			onNeutral={() => {
				closeConsentTool(cookieModule.banner);
			}}
