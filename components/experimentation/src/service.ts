...
export default class ExperimentationService {
	...
	public getAllValuesForLayer(
		...
		return post<BulkEnrollmentResponse>(urlConfig, requestBody).then(
			(response) => {
	...
	private static getInputMetadataKey(
		layerName: string,
		inputs: ExperimentationInputs,
	): string {
	...
	private logLayerExposureForMetadataKey(
		...
		const bulkEnrollmentResponse =
			this.projectLayerMetadataMaps[projectId]?.get(metadataKey);
