 ...
 */
const MaybeLinkActionWrapper = ({ containerClassName, callback, linkPath, ariaLabel, tabIndex, onFocus, onFocusLost, dataTestId, children }) => {
    const { classes: { linkContainerOverride, buttonContainerOverride } } = useStyles$c({});
    if (linkPath) {
        return (React.createElement("a", { href: linkPath, onClick: e => {
                e.stopPropagation();
                if (callback) {
                    callback();
                }
            ...
            }, className: classNames(containerClassName, linkContainerOverride), "aria-label": ariaLabel, tabIndex: tabIndex, onFocus: onFocus, onMouseOver: onFocus, onBlur: onFocusLost, onMouseLeave: onFocusLost, "data-testid": dataTestId }, children));
    }
    if (callback) {
...
const { linkStart: linkStartDelimiter, linkEnd: linkEndDelimiter } = linkDelimiters;
/**
 * Adds link decoration if necessary to a text string that may include linkStart and linkEnd delimiters
 *
 * @returns parsedTextContent: React node output that includes styling (bold/underline) for the link text.
 * @returns cleansedTextLabel: Cleansed text output without the delimeters for the aria-label.
 */
const useFormatPossibleLinkDecoration = (text) => {
    return useMemo(() => {
        if (!text) {
            return {
                parsedTextContent: '',
        ...
        const startIdx = text.indexOf(linkStartDelimiter);
        const endIdx = text.indexOf(linkEndDelimiter);
        if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
            const prefix = text.slice(0, startIdx);
            const linkText = text.slice(startIdx + linkStartDelimiter.length, endIdx);
            const suffix = text.slice(endIdx + linkEndDelimiter.length);
            return {
                parsedTextContent: (React.createElement(React.Fragment, null,
                    prefix,
                    React.createElement("b", null,
                        React.createElement("u", null, linkText)),
                    suffix)),
                cleansedTextLabel: `${prefix}${linkText}${suffix}`
            };
        }
        return {
            parsedTextContent: text,
            cleansedTextLabel: text
        };
    }, [text]);
};
...
const useStyles$b = makeStyles()((_theme, { gap, textColor, fontStyle, containerOverrideStyles, textOverrideStyles }) => ({
    textIconRow: {
        display: 'inline-flex',
        alignItems: 'center',
        gap: `${gap}px`,
...
const TextIconRow = ({ onActivated, linkPath, text, textColor, fontStyle, gap = 0, iconClassName, iconWidth, iconColor, iconFirst = false, containerOverrides, textOverrides, iconOverrides }) => {
    ...
    const { classes: { textIconRow, textIconRowText, textOverride, iconBaseStyles } } = useStyles$b({
        gap,
        textColor,
        fontStyle,
        containerOverrideStyles,
    ...
    const { parsedTextContent, cleansedTextLabel } = useFormatPossibleLinkDecoration(text);
 ...
 * />
 */
const useStyle$6 = makeStyles({ name: 'AttributionTextContent' })((_theme, { subtitleMaxLines, subtitleFontStyle, textColor, textGap }) => {
    return {
...
const AttributionTextContent = ({ title, subtitle, textColor, titleFontStyle, subtitleFontStyle, subtitleMaxLines = 1, textGap = 0 }) => {
    ...
    return (React.createElement("div", { className: classes.attributionTextContentContainer, "data-testid": 'attribution-text-content-container' },
        React.createElement(TextIconRow, { text: title, fontStyle: titleFontStyle, textColor: textColor }),
 ...
 * />
 */
const AttributionRow = ({ title, subtitle, textColor, height, imageComponent, rightButtonContent, titleFontStyle, subtitleFontStyle, subtitleMaxLines, titleSubtitleGap }) => {
    ...
    return (React.createElement("div", { className: attributionRowContainer },
        ...
        React.createElement(AttributionTextContent, { title: title, subtitle: subtitle, textColor: textColor, titleFontStyle: titleFontStyle, subtitleFontStyle: subtitleFontStyle, subtitleMaxLines: subtitleMaxLines, textGap: titleSubtitleGap }),
...
const useStyles$a = makeStyles()((_theme, { itemWidth, columnGap, sideMargin, gapBetweenHeaderAndItems, scrollArrowBackgroundColor, scrollArrowBoxShadowColor, scrollArrowBaseClassName, scrollArrowPrevClassName, scrollArrowNextClassName }) => ({
...
const CollectionCarousel = ({ itemsContainerRef, items, renderItem, collectionItemSize, updateItemsPerRow, layoutOverrides, headerComponent, gapBetweenHeaderAndItems, isHorizontalScrollEnabled, scrollArrowBackgroundColor, scrollArrowBoxShadowColor, thresholdFromEnd, onReachedThresholdFromEnd, reportHorizontalScrollTelemetry }) => {
    ...
    const { classes: { collectionCarouselContainer, carouselContainer, carousel, carouselItem } } = useStyles$a({
        itemWidth,
        columnGap,
        sideMargin,
...
const useStyles$9 = makeStyles()((_theme, { itemWidth, columnGap, sideMargin, gapBetweenHeaderAndItems }) => ({
...
const CollectionGrid = ({ itemsContainerRef, items, renderItem, collectionItemSize, updateItemsPerRow, layoutOverrides, headerComponent, gapBetweenHeaderAndItems }) => {
    ...
    const { classes: { collectionGridContainer, grid, gridItem } } = useStyles$9({
        itemWidth,
        columnGap,
        sideMargin,
        gapBetweenHeaderAndItems
...
const useStyles$8 = makeStyles()((_theme, { iconWidth, avatarContainerBackgroundColor, avatarImageBackgroundColor, avatarBorderColor, maxZIndex }) => ({
    facepileContainer: {
        width: 'fit-content',
        display: 'flex',
        alignItems: 'center'
...
const GameTileActiveFriendsFacepile = ({ avatarThumbnails, iconWidth, avatarContainerBackgroundColor, avatarImageBackgroundColor, avatarBorderColor }) => {
    const { classes: { facepileContainer, avatarContainer } } = useStyles$8({
        iconWidth,
...
const defaultTitleImageHeightPercentCompact = 0.25;
const defaultTitleImageHeightPercentLarge = 0.3;

const useStyles$7 = makeStyles()((_theme, { hasTitleImage, forceSmallView, forceMediumView, forceSmallOrMediumView, minCardHeight, titleImageHeightPercentLarge, titleImageHeightPercentCompact, titleImageAspectRatio }) => ({
...
const HeroUnitContent = ({ title, subtitle, titleImageComponent, heroUnitRef, gradient, gradientHeightPercent, gradientWidthPercent, bottomRowComponent, overlayPillComponent, forceViewportWidth, titleImageAspectRatio = 3, titleImageHeightPercentage, minCardHeight }) => {
    ...
    const titleImageHeightPercentCompact = titleImageHeightPercentage ?? defaultTitleImageHeightPercentCompact;
    const titleImageHeightPercentLarge = titleImageHeightPercentage ?? defaultTitleImageHeightPercentLarge;
    const { classes: { heroUnitContentContainer, heroUnitTitleContainer, heroUnitTitle, heroUnitSubtitle, heroUnitTitleImageContainer } } = useStyles$7({
    ...
    const titleComponent = titleImageComponent ? (React.createElement("div", { className: heroUnitTitleImageContainer }, titleImageComponent)) : (React.createElement("span", { className: heroUnitTitle }, title));
...
const HeroUnitBackground = ({ backgroundImageComponent, forceViewportWidth, minCardHeight }) => {
...
const useStyles$6 = makeStyles()((_theme, { forceViewportWidth, maxForegroundHeightPercentString, forceSmallOrMediumView, minForegroundHeightPercentString, foregroundScaleFactor, foregroundAspectRatio }) => ({
    heroUnitContainer: {
        width: '100%',
        position: 'relative',
        display: 'flex',
...
const HeroUnit = ({ title, subtitle, titleImageComponent, foregroundImageComponent, backgroundImageComponent, gradient, gradientHeightPercent = 0.5, gradientWidthPercent = 1, backgroundClickAction: onBackgroundClick, backgroundClickLinkPath, bottomRowComponent, overlayPillComponent, minForegroundHeightPercent = 0.8, maxForegroundHeightPercent = 1, forceViewportWidth, titleImageAspectRatio = 3, titleImageHeightPercentage, minCardHeight = 262, foregroundAspectRatio = 1 }) => {
    ...
    const { classes: { heroUnitContainer, heroUnitForegroundContainer, heroUnitTopSpacer } } = useStyles$6({
        forceViewportWidth,
    ...
    return (React.createElement("div", { ref: heroUnitContainerRef },
        React.createElement(MaybeLinkActionWrapper, { containerClassName: heroUnitContainer, callback: onBackgroundClick, linkPath: backgroundClickLinkPath, ariaLabel: title, dataTestId: 'hero-unit' },
            ...
            React.createElement(HeroUnitBackground, { backgroundImageComponent: backgroundImageComponent, forceViewportWidth: forceViewportWidth, minCardHeight: minCardHeight }),
...
const useStyles$5 = makeStyles()((_theme, { verticalGap, containerOverrideStyles }) => ({
    sectionHeader: {
        position: 'relative',
        display: 'flex',
...
iconComponent, containerOverrides }) => {
    ...
    const { classes: { sectionHeader, titleSubtitleContainer } } = useStyles$5({
        verticalGap,
        containerOverrideStyles
    });
...
const useStyles$4 = makeStyles()((_theme, { padding, containerOverrideStyles }) => {
...
const SlotOverlay = ({ topLeftSlot, topMiddleSlot, topRightSlot, centerLeftSlot, centerMiddleSlot, centerRightSlot, bottomLeftSlot, bottomMiddleSlot, bottomRightSlot, padding = 0, containerOverrides, children }) => {
    ...
    const { classes } = useStyles$4({ padding, containerOverrideStyles });
...
export { AttributionRow, AttributionTextContent, CollectionCarousel, CollectionGrid, CollectionItemSize, CountdownTextIconRow, CtaButton, DetailsPageHeader, GameTileActiveFriendsFacepile, Gradient, HeroUnit, ImageWithGradient, MaybeLinkActionWrapper, OverlayPill, Page, SectionHeader, SlotOverlay, TAutomaticSize, TextIconRow, TextPill, Tile, TileFooter, VerticalFeed };
