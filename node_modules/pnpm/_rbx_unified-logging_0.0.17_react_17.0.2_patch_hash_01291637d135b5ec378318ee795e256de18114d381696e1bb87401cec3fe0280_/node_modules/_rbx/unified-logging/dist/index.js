...
const getSortedImpressedIndexes = (tileElements, entries, observer) => {
...
const useCollectionItemsIntersectionTracker = (containerRef, childrenLength, onImpressionsDetected) => {
    ...
    const impressedRangeRef = useRef(new Set());
    const disconnectRef = useRef(null);
    const sendImpressions = useCallback(() => {
        const impressedRangeArray = Array.from(impressedRangeRef.current);
        const subArrays = splitArray(impressedRangeArray.filter(index => !reportedImpressionsRef.current.has(index)), maxItemsInImpressionsEvent).filter((indexes) => indexes.length > 0);
        subArrays.forEach((indexesToSend) => {
            onImpressionsDetected(indexesToSend);
            indexesToSend.forEach(index => reportedImpressionsRef.current.add(index));
        });
        impressedRangeRef.current.clear();
    }, [onImpressionsDetected]);
    const sendImpressionsDebounced = useMemo(() => {
        return debounce(() => sendImpressions(), 200);
    }, [sendImpressions]);
    useEffect(() => {
        ...
        const onObserve = (entries, observer) => {
            ...
            const sortedImpressedIndexes = getSortedImpressedIndexes(tileElements, entries, observer);
            sortedImpressedIndexes.forEach(index => impressedRangeRef.current.add(index));
            sendImpressionsDebounced();
        };
        // Disconnect the previous observer, if one exists
    ...
    }, [containerRef, childrenLength, sendImpressionsDebounced, impressedRangeRef]);
};
...
(function (EventContext) {
    EventContext["Games"] = "Games";
    EventContext["Home"] = "Home";
    EventContext["SearchLanding"] = "SearchLanding";
    EventContext["Spotlight"] = "Spotlight";
    EventContext["UserProfile"] = "UserProfile";
    EventContext["GameDetail"] = "GameDetail";
    EventContext["SongList"] = "SongList";
    EventContext["SearchPage"] = "SearchPage";
    EventContext["PreAuthLanding"] = "PreAuthLanding";
...
(function (SessionInfo) {
    ...
    SessionInfo["SpotlightPageSessionInfo"] = "spotlightPageSessionInfo";
    SessionInfo["PreAuthLandingPageSessionInfo"] = "preAuthLandingPageSessionInfo";
...
(function (ItemActionMetadata) {
    ...
    ItemActionMetadata["ItemComponentType"] = "itemComponentType";
