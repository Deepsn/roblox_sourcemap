 ...
 */
export const isRecord = (v: unknown): v is Record<string, unknown> => v !== null && typeof v === 'object';
...
export const getByPath = (object: JsonValue, path: string): JsonValue | undefined => {
  const value = path
    .split('.')
    .reduce((objectOrChild, keyPart) => (isRecord(objectOrChild) ? objectOrChild[keyPart] : undefined), object);
  return value;
};
...
export const resolveMap = {
  $resource: {
    is: (value: object): value is ResourceStatement => '$resource' in value && typeof value.$resource === 'string',
    resolve: (stmt: ResourceStatement, { resources }: Stores): JsonValue | undefined => resources[stmt.$resource]?.value
  },
  $attribute: {
    is: (value: object): value is AttributeStatement => '$attribute' in value && typeof value.$attribute === 'string',
    resolve: (stmt: AttributeStatement, { attributes }: Stores): JsonValue | undefined => attributes[stmt.$attribute]
  },
  $store: {
    is: (value: object): value is StoreStatement => '$store' in value && typeof value.$store === 'string',
    resolve: (stmt: StoreStatement, { store }: Stores): JsonValue | undefined => getByPath(store, stmt.$store)
  },
  $switch: {
    is: (value: object): value is SwitchStatement<JsonValue> => '$switch' in value && Array.isArray(value.$switch),
    resolve: (stmt: SwitchStatement<JsonValue>): { value: JsonValue | undefined; caseIndex: number } => {
