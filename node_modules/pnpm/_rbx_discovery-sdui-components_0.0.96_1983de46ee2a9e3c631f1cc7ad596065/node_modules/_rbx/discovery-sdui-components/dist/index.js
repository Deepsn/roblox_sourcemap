...
/*
  Returns the styles for the text, wrapped in the data attribute.
  Overrides the padding of the text to 0.
*/
 ...
 * @param {TWebTextElement} titleWebTextElement - The element to be used for the title, defaults to span if not provided.
 ...
 * @param {TWebTextElement} subtitleWebTextElement - The element to be used for the subtitle, defaults to span if not provided.
...
const AttributionTextContent = ({ title, titleWebTextElement: TitleWebTextElement = 'span', titleComponent, subtitle, subtitleWebTextElement: SubtitleWebTextElement = 'span', textColor, titleFontStyle, subtitleFontStyle, subtitleMaxLines = 1, textGap = 0, titleMaxLines = 1 }) => {
    ...
    return (React.createElement("div", { className: classes.attributionTextContentContainer, "data-testid": 'attribution-text-content-container' },
        titleComponent || (React.createElement(TitleWebTextElement, { className: classes.attributionTitle, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, title)),
        subtitle && (React.createElement(SubtitleWebTextElement, { className: classes.attributionSubtitle, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, subtitle))));
};
 ...
 * />
 */
const AttributionRow = ({ title, titleWebTextElement, titleComponent, subtitle, subtitleWebTextElement, textColor, height, imageComponent, rightButtonContent, titleFontStyle, subtitleFontStyle, subtitleMaxLines, titleSubtitleGap, titleMaxLines }) => {
    ...
    return (React.createElement("div", { className: attributionRowContainer },
        ...
        React.createElement(AttributionTextContent, { title: title, titleWebTextElement: titleWebTextElement, titleComponent: titleComponent, subtitle: subtitle, subtitleWebTextElement: subtitleWebTextElement, textColor: textColor, titleFontStyle: titleFontStyle, subtitleFontStyle: subtitleFontStyle, subtitleMaxLines: subtitleMaxLines, textGap: titleSubtitleGap, titleMaxLines: titleMaxLines }),
...
/**
 * If a linkPath is specified, the container element will be an <a> tag (or LinkComponent if provided).
 ...
 */
const MaybeLinkActionWrapper = ({ containerClassName, callback, linkPath, ariaLabel, openInNewTab, tabIndex, onFocus, onFocusLost, dataTestId, children, permitLinkClickPropagation, linkComponent: LinkComponent }) => {
    const { classes: { linkContainerOverride, buttonContainerOverride } } = useStyles$f({});
    if (linkPath) {
        const handleLinkActivation = (e) => {
            if (permitLinkClickPropagation !== true) {
                e.stopPropagation();
            }
            if (callback) {
                callback();
        ...
        const linkProps = {
            ...
            className: classNames(containerClassName, linkContainerOverride),
        ...
        if (LinkComponent) {
            return (React.createElement(LinkComponent, { to: linkPath, ...linkProps }, children));
        }
        return (React.createElement("a", { href: linkPath, ...linkProps }, children));
    }
    if (callback) {
...
const VALID_WEB_TEXT_ELEMENTS = ['span', 'div', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6'];
const WebTextElementValidSet = new Set(VALID_WEB_TEXT_ELEMENTS);
...
const TextIconRow = ({ onActivated, linkPath, text, textColor, fontStyle, gap = 0, iconClassName, iconWidth, iconColor, iconFirst = false, containerOverrides, textOverrides, iconOverrides, webTextElement: WebTextElement = 'span', permitLinkClickPropagation, linkComponent }) => {
    ...
    const textElement = useMemo(() => {
        return (React.createElement(WebTextElement, { className: classNames(textIconRowText, textOverride), "data-testid": 'text-icon-row-text', ...getTextDataAttribute(defaultTextDataAttributeConfig) }, parsedTextContent));
    }, [parsedTextContent, textIconRowText, textOverride, WebTextElement]);
    ...
    const textIconRowContent = iconFirst ? (React.createElement(React.Fragment, null,
        ...
        textElement,
        iconElement));
    return (React.createElement(MaybeLinkActionWrapper, { containerClassName: textIconRow, callback: onActivated, linkPath: linkPath, ariaLabel: cleansedTextLabel, dataTestId: 'text-icon-row', permitLinkClickPropagation: permitLinkClickPropagation, linkComponent: linkComponent }, textIconRowContent));
};
...
const foregroundInitialOffset = 8;
const foregroundSquareExtraTravel = 60;
...
const useStyles$a = makeStyles()((_theme, { minCardHeight, forceSmallOrMediumView, enableBackgroundAnimation }) => ({
    ...
    heroUnitBackgroundContainer: {
        ...
        ...(enableBackgroundAnimation && {
            transform: 'translateY(calc(var(--parallax-scale) * var(--scroll) * -0.5px))',
...
const customEasing = (t, p0, p1, p2, p3) => (1 - t) ** 3 * p0 + 3 * (1 - t) ** 2 * t * p1 + 3 * (1 - t) * t ** 2 * p2 + t ** 3 * p3;
const easeBidirectional = (t) => {
    const clampedT = Math.max(-1, Math.min(t, 1));
    if (clampedT >= 0) {
        return customEasing(clampedT, 0, 0.3, 0.6, 1);
    }
    return -customEasing(-clampedT, 0, 0.3, 0.6, 1);
};
const captureInitialNormalizedTopPosition = (normalizedDistanceFromTop, rectTop) => {
    if (normalizedDistanceFromTop >= 1) {
        return 1;
    }
    if (rectTop > 0) {
        return normalizedDistanceFromTop;
    }
    return 1;
};
const calculateScrollProgress = (normalizedDistanceFromTop, initialNormalizedTopPosition, normalizedHeroUnitHeight) => {
    if (normalizedDistanceFromTop >= 0) {
        return Math.min(1, normalizedDistanceFromTop / initialNormalizedTopPosition);
    }
    return normalizedHeroUnitHeight > 0
        ? Math.max(-1, normalizedDistanceFromTop / normalizedHeroUnitHeight)
        : -1;
};
const calculateForegroundParallaxMagnitudes = ({ isSmallView, foregroundAspectRatio, bgHeight }) => {
    const foregroundOverflow = isSmallView ? foregroundParallaxSmall : foregroundParallaxLarge;
    const containerHeight = bgHeight + foregroundOverflow;
    const fgHeight = bgHeight / foregroundAspectRatio;
    const fgMax = foregroundAspectRatio === 1
        ? foregroundOverflow + foregroundInitialOffset
        : foregroundOverflow - foregroundInitialOffset;
    let fgMin = -(fgHeight - containerHeight);
    if (foregroundAspectRatio === 1) {
        fgMin -= foregroundSquareExtraTravel;
    }
    return {
        fgMax,
        fgMinMagnitude: Math.abs(fgMin)
    };
};

/* eslint-disable jsx-a11y/click-events-have-key-events */
/* eslint-disable jsx-a11y/no-static-element-interactions */
const useStyles$9 = makeStyles()((_theme, { forceViewportWidth, forceSmallOrMediumView, foregroundAspectRatio, enableForegroundAnimation }) => ({
    heroUnitContainer: {
        width: '100%',
        position: 'relative',
        display: 'flex',
    ...
    heroUnitForegroundContainer: {
        width: 'var(--hero-unit-content-height, 336px)',
        ...
        ...(enableForegroundAnimation && {
            transform: 'translateY(calc(' +
                'max(var(--scroll), 0) * var(--foreground-max-magnitude-px, 0px) + ' +
                'min(var(--scroll), 0) * var(--foreground-min-magnitude-px, 0px)' +
                '))',
            '@media (prefers-reduced-motion)': {
                // Disable parallax effect for users that prefer reduced motion
                transform: 'translateY(0px)'
 ...
 */
const HeroUnit = ({ title, subtitle, titleImageComponent, foregroundImageComponent, backgroundImageComponent, gradient, gradientHeightPercent = 0.5, gradientWidthPercent = 1, backgroundClickAction: onBackgroundClick, backgroundClickLinkPath, bottomRowComponent, overlayPillComponent, forceViewportWidth, titleImageAspectRatio = 3, titleImageHeightPercentage, minCardHeight = 262, foregroundAspectRatio = 1, enableBackgroundAnimation = true, enableForegroundAnimation = true }) => {
    const initialNormalizedTopPositionRef = React.useRef(null);
    const isInitialTopPositionFinalizedRef = React.useRef(false);
    const forceSmallOrMediumView = forceViewportWidth !== undefined && forceViewportWidth <= 600;
    ...
    const updateParallaxCss = useCallback(() => {
        if (!heroUnitContentRef.current || !heroUnitContainerRef.current || !window.innerHeight) {
            return;
        }
        const rect = heroUnitContentRef.current.getBoundingClientRect();
        const normalizedDistanceFromTop = rect.top / window.innerHeight;
        if (!isInitialTopPositionFinalizedRef.current) {
            initialNormalizedTopPositionRef.current = captureInitialNormalizedTopPosition(normalizedDistanceFromTop, rect.top);
        }
        const initialNormalizedTopPosition = initialNormalizedTopPositionRef.current;
        if (initialNormalizedTopPosition == null) {
            return;
        }
        const scrollProgress = calculateScrollProgress(normalizedDistanceFromTop, initialNormalizedTopPosition, rect.height / window.innerHeight);
        const easedScrollProgress = easeBidirectional(scrollProgress);
        const isSmallView = forceSmallOrMediumView || window.matchMedia('(max-width: 600px)').matches;
        const { fgMax, fgMinMagnitude } = calculateForegroundParallaxMagnitudes({
            isSmallView,
            foregroundAspectRatio,
            bgHeight: rect.height
        });
        const root = heroUnitContainerRef.current;
        root.style.setProperty('--scroll', easedScrollProgress.toString());
        root.style.setProperty('--hero-unit-content-height', `${rect.height}px`);
        root.style.setProperty('--foreground-max-magnitude-px', `${fgMax}px`);
        root.style.setProperty('--foreground-min-magnitude-px', `${fgMinMagnitude}px`);
    }, [forceSmallOrMediumView, foregroundAspectRatio]);
    ...
    useEffect(() => {
        ...
        const observer = new MutationObserver(throttle(updateParallaxCss, 100));
        // if first scroll hasn't happened yet, observe body to indirectly detect hero unit position change
        if (document.body && !isInitialTopPositionFinalizedRef.current) {
        ...
        updateParallaxCss();
        // finalize initial top position after first scroll event
        const updateParallaxCssAndFinalizeInitialTopPosition = () => {
            updateParallaxCss();
            isInitialTopPositionFinalizedRef.current = true;
            observer.disconnect();
        };
        window.addEventListener('scroll', updateParallaxCssAndFinalizeInitialTopPosition);
        window.addEventListener('resize', updateParallaxCssAndFinalizeInitialTopPosition);
        return () => {
            window.removeEventListener('scroll', updateParallaxCssAndFinalizeInitialTopPosition);
            window.removeEventListener('resize', updateParallaxCssAndFinalizeInitialTopPosition);
            observer.disconnect();
        };
    }, [updateParallaxCss]);
...
const SectionHeader = ({ onTitleActivated, titleText, titleLinkPath, titleTextColor, titleFontStyle, titleWebTextElement, titleGap, titleIconClassName, titleIconWidth, titleIconColor, titleIconFirst = false, titleComponent, titleLinkComponent, onSubtitleActivated, subtitleLinkPath, subtitleText, subtitleTextColor, subtitleFontStyle, subtitleWebTextElement, subtitleGap, subtitleIconClassName, subtitleIconWidth, subtitleIconColor, subtitleIconFirst = false, subtitleComponent, subtitleLinkComponent, verticalGap = 0, 
// onInfoIconActivated,
// iconClassName,
// iconWidth = 16,
// infoText,
iconComponent, containerOverrides, permitLinkClickPropagation }) => {
    const containerOverrideStyles = useMemo(() => {
        if (containerOverrides) {
            return guiPropsToStyle(containerOverrides);
        }
    ...
    const titleElement = useMemo(() => {
        ...
        if (titleText !== undefined && titleTextColor !== undefined && titleFontStyle !== undefined) {
            return (React.createElement(TextIconRow, { text: titleText, textColor: titleTextColor, fontStyle: titleFontStyle, gap: titleGap, iconClassName: titleIconClassName, iconWidth: titleIconWidth, iconColor: titleIconColor, iconFirst: titleIconFirst, webTextElement: titleWebTextElement }));
        }
        return null;
    }, [
        titleComponent,
    ...
    const wrappedTitleSubtitleContent = useMemo(() => {
        return (React.createElement(MaybeLinkActionWrapper, { containerClassName: titleSubtitleContainer, callback: onTitleActivated, linkPath: titleLinkPath, ariaLabel: titleText, dataTestId: 'section-header-title-subtitle-container', permitLinkClickPropagation: permitLinkClickPropagation, linkComponent: titleLinkComponent },
            titleElement,
            subtitleElement));
    }, [
        onTitleActivated,
 ...
 * @param {string} textColor - The color of the text to be displayed.
 * @param {TWebTextElement} webTextElement - The element to be used for the text, defaults to span if not provided.
 *
 * @example
 * <Text
 *   text="Sample Text."
 ...
 * />
 */
const Text = ({ text, textFontStyle, textColor, webTextElement: WebTextElement = 'span' }) => {
    const { classes: { textElement } } = useStyles$5({
        textFontStyle,
        textColor
    });
    return (React.createElement(WebTextElement, { className: textElement, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, text));
};
 ...
 * />
 */
const WebText = ({ text, textFontStyle, textColor, webTextElement: WebTextElement = 'span' }) => {
    const { classes: { textClassName } } = useStyles$4({
        textFontStyle,
        textColor
    });
    const sanitizedText = React.useMemo(() => domPurify.sanitize(text, DOM_PURIFY_CONFIG), [text]);
    return (React.createElement(WebTextElement, { className: textClassName, ...getTextDataAttribute(defaultTextDataAttributeConfig), 
 ...
 */
const TileBottomContent = ({ titleText, titleLines, titleFontStyles, titleColor, titleWebTextElement: TitleWebTextElement = 'div', titleComponent, footerComponent, ctaButtonComponent, isContained, containmentPadding }) => {
    ...
    const titleContent = useMemo(() => {
        ...
        if (titleText) {
            return (React.createElement("div", { className: tileTitleContainer, "data-testid": 'tile-title-container' },
                React.createElement(TitleWebTextElement, { className: tileTitleText, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, titleText)));
        }
        return null;
    }, [titleComponent, titleText, tileTitleContainer, tileTitleText, TitleWebTextElement]);
...
const Tile = ({ isFocused, imageComponent, imageAspectRatio, thumbnailOverlayComponent, onActivated, linkPath, isContained, containmentPadding, containmentBackgroundColor, cornerRadius, titleText, titleColor, titleFont, titleLines, titleWebTextElement, titleComponent, footerComponent, ctaButtonComponent, isOnScreen, placeholderImageBackgroundColor }) => {
    ...
    return (React.createElement(MaybeLinkActionWrapper, { containerClassName: tileContainer, callback: onActivated, linkPath: linkPath, tabIndex: isOnScreen ? 0 : -1, onFocus: onFocus, onFocusLost: onFocusLost, ariaLabel: titleText },
        ...
        React.createElement(TileBottomContent, { titleText: titleText, titleLines: titleLines, titleColor: titleColor, titleFontStyles: titleFont, titleWebTextElement: titleWebTextElement, titleComponent: titleComponent, footerComponent: footerComponent, ctaButtonComponent: ctaButtonComponent, isContained: isContained, containmentPadding: containmentPadding ?? 0 })));
};
...
const TileFooter = ({ onActivated, linkPath, textColor, fontStyle, textIconGap, sectionGap, iconWidth, iconColor, leftIcon, leftIconComponent, leftText, leftWebTextElement: LeftWebTextElement = 'div', rightIcon, rightIconComponent, rightText, rightWebTextElement: RightWebTextElement = 'div' }) => {
    const textHeight = fontStyle.LineHeight * fontStyle.FontSize;
    // Default icon color and width to text color and height
    const derivedIconColor = iconColor ?? textColor;
    const derivedIconWidth = iconWidth ?? textHeight;
    ...
    const tileFooterContent = useMemo(() => (React.createElement(React.Fragment, null,
        React.createElement("div", { className: leftContainer },
            ...
            React.createElement(LeftWebTextElement, { className: textClassName, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, leftText)),
        React.createElement("div", { className: rightContainer },
            ...
            rightText && (React.createElement(RightWebTextElement, { className: textClassName, ...getTextDataAttribute(defaultTextDataAttributeConfig) }, rightText))))), [
        leftContainer,
        rightContainer,
        textClassName,
        leftIconDisplay,
...
export { AssociatedItemsRow, AttributionRow, AttributionTextContent, CollectionCarousel, CollectionGrid, CollectionItemSize, CountdownTextIconRow, CtaButton, DetailsPageHeader, GameTileActiveFriendsFacepile, Gradient, HeroUnit, HeroUnitBottomRow, ImageWithGradient, MaybeLinkActionWrapper, OverlayPill, Page, SectionHeader, SlotOverlay, TAutomaticSize, Text, TextIconRow, TextPill, Tile, TileFooter, VerticalFeed, WebText, WebTextElementValidSet, useElementWidthResizeObserver };
